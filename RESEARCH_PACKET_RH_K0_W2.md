...
# MIG-026 Addendum — Self-Verifying Certificate Closure

Three packaging defects from the MIG-025 review, all reproduced and fixed:

1. **Malformed UTF-8** at byte 9044 of this packet (M₄ stored as `M 82 84`, missing the E2 lead byte) repaired; validator **check 9** now strict-UTF-8-decodes every tracked text file and fails on any violation (adversarially confirmed: reintroducing the bad byte is rejected).
2. **Ambiguous provenance** resolved. `git_commit` (which merely recorded whatever was checked out at generation time, not a pointer to the generator source) is replaced by `generation_start_commit` plus `generator_script_sha256` (the SHA-256 of the exact `weil_engine.py` that produced the certificate). The commit *containing* the certificates is recorded separately, post-commit, in `release/CERTIFICATE_MANIFEST_MIG026.json` (`certificate_container_commit`) — resolving the chicken-and-egg (a certificate cannot contain the hash of the commit that contains it). Precision/N/M are now strictly per-result; the Q₁₂ top-level no longer carries a global `precision_bits`.
3. **Validator check 8 weakness** fixed. It previously enforced positivity only when the ball *already* claimed `sign_certified: positive`. It now (a) reconstructs the exact dyadic ball from serialized `mid_dyadic`/`radius_dyadic` mantissa+exponent and **proves the decimal endpoints enclose mid ± radius**, and (b) requires **every** pivot under `ldl_status: PD` and every eigenvalue under an all-positive claim to have a strictly positive lower endpoint *and* `sign_certified: positive`. Adversarially confirmed: a PD-result pivot tampered to straddle zero (with its sign field flipped to `indeterminate`) is now **rejected** — the exact case that previously passed.

Also: the tracked `scripts/__pycache__/weil_engine.cpython-312.pyc` is removed and `.gitignore` excludes `__pycache__/`, `*.pyc`, and LaTeX aux files.

All certificates were regenerated by the final engine; every ball now serializes exact dyadics, and `make check` passes with the hardened checks 8–9. The Q₃–Q₁₂ positive-definiteness conclusion is unchanged; it is now independently machine-verifiable from the serialized certificates alone.
