...
## [PROSPECTIVE-PROGRAM] Project RIG-Q — Canonical Forcing of Representation and Induced Metric *(intake L+49)*

**Source:** synthesis note "NFC – Lawful Representation" (443 pp). **Assessment:** NFC_RIGQ_INTEGRATION_ASSESSMENT.md (L+49).

**What it is.** A bridge-hardening proof program (not a branch; CERT-PROJ at most) aiming to discharge the (SI) hypothesis of Book VI `thm:lorentzian-signature [C]` by canonically forcing the representation class and induced quadratic form from the source encoding datum, then welding Book VI's Q to GR's g_{μν} and deriving Lorentzian signature.

**Status: NOT promotable** (three independent grounds): load-bearing results RQ.5–RQ.7, RQ.9, RQ.11, RQ.12, RQ.14, RQ.15 are all **[O]** theorem shells; the [C] corollaries (RQ.8, RQ.13, RQ.16) are conditional on those open shells; the document's later "provisionally hardened/closed" status is non-canonical and partly governance-rule-forced (would be status inflation if imported). Chain is gated on the open hinge **O-RQ.1** (source pre-inner-product extraction; may fail per RQ.F1/F2) and on open GR obligations **O-GR.real**, **O-GR.compat**.

**Endpoint target (RQ.E):** representation class unique up to canonical unitary equivalence; induced form realization-independent; induced form = GR interface metric; Lorentz signature derived for the canonically induced form.

**Closure obligations (prospective-program, held — NOT corpus [O] tags):** O-RQ.1 source pre-inner-product extraction (load-bearing hinge) → O-RQ.2 universal realization → O-RQ.3 canonical unitary uniqueness → O-RQ.4 induced-form invariance → O-RQ.5 nondegeneracy from source rigidity → O-RQ.6 metric identification bridge (Q_can = g) → O-RQ.7 cone-null compatibility derivation → O-RQ.8 derived Lorentz-signature upgrade. Failure frontiers RQ.F1–F4 declared.

**Legitimate role.** The most developed roadmap for the out-of-toolkit Metric/representation/Lorentz-signature frontier (L+42/L+46). It correctly diagnoses the open (SI) hypothesis and the Book VI Q vs GR g parallel-geometry problem, both already tracked in the corpus. Contribution is the roadmap, not any discharge.

**Status-translation caveat.** Any future mining must translate RIG-Q's internal "provisionally closed/hardened" statuses to canonical tags first; under that translation the main chain reads [O]. Importing internal statuses verbatim = status inflation. Promote nothing without proving O-RQ.1 and discharging the GR gating obligations.

**O-RQ.1 proof effort (L+50; NFC_RIGQ_ORQ1_RESULT.md).** The load-bearing hinge was opened. Result: **sharpened, not discharged.** The positivity hinge (Lemma RQ.1d) — the note's predicted first failure into a Krein/indefinite structure — is instead resolved affirmatively, because the corpus already carries a canonical positive-semidefinite source-descended form: the L+34 energy/persistence form K(x,y)=⟨x,H_B y⟩ on the L+30 nonnegative-generator sector (H_B ≥ 0). Lemmas RQ.1a–c,e,f hold; the null space (RQ.1e) is **exactly the L+30 superselection kernel** ker H_B (no new notion of collapse, as the note hoped). Residual reduced to two named items: **(i)** the Hilbert-completion step = the **already-existing L+34 closable-realization hypothesis** (thm:vrp-hamiltonian bracket; not new); **(ii)** a **new sharp obligation O-RQ.1-rich (representation-richness/faithfulness)** — whether the energy form separates the full transport (L) and symmetry (Π) data, not only energy/metric. The missing rigidity is thus **faithfulness, not positivity.** RIG-Q remains non-promotable; this is a holding-layer result; no corpus [O] created (O-RQ.1-rich is a prospective-program obligation). Next hinge: O-RQ.1-rich.

**O-RQ.1-rich proof effort (L+51).** Faithfulness hinge opened. Result: **sharpened, not discharged.** The bare energy form is proved **non-faithful for transport** — its null space ker H_B is Π-invariant but not T_∂-invariant (transport moves zero-energy states off the kernel), so it blurs transport distinctions and fails RQ.5 universality for the (Γ, L, Π) package. Structural fix: a **joint energy-plus-transport-cost form** whose null space is the doubly-trivial degeneracy; the transport cost has a corpus candidate (the L+28/29 entropy/defect ledger), faithful for energy, transport, and symmetry. Residual (three named): (a) closability of the joint form (extended L+34 hypothesis); (b) **canonical energy/defect normalization** — promising lead that the L+35 temperature slope β_B = ∂S_B/∂E_B fixes it, to be proved not assumed; (c) joint-kernel/encoding-predicate alignment. Missing rigidity now = a canonical transport cost + its energy-relative normalization. Notable: the O-RQ.1 line resolves **into the closed VRP structure** (L+30 kernel, L+34 energy form, L+28/29 defect ledger, L+35 temperature), not external machinery. Next hinge: residual (b), the β_B normalization.

**Residual (b) — β_B normalization (L+52). RESOLVED (conditionally).** Transport-covariance argument: under a lawful interface the joint cost 𝔥_B + λ·𝔡_B is covariant (↦ g_I·itself) **only** if the weight λ scales as g_I; a constant λ fails. The temperature T_B = 1/β_B scales exactly as g_I (L+35 cl.3, from β_B ↦ g_I⁻¹β_B) AND is by definition the canonical exchange rate between the energy and entropy ledgers (L+35), using entropy absoluteness (L+29) to break the symmetry. So **λ = T_B is forced**: the canonical pre-inner-product is **C_can = 𝔥_B + T_B·𝔡_B** (positive on the positive-T sector, faithful, canonically normalized, covariant). No free normalization remains. Conditions: exchange-coherence + positive-T sector (L+35), entropy absoluteness (L+29), quadratic realization of 𝔡_B. **O-RQ.1 now reduces to the extended L+34 closability hypothesis + kernel alignment + sector conditions** — i.e. to the same realization hypothesis the whole VRP arc carries; on YM/GR/NS (where L+34 holds for the energy part) O-RQ.1 is plausibly dischargeable at CERT-PROJ pending joint-closability confirmation. The representation/metric hinge resolves entirely into the closed VRP structure: the forcing inner product is **energy ⊕ (temperature × distinguishability)**. Still non-promotable (reduced-not-closed; downstream O-RQ.2–8 and GR gating untouched). Next: (i) joint closability on YM, or (ii) O-RQ.2 universal realization.

**Step (i) — joint closability on YM (L+53). CONFIRMED at CERT-PROJ.** The YM energy form is certified closable (thm:B1-closable); the distinguishability form 𝔡_B is **bounded** (the collar alphabet is finite, K_0=7, so the transport operator T_∂ is finite-dimensional). By the KLMN form-perturbation theorem (bounded perturbation, relative form-bound 0), **C_can = 𝔥_B + T_B·𝔡_B is closable on YM**, completing to a canonical carrier H_can^YM. This **discharges the existence/construction content of O-RQ.1 (Theorem RQ.5) at CERT-PROJ force on YM** — the first concrete model realizing the RIG-Q representation target, with the canonical inner product explicitly C_can = energy ⊕ (temperature × distinguishability), built entirely from recovered-variable machinery. Conditions: thm:B1-closable's Y6 framework; positive-T exchange-coherent sector (L+35); identification of 𝔡_B with the bounded transport form. Remaining even on YM: kernel-alignment (c) and **universality (O-RQ.2, untouched)**; downstream O-RQ.3–8 and GR gating (O-GR.real/compat) open. Still non-promotable. Next: O-RQ.2 (universal realization).

**O-RQ.2 — universality on YM (L+54). REDUCED, not fully discharged.** The factoring map φ_R: H_can^YM → H, [x] ↦ Γ(x), is isometric iff the realization's inner product agrees with C_can on source classes (★). Lawfulness forces (★) on the C_can-visible (energy/transport/symmetry) data; the gap is extra lawful structure. Universality reduces to **minimality/cyclicity** (enforced by RIG-Q's anti-smuggling Standing Rules) + **comparison-rigidity** (C_can is the complete source-comparison datum — the open content). **Within a fixed topological sector**, universality holds at CERT-PROJ modulo local comparison-rigidity. **Cross-sector obstruction identified:** C_can is built from local/spectral data and does not separate YM's topological (theta/instanton) sectors (confirmed present in branch); H_can^YM is per-background, so universality is naturally within-sector. Named refinement **O-RQ.2-topological**: either the source datum fixes the topological sector (within-sector universality is the full statement) or C_can needs topological enrichment for cross-sector. Still non-promotable; downstream O-RQ.3–8 + GR gating untouched. Next: (i) local comparison-rigidity, or (ii) O-RQ.2-topological.

**O-RQ.2-topological + consolidation (L+55).** Resolved O-RQ.2-topological structurally: the source encoding datum is local (Book II, no global gluing), so the topological-sector data C_can omits is interface-nerve gluing cohomology (Čech H¹(cover; G)) — the same kind of object as the **L+36 synchronization obstruction** (β₁-supported H¹), already in the corpus. H_can^YM is per-gluing-class; **within-sector universality is the natural full statement** (force the representation given the sector), cross-sector data = existing L+36-type cohomology. **Effort consolidated.** Net achieved (CERT-PROJ, holding, on YM): O-RQ.1 construction discharged (C_can = energy ⊕ temperature × distinguishability); O-RQ.2 within-sector universality reduced to one clean residual — **local comparison-rigidity** (energy+transport+symmetry+distinguishability = complete local comparison datum?). Structural finding: the hinge resolves entirely into established NFC structure (VRP variables + L+30 kernel + finite alphabet + L+36 cohomology). **External gate unchanged:** the endpoint (metric weld O-RQ.6 → O-RQ.7–8) re-enters out-of-toolkit GR (O-GR.real/compat); within-YM work cannot close the program. **Project-Lead recommendation: HOLD RIG-Q here, consolidated** — diminishing within-toolkit returns, decisive gate is external, valuable result secured. Next substantive move is the GR nonlinear-PDE scope decision (program owner's call), not more RIG-Q drilling.

**L+61 — RIG-Q line resumed (GR gate lifted in the CK corner at L+60).** Local comparison-rigidity **settled**: the observable family is the certified triple (Spec, R_probe, T_Spec) with content encoding-fixed; in any lawful realization the generator/transport/symmetry are the same licensed operators and C_can is the forced inner product, so two lawful realizations agree on both inner product and operators — no room for an extra lawful invariant. Hence **O-RQ.2 within-sector universality discharges at CERT-PROJ on YM**, and **O-RQ.3 (uniqueness), O-RQ.4 (induced-form invariance), O-RQ.5 (nondegeneracy) cascade**. The **representation-forcing half O-RQ.1–5 is now [C]/CERT-PROJ on YM within a fixed topological sector**. Chain now stands at the **metric weld O-RQ.6**, whose GR target g_μν was realized at L+60 (prop:gr-ck-corner-extension) — so O-RQ.6 welds Q_can (YM induced form) to a realized GR-corner metric; remaining content is the cross-branch identification Q_can ↔ g_μν, then O-RQ.7 cone-null, O-RQ.8 signature. Still non-promotable (CERT-PROJ, one model, within-sector, conditional; O-RQ.6–8 open). Next: O-RQ.6 cross-branch weld.

**L+62 — O-RQ.6 metric weld structured; reduced to cone-null = (SI).** Direct identification Q_can = g_μν rejected (signature mismatch: Q_can positive-definite vs g_μν Lorentzian; domain mismatch: infinite-dim representation vs 4D tangent space). The weld is **structured**: the 4D metric content is carried by R_resp (dim = 9 = traceless-symmetric 4D metric dof), Q_can supplies the positive-definite spatial block, the VRP time rung (thm:vrp-time) supplies the temporal direction, and the Lorentzian (−+++) signature emerges from the spatial(+++)/temporal(−) split. The remaining content — that the split assembles consistently (null cone = causal cone) — is **cone-null compatibility, verbatim the (SI) hypothesis of Book VI thm:lorentzian-signature**. So **O-RQ.6 reduces to O-RQ.7 = (SI)**, closing the loop with RIG-Q's founding purpose (discharge (SI) by forcing Q canonically). Nondegeneracy of Q is in hand (O-RQ.5); the cone-null half is the last genuine condition; O-RQ.8 (derived signature) then follows from thm:lorentzian-signature. The whole weld resolved into established NFC structure (R_resp 4D dof, VRP time, Q_can from VRP variables, Book VI signature theorem) — no new external machinery beyond CK. Still non-promotable (holding-layer; cone-null open; frame map structured not fully proven; GR side the scoped corner). **The chain now stands one hinge (O-RQ.7 cone-null = (SI)) from its Lorentz-signature endpoint.** Next: O-RQ.7 cone-null compatibility.

**L+63 — close ATTEMPTED; not achieved; reduces to emergent Lorentz invariance.** Cone-null compatibility splits three ways: (i) cone structure — follows from collar locality (finite collar alphabet K_0=7 → finite max propagation speed → causal cone); (ii) calibration (∂C = null cone of g) — follows by fixing the light-speed constant to the collar propagation speed; (iii) **isotropy (round cone = exact Lorentz invariance) — IRREDUCIBLE.** The minimal symmetry carrier (MSC) is a discrete structure (space/point group), not full SO(3); a discrete substrate generically gives an anisotropic cone; and Book VI's isotropy is exactly the assumed (SI) hypothesis. So cone-null/(SI) reduces to **isotropy restoration in the Book VI continuum bridge** — the emergent-Lorentz-invariance-from-a-discrete-substrate problem: plausible (if anisotropic operators are continuum-irrelevant) but not automatic and not derivable from the discrete structure (which lacks SO(3)). **The close is not achieved.** Terminal state: O-RQ.1–6 at CERT-PROJ on YM within-sector; O-RQ.7 reduced to isotropy; O-RQ.8 contingent on it. The entire 443-page RIG-Q program reduces, end-to-end, to one sharply-located deep question (emergent spatial isotropy / Lorentz invariance), the one principle a discrete substrate cannot supply for free. **RIG-Q line at its genuine terminus** — further within-NFC work cannot close it. Non-promotable; holding-layer; no corpus [O].

---

## [S-PATTERN] ε-Convergence Cross-Branch Schema — observed, real, but subsumed (intake L+57)

**Source:** short synthesis note (ε-Convergence Cross-Branch Transfer Theorem proposal). **Disposition: NOT promotable as a new theorem; recorded as an observed expository pattern.**

**What it is.** An accurate observation that several branches license Book VI continuum/Fourier notation via the same proof shape: certified finite observables + finite-state/collar/depth-sum control + a positive gap or contraction bound ⟹ ε_n → 0 ⟹ continuum notation licensed. Three modes: boundary-ratio (Van Hove), depth-tail/geometric (positive gap), window-contraction (Banach). Verified instances are real: CRYST **DOM-ε** (Domain No-Loss Convergence, Van Hove boundary-to-volume decay), CRYST **EWALD-ε** (depth-sum tail under the positive Bragg gap), NS **window-to-window contraction** (λ_NS < 1).

**Why not promotable (assessed against Book VII discipline):**
1. **Subsumed by `thm:continuum-bridge-schema` [U].** That theorem already states the general licensing law unconditionally: continuum force requires (1) a scoped discrete certificate + (2) a separately proved continuum bridge carrying it. The proposed meta-theorem is exactly this principle; its three "modes" are standard techniques for proving ingredient (2). The note's own most-general form (∑δ_d → 0 ⟹ licensed) collapses to "the loss converges" — precisely the schema's requirement. **No new deductive content.**
2. **The instances are branch data by design.** Each ε-bound is proved from branch-specific structure (CRYST Van Hove/Bragg gap; NS contraction constant). A shared *shape* across branches is not a transfer; per-branch facts do not promote to a spine theorem on resemblance.
3. **Promotion requires transfer (`thm:promotion-requires-transfer`), absent here.** There is no transfer theorem carrying a single certified result across branches — only a reused proof technique. A reusable technique is not a canonical object.

**Legitimate value (expository only).** The pattern is a genuine and elegant piece of cross-branch synthesis: it names how the [U] bridge schema is instantiated uniformly via three standard convergence modes. If signposting is wanted, a one-line expository cross-reference in Book VI (listing the branch instances of the schema) would be proportionate. A standalone meta-theorem is **not** warranted — adding one would duplicate `thm:continuum-bridge-schema` and risk the redundancy/inflation the canon guards against.

**No canon change; no [O] change.** Recorded here as an observed-but-subsumed pattern, not a promotion candidate.

---

## [EXTERNAL-PROGRAM] CK–GR External Stability Bridge — opened, scoped (intake L+58)

**Architectural decision (program owner):** open the GR external-machinery program as a **scoped external bridge**, not a universal closure theorem. Recorded in canon as `rmk:ck-gr-stability-bridge` [B] in the GR branch.

**Bridge.** Christodoulou–Klainerman (1993): global nonlinear stability of Minkowski spacetime for the Einstein vacuum equations under small asymptotically flat initial data (geodesic completeness, asymptotic-Minkowski decay). Accepted as a candidate external nonlinear-PDE bridge for the GR global-closure frontier (def:curv-subcrit-global).

**Status: [B] import; [C] conditional application.** External theorem (not NFC-proved), imported under Book VII import discipline. Does NOT self-promote to [U] or to global GR closure.

**Scope:** only GR continuum regimes whose endpoint data match CK's small-data asymptotically flat Einstein-vacuum hypotheses.

**Conditional discharge — pending, two NFC-side verifications:** (1) NFC supplies the Book VI continuum interface for GR (def:continuum-interface-regime); (2) the GR endpoint data are verified to satisfy CK's initial-data hypotheses. Until both hold, the bridge is opened/candidate and does NOT discharge def:curv-subcrit-global.

**What CK does NOT supply:** arbitrary/large-data global stability, black-hole exterior stability, cosmological stability, matter-coupled SM/GR closure, the RIG-Q metric weld, Lorentz-signature emergence from NFC source data. All remain out of scope and open.

**Relation to ε-pattern:** CK is the decay/contraction-mode (ε(t) → 0 via weighted energy estimates, decay hierarchies, null geometry, bootstrap) external analytic input, operating after Book VI licenses continuum GR notation.

**Next work to make the conditional discharge real:** verify the two NFC-side conditions (Book VI GR continuum interface + endpoint-data/CK-hypothesis match). Only then does the CK-compatible-regime global-stability residual discharge at [C]. This also bears on RIG-Q's metric-weld environment (O-RQ.6), which re-enters the GR obligations.

**Verification (L+59).** (1) Continuum interface: present (GR carries a Book VI continuum interface + causal-geometric realization functor). (2) Regime relation: CK's small-data AF vacuum hypotheses define a **proper sub-regime** of GR curvature-subcriticality (small-data ⟹ small curvature ⟹ B^grav < C^grav; converse fails; matter sectors are outside CK's vacuum scope). **Result: a [C] conditional reduction, not a discharge.** On the small-data AF vacuum sub-regime, CK supplies the global extension (stability, geodesic completeness, asymptotic-Minkowski decay) that def:curv-subcrit-global / O-GR.extension requires — but only conditional on the GR structural EFE there realizing genuine Einstein-vacuum Cauchy data, i.e. on **O-GR.real / O-GR.compat** holding in that sub-regime. So CK **removes the external global-PDE-stability burden** and reduces the global-extension residual (on the CK-compatible corner) to the GR structural realization obligations. The full frontier (large-data, matter-coupled, non-AF) remains open; the reduction is still gated on the open O-GR.real/compat even within the CK corner. Next: discharge O-GR.real/compat in the small-data AF vacuum corner (the remaining structural step), which would complete the CK-compatible-regime global extension at [C].

**L+60 — CK-corner global extension DISCHARGED [C] (scoped).** O-GR.real/compat discharged in the small-data AF vacuum corner via `thm:gr-efe-limit` [C], which establishes the genuine EFE tensor equation G_μν + Λg_μν = κT_μν at the K_0=7 continuum limit. In the corner (T=0, Λ=0 forced by asymptotic flatness) this is genuinely G_μν = 0, the Einstein vacuum equation; the realization packet (prop:GR-CP1) realizes g_μν. So the NFC-realized metric is genuine small AF Einstein-vacuum Cauchy data, and CK supplies the global extension (global nonlinear stability, geodesic completeness, asymptotic-Minkowski decay). Recorded as **prop:gr-ck-corner-extension [C]**: the global-extension residual (def:curv-subcrit-global / O-GR.extension) is discharged at [C] on the small-data AF vacuum sub-regime — the **first scoped closure of any part of the GR global frontier**. Conditions: GR standing conditions (K_0=7, Phase-A, KPO-3, NFC-5/6/7), realization packet, corner restriction (small/AF/Λ=0/vacuum/T=0), realized data in CK's regularity/decay class, CK import. **Full frontier (large-data, matter-coupled, non-AF, Λ≠0) remains open.** The external global-PDE burden is carried by CK; the NFC-side content (realization + vacuum EFE match) is established in the corner.

---

## [S-CANDIDATE] Governance Pattern Audit Schema (GPAS) — scoped audit instrument (intake L+65)

**What it is.** A proposed *internal audit instrument* that classifies the recurring source → endpoint → bridge → frontier closure shapes that lawful branches already instantiate when they obey the existing governance (Book I anti-smuggling; Book V Universal Branched Legitimacy; Book VI Effective Continuum Legitimacy / ε-convergence; Book VII scoped-certificate / import discipline). It is descriptive meta-methodology about the *form* of legitimate descent — a way to run the per-branch legitimacy audit uniformly — not a new primitive layer.

**Status: S-CANDIDATE — not promoted, not canon, not a Book VII addition.** Consistent with the L+44 NFR assessment and the L+64 three-note assessment: as *deductive* content it is subsumed by Book VII discipline plus the already-tracked per-branch frontiers; it adds no new mathematics about the objects and surfaces no obligation the [O] census was missing. It pays no new structural cost and is therefore held here, outside the spine, rather than promoted.

**Governing constraint (load-bearing — the reason it is safe).** GPAS may be used **only** as an organizing checklist for running the per-branch audit — i.e. to tell an auditor *what to go check* in a branch. It may **never** be used as a premise in a branch proof, may **never** discharge an obligation, and may **never** be cited across branches without an explicit, separately-proved transfer/bridge theorem that preserves scope, status, and named frontiers (Book VII). It generates no physics and relaxes no gate. A pattern named in GPAS can direct attention; it can never carry deductive weight. This single rule is what keeps naming the patterns from becoming cross-branch analogy-without-transfer — the precise failure mode Book VII forbids.

**Corrections recorded (verify-before-cite).** The originating proposal used two conventions that do not match the corpus: (i) "[R] research layer" — in the canon [R] = *Remark* (expository, non-load-bearing); there is no research-layer status, and a proposed research classification is therefore a holding-register item, not a canon status; (ii) "Woodin-Witness firewall" — not a corpus term; the actual mechanism is the SCC TSI Firewall (sr:scc-no-consciousness). NFR itself is real (assessed L+44).

**Why S-CANDIDATE and not canon.** Canonizing the residue-audit / resolution vocabulary as new [U] governance theorems would inflate procedure into mathematics (the shells restate thm:governing, thm:promotion-requires-transfer, anti-smuggling, import discipline). The holding register is the precedent-consistent home (cf. NFR, the ε-pattern, RIG-Q, the CK bridge before verification). Maturation path, if ever pursued: a single scoped "Governance Pattern Audit Schema" certificate in a governance document (not the spine), each use accompanied by an explicit per-branch status ledger and named bridge conditions — an architectural decision reserved for the program owner. Until then it remains a held audit instrument, not invoked in any proof.

**Net effect on canon: none.** No .tex modified; no obligation status changed; nothing promoted; [O] unchanged at 5.
