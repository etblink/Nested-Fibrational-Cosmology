...
O-SCC.WeakGlue & SCC Branch-Local Pairwise Amalgamation:
  ...
  distinct from source-side O-WeakGluing) & [O] &
...
\textup{(O-SCC.WeakGlue: Pairwise Amalgamation.)}
For any two SCC carrier data $c_1, c_2 \in \mathcal{W}_X$ with
overlapping support, the pairwise amalgamation $c_1 \wedge c_2$
exists and is unique in the carrier order.  This is the
branch-specific analogue of the source-side pairwise amalgamation
(Book~IV, Thm.~IV.ORA); the SCC-branch version requires the
carrier data to be amalgamated, not just the source observables.

\emph{Status:} Proof skeleton exists in retired SCC Branch Paper
(\texttt{lem:weakglue}).  The key difficulty is that SCC carrier
data may have non-trivial overlap structure that requires a
separate amalgamation argument from the source-side version.
\end{openobligation}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
  ...
  \item $\mathcal{M}_X$ is unique up to canonical carrier
        equivalence.
\end{enumerate}
This blocks silent carrier inflation: no larger carrier than
$\mathcal{M}_X$ is needed to certify the SCC structural property.

\emph{Status:} Main open burden of the SCC branch.  Structural
form established; existence and uniqueness proof is the open
question.  Until MCS is discharged, the branch cannot advance
beyond CERT-PROJ.
\end{openobligation}
