...
%% Current status: Conditional CERT-CLOSE at the declared structural endpoint (UCTI/depth-sum/threshold-stability + source-descent). See prop:scc-status / cap:SCC-status.
%% Constitutional intake (as constituted): CERT-PROJ at most; MCS is the main frontier. [retained as intake; superseded — MIG-005]
%%
%% Status legend:
%%   [U]  Unconditional
%%   [C]  Conditional
%%   [B]  Bridge        — carries force across a scoped domain via a separately proved Transfer Theorem
...
%% DEPRECATED (v1.1, MIG-019): `openobligation` is retained below as a
%% compatibility alias only; new obligation declarations use `obligation`.
...
%%   MCS              Minimal Structural Carrier Selection
%%                    (constitution-time main burden; conditionally
%%                    discharged at current scope; residual to
%%                    unconditional force remains)
...
% ============================================================
\begin{remark}[\status{R}]\label{cap:SCC-status}
\textbf{Current Status Capsule.}
\emph{Current posture:} Conditional CERT-CLOSE at the declared structural endpoint, conditional on the stated UCTI / depth-sum / threshold-stability and source-descent conditions. No consciousness, phenomenology, qualia, or stronger ontological interpretation is licensed.

\emph{Authoritative source:} \ref{prop:scc-status}~(final status proposition (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 0; reduced irreducible frontier --- none; branch posture --- conditional CERT-CLOSE at the structural endpoint.}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 \item Four named obligations, each now conditionally discharged
 (see the Closure Ledger, \S\ref{sec:ledger}, and
 Thm.~\ref{thm:scc-frontier} for the residuals): O-SCC.MCS
 (primary), O-SCC.WeakGlue (prerequisite to MCS), O-SCC.UC,
 O-SCC.TSI.
\end{enumerate}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 the principal named burden, conditionally discharged
 (Thm.~\ref{thm:scc-mcs-full}); residual per
 Thm.~\ref{thm:scc-frontier}(i).
...
\textbf{Conditional CERT-CLOSE at the declared structural endpoint}
(conditional on the stated UCTI / depth-sum / threshold-stability and
source-descent conditions). See the Current Status Capsule
(Rem.~\ref{cap:SCC-status}) and the Final Status Proposition
(Prop.~\ref{prop:scc-status}). No consciousness, phenomenology, qualia,
or stronger ontological interpretation is licensed.

\emph{Constitutional intake (as constituted; superseded --- migration
\texttt{MIG-008}):} at constitution the branch was recorded as
``CERT-PROJ at most; main frontier O-SCC.MCS; all four core obligations
(MCS, WeakGlue, UC, TSI) open in canonical papers.'' O-SCC.MCS,
O-SCC.UC, and structural O-SCC.TSI have since been conditionally
discharged in the branch body (Thms.~\ref{thm:scc-mcs-full},
\ref{thm:scc-uc}, \ref{thm:scc-tsi}); current status is governed by
Prop.~\ref{prop:scc-status}.
...
status alphabet applies. Current status: conditional CERT-CLOSE at the
declared structural endpoint (see Prop.~\ref{prop:scc-status}). Claims
in this book do not exceed that conditional structural endpoint, and in
particular license no consciousness or phenomenology.
\end{remark}
...
(2)~Intake survival: at constitution, the ORA--CTM--TIN rungs were
proposed or partial, and MCS was the main open burden. The
subsequent conditional-discharge chain is recorded in
Section~\ref{sec:ledger} and Proposition~\ref{prop:scc-status}.
The branch survives through TIN at its declared scope.
(3)~Ledger completeness: O-SCC.MCS through O-SCC.TSI are
explicitly ledgered in Section~\ref{sec:ledger}.
(4)~Status honesty: no claim exceeds conditional CERT-CLOSE at the
declared structural endpoint (Prop.~\ref{prop:scc-status}).
...
Every link is now conditionally discharged in this book:
O-SCC.WeakGlue (Theorems~\ref{thm:scc-w1}--\ref{thm:scc-weakglue},
conditional on the frozen Support Sufficiency Schema and the SCC
object definitions, both established here);
O-SCC.MCS-Uniqueness (Theorem~\ref{thm:scc-mcs-uniqueness});
O-SCC.MCS in full, including existence
(Theorem~\ref{thm:scc-mcs-full}, conditional on the stated UCTI /
depth-sum / threshold-stability conditions --- the existence residual
SCC-MinExist~(\ref{ob:scc-mcs}) is carried inside those conditions);
O-SCC.UC (Theorem~\ref{thm:scc-uc}); and structural O-SCC.TSI
(Theorem~\ref{thm:scc-tsi}). The chain therefore reaches
\emph{conditional} CERT-CLOSE (Prop.~\ref{prop:scc-status}); the
residual to \emph{unconditional} closure is exactly the named
conditions, per the Failure Frontier
...
\begin{obligation}[\status{C}]\label{ob:scc-weakglue}
\textup{(O-SCC.WeakGlue: DISCHARGED.)}
See Theorem~\ref{thm:scc-weakglue}. This obligation is now [C].
\end{obligation}

\begin{obligation}[\status{C}]\label{ob:scc-mcs}
...
\begin{obligation}[\status{C}]\label{ob:scc-uc}
...
\begin{obligation}[\status{C}]\label{ob:scc-tsi}
...
O-SCC.MCS. With the conditional discharges of this book, the SCC
branch has reached conditional CERT-CLOSE; the listed verifications
remain necessary for \emph{unconditional} closure.
...
\begin{obligation}[\status{C}]\label{ob:rh-d1-full}
...
named failure frontier. Each item below is conditionally discharged
in this book; the frontier's unconditional content is the
\emph{residual}: what remains between each conditional discharge and
unconditional force.
\begin{enumerate}[label=\textup{(\roman*)}]
 \item \textbf{Primary frontier (O-SCC.MCS):} the Minimal
 Structural Carrier Theorem. Conditionally discharged
 (Thm.~\ref{thm:scc-mcs-full}) at the declared structural scope;
 the residual is UCTI / depth-sum / threshold-stability
 certification \emph{without} the stated conditions. Until that
 residual is closed, the SCC branch cannot advance beyond
 conditional CERT-CLOSE to unconditional closure. Silent carrier
 inflation remains a structural risk.
 \item \textbf{Prerequisite frontier (O-SCC.WeakGlue):}
 the SCC branch-local pairwise amalgamation lemma. Conditionally
 discharged (Thm.~\ref{thm:scc-weakglue}); the residual is removal
 of its named conditions.
 \item \textbf{Completion frontier (O-SCC.UC):} branchwise
 SCC universal completion. Conditionally discharged
 (Thm.~\ref{thm:scc-uc}); the residual is unconditional
 source-descent/universal-completion force.
 \item \textbf{Identification frontier (O-SCC.TSI):} endpoint TSI
 certification. Conditionally discharged at structural-label scope
 (Thm.~\ref{thm:scc-tsi}); the residual is unconditional endpoint
 identification beyond structural-label form.
\end{enumerate}
\end{theorem}

\begin{proof}
The Frontier Stability Theorem (Book~VII, Thm.~VII.4.3) requires
a positive named failure frontier. The four labels above are the
SCC branch's frontier and remain the stable governance ledger for
the residual-to-unconditional program: each names a precise
mathematical statement, records its conditional discharge in this
book, and identifies exactly what separates that discharge from
unconditional force. The frontier is not dissolved by the
conditional discharges; it is sharpened to the stated residuals.
\qed
\end{proof}
...
\textup{[Conditional on MCS (Thm.~\ref{thm:scc-mcs-full}),
UC (Thm.~\ref{thm:scc-uc}), and TSI (Thm.~\ref{thm:scc-tsi}),
each conditionally discharged.]}
...
\emph{Status:} Conditionally discharged at the same structural scope as
MCS/UC/TSI (migration \texttt{MIG-009}). This R1 obligation originated
in the retired Branch~A* governance layer, where it was recorded as
\emph{Reserved}; it is now carried at conditional force by the proof
below, and its scope is exactly the conjunction of the MCS/UC/TSI
conditions. It may be cited only at that conditional structural scope,
never as an unconditional projection.
\end{theorem}
\begin{proof}
...
\begin{proposition}[\status{C}]\label{prop:scc-status}
\textup{(Canonical Status --- Updated.)} The SCC branch currently satisfies:
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 \item \textbf{MCS/UC/TSI:} conditionally discharged at the declared
 structural scope (Thms.~\ref{thm:scc-mcs-full}, \ref{thm:scc-uc},
 \ref{thm:scc-tsi}); TSI holds in structural-label form. The residual
 to \emph{unconditional} closure is the named UCTI / depth-sum /
 threshold-stability and source-descent conditions.
 ...
 \item \textbf{Status:} \emph{Conditional CERT-CLOSE at the declared
 structural endpoint}, conditional on the stated UCTI / depth-sum /
 threshold-stability and source-descent conditions. Not unconditional
 CERT-CLOSE.
\end{enumerate}
\end{proposition}
...
\begin{enumerate}
 \item The SCC branch is a lawful NFC branch at conditional CERT-CLOSE
 on the declared structural endpoint (Prop.~\ref{prop:scc-status}).
 ...
 \item The four precisely named obligations
 (Thm.~\ref{thm:scc-frontier}); MCS, UC, and structural TSI are
 conditionally discharged (Thms.~\ref{thm:scc-mcs-full},
 \ref{thm:scc-uc}, \ref{thm:scc-tsi}).
 \item The SCC-ICDC conditional theorem
 (Thm.~\ref{thm:scc-icdc}).
 \item The R1 SCC Branch-Projection theorem, conditionally discharged
 at the MCS/UC/TSI structural scope (Thm.~\ref{thm:scc-r1}).
\end{enumerate}

\textbf{What this branch book does not establish.}
\begin{enumerate}
 \item \emph{Unconditional} discharge of the core obligations: MCS, UC,
 and structural TSI are discharged only \emph{conditionally}, on the
 stated UCTI / depth-sum / threshold-stability and source-descent
 conditions; WeakGlue is a prerequisite carried at the same conditional
 scope.
 \item \emph{Unconditional} CERT-CLOSE status (the branch reaches
 conditional CERT-CLOSE only).
...
\ref{thm:scc-frontier}\ Failure Frontier
 ...
 & External scrutiny; residual-to-unconditional-closure audit \\

\ref{thm:scc-icdc}\ SCC-ICDC
 & [C] & O-SCC.MCS + WeakGlue; Bk.~III Sch.~III.8.1
 & O-SCC.UC; scale-limit SCC \\

\ref{thm:scc-r1}\ R1 (cond.\ discharged)
 & [C] & O-SCC.MCS + UC + TSI (all cond.\ discharged)
 & Cited only at MCS/UC/TSI structural scope \\

\ref{prop:scc-status}\ Canonical Status
 & [C] & All of SCC Branch Book; UCTI/depth-sum/threshold-stability
 & Program ledger \\
\hline
\end{tabular}
\renewcommand{\arraystretch}{1}
