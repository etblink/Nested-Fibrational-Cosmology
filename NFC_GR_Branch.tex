...
% ============================================================
\begin{remark}[\status{R}]\label{cap:GR-status}
\textbf{Current Status Capsule.}
\emph{Current posture:} Domain-bounded conditional CERT-CLOSE on the realized domain, with a scoped CK-corner global extension on the small-data asymptotically-flat vacuum sub-regime; global curvature-subcriticality remains the open frontier.

\emph{Authoritative source:} \ref{prop:gr-status}~(status proposition (current posture)); \ref{prop:gr-ck-corner-extension}~(CK-corner scoped extension).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 0 (the global frontier is tracked as \texttt{def:curv-subcrit-global}, a definition-marker, not an~[O] tag); reduced irreducible frontier --- 1 (GR global curvature-subcriticality); branch posture --- domain-bounded CERT-CLOSE + CK-corner extension.}
...
\textup{[Conditional on: Prop.~\ref{prop:gr-topo-nonload}
...
\begin{remark}[\status{B}]\label{rmk:ck-gr-stability-bridge}
\textbf{CK--GR External Stability Bridge (opened; scoped; import-disciplined).}
Under Book~VII import discipline, the Christodoulou--Klainerman theorem
(Christodoulou \& Klainerman, \emph{The Global Nonlinear Stability of the
Minkowski Space}, Princeton, 1993) --- global nonlinear stability of
Minkowski spacetime for the Einstein vacuum equations under small
asymptotically flat initial data, yielding geodesic completeness and
asymptotic-Minkowski decay --- is accepted as a \emph{candidate external
nonlinear-PDE bridge} for the GR global-closure frontier
(Def.~\ref{def:curv-subcrit-global}).

\emph{Status.} \status{B} as an import (an external theorem, not
NFC-proved); its \emph{application} to the GR global-stability residual
is \status{C}, conditional on the two NFC-side verifications below, and
it does \emph{not} self-promote to \status{U} or to global GR closure.

\emph{Scope.} CK applies only to GR continuum regimes whose endpoint data
match its small-data asymptotically flat Einstein-vacuum initial-data
hypotheses.

\emph{Conditional discharge (pending --- not yet claimed).} The bridge
may discharge the global-stability residual (global nonlinear stability /
geodesic completeness / asymptotic-Minkowski decay) in the CK-compatible
regime only \emph{after} (1)~NFC supplies the Book~VI continuum interface
for the GR branch (Def.~\ref{def:continuum-interface-regime}), and
(2)~the GR endpoint data are verified to satisfy CK's initial-data
hypotheses. Until both hold, the bridge is opened and accepted as a
candidate; it does \emph{not} discharge Def.~\ref{def:curv-subcrit-global}.

\emph{What CK does not supply.} Arbitrary or large-data global stability,
black-hole exterior stability, cosmological stability, matter-coupled
SM/GR closure, the RIG-Q metric weld, and Lorentz-signature emergence
from NFC source data all remain out of scope and open.

\emph{Relation to the $\varepsilon$-convergence pattern.} CK instantiates
the decay/contraction mode ($\varepsilon(t)\to 0$ via weighted energy
estimates, decay hierarchies, null geometry, and bootstrap closure): the
external analytic decay input that operates \emph{after} Book~VI licenses
continuum GR notation, not a finite-collar argument.

\emph{Verification (L+59): regime match and conditional reduction.} Two
checks. \textbf{(1) Continuum interface:} the GR branch carries a Book~VI
continuum interface and causal-geometric realization functor
(referencing Def.~\ref{def:continuum-interface-regime}); the interface is
present, so continuum GR notation is licensable in the declared regime.
\textbf{(2) Regime relation:} CK's hypotheses (small-data asymptotically
flat Einstein \emph{vacuum} data) define a \emph{proper sub-regime} of the
GR curvature-subcriticality regime (Def.~\ref{def:GR-curvature-subcritical}):
small-data near Minkowski forces small curvature, hence
$B_n^{\mathrm{grav}} < C_n^{\mathrm{grav}}$, so
$\{\text{small-data AF vacuum}\} \subseteq \{\text{curvature-subcritical}\}$;
the converse fails (subcriticality is a local defect-margin bound, not
global smallness), and the GR branch's matter sectors lie outside CK's
vacuum scope. \textbf{Consequence (honest, conditional).} On the
small-data AF vacuum sub-regime, CK supplies the \emph{global extension}
(global nonlinear stability, geodesic completeness, asymptotic-Minkowski
decay) that the frontier Def.~\ref{def:curv-subcrit-global} /
O-GR.extension otherwise requires --- but only \emph{conditional} on the
GR structural EFE there realizing genuine Einstein-vacuum Cauchy data,
i.e.\ on O-GR.real / O-GR.compat holding in that sub-regime. CK therefore
\emph{removes the external global-PDE-stability burden} and reduces the
global-extension residual, on the CK-compatible sub-regime, to the
GR-structural realization obligations O-GR.real / O-GR.compat restricted
to small-data AF vacuum data. This is a \status{C} conditional reduction,
\emph{not} a discharge of Def.~\ref{def:curv-subcrit-global}: the full
frontier (large-data, matter-coupled, and non-asymptotically-flat regimes)
remains open, and the reduction is gated on the still-open O-GR.real /
O-GR.compat even within the CK-compatible corner.
\end{remark}


\begin{proposition}[\status{C}]\label{prop:gr-ck-corner-extension}
\textup{(CK-Corner Global Extension: Scoped Discharge of the Global-Extension
Residual on the Small-Data AF Vacuum Sub-Regime.)}
\textup{[Conditional on: Thm.~\ref{thm:gr-efe-limit} (and its conditions
$K_0=7$, Phase-A, KPO-3, NFC-5/6/7); the realization packet
(Prop.~\ref{prop:GR-CP1}); restriction to the corner --- small-data,
asymptotically flat ($\Lambda=0$), vacuum ($T_{\mu\nu}=0$); the realized
Cauchy data lying in CK's regularity/decay class; and the CK import
(Rmk.~\ref{rmk:ck-gr-stability-bridge}).]}
On the small-data asymptotically flat vacuum sub-regime, O-GR.real and
O-GR.compat hold and the CK bridge supplies the global extension: the
NFC-realized metric is a genuine small asymptotically flat Einstein-vacuum
Cauchy datum, and its maximal development is globally nonlinearly stable,
geodesically complete, and asymptotically Minkowskian. Hence the
global-extension residual (Def.~\ref{def:curv-subcrit-global} /
O-GR.extension) is discharged at \status{C} on this sub-regime.
\emph{Scope:} the full frontier --- large-data, matter-coupled,
non-asymptotically-flat, and $\Lambda\neq0$ regimes --- remains open.
\end{proposition}

\begin{proof}
Thm.~\ref{thm:gr-efe-limit} gives the Einstein field equations in genuine
tensor form $G_{\mu\nu} + \Lambda g_{\mu\nu} = \kappa T_{\mu\nu}$ at the
$K_0=7$ continuum limit. Restricting to the corner ($T_{\mu\nu}=0$,
$\Lambda=0$ --- the latter forced by asymptotic flatness) yields
$G_{\mu\nu}=0$, the Einstein vacuum equation. The realization packet
(Prop.~\ref{prop:GR-CP1}) realizes the metric $g_{\mu\nu}$ from
response-algebra data (discharging O-GR.real in the corner), and the
vacuum EFE then holds for it (discharging O-GR.compat in the corner). By
the corner restriction the data are small and asymptotically flat, and by
hypothesis they lie in CK's regularity/decay class; they are therefore
admissible CK initial data. The CK bridge
(Rmk.~\ref{rmk:ck-gr-stability-bridge}) applied to such data supplies
global nonlinear stability, geodesic completeness, and
asymptotic-Minkowski decay --- the global extension
$\mathcal{D}\to M^4$. This discharges the global-extension residual on
the sub-regime, at \status{C} on the listed conditions. The external
global-PDE-stability content is carried entirely by CK; the NFC-side
content is the realization (O-GR.real) and the vacuum EFE structural
match (O-GR.compat), both established in the corner by the cited GR
machinery. \qed
\end{proof}
