...
\textup{(O-SM.Harmonization: POT Packaging Discharged.)}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
  ...
  \item showing the symmetry breaking
    $\mathrm{SU}(2) \times \mathrm{U}(1) \to \mathrm{U}(1)_{\mathrm{em}}$
...
for $\mathrm{SU}(3)$, $\mathrm{SU}(2)$, $\mathrm{U}(1)$
...
\begin{proposition}[\status{U}]\label{prop:SM-status}
...
(§\ref{sec:sm-setup}): the SM super-branch cannot be stronger than
