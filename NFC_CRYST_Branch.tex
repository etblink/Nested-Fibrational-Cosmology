...
%% CRYST Branch Book: Crystallography Branch (Constituted; see Current Status Capsule)
%% Current status: Conditional CERT-CLOSE (diffraction-periodicity-symmetry) pending two ε-convergence conditions + phase frontier. See prop:cryst-status / cap:CRYST-status.
%% Constitutional intake (as constituted): Architecturally constituted; pre-closure. [retained as intake; superseded — MIG-005]
%%
%% Status legend:
...
% ============================================================
\begin{remark}[\status{R}]\label{cap:CRYST-status}
\textbf{Current Status Capsule.}
\emph{Current posture:} Conditional CERT-CLOSE on the diffraction--periodicity--symmetry endpoint, pending two named $\varepsilon$-convergence conditions; the phase problem (modulo the certified invariance group) is the principal frontier.

\emph{Authoritative source:} \ref{prop:cryst-status}~(branch status proposition (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 2 (phase problem, toolkit-boundary); reduced irreducible frontier --- none; branch posture --- conditional CERT-CLOSE (diffraction--periodicity--symmetry).}
...
\begin{obligation}[\status{C}]\label{ob:cryst-PR}
...
\begin{obligation}[\status{C}]\label{ob:cryst-GAP}
...
\begin{obligation}[\status{C}]\label{ob:cryst-LAT}
...
\begin{obligation}[\status{C}]\label{ob:cryst-NC}
...
\begin{obligation}[\status{C}]\label{ob:cryst-SG}
...
\begin{obligation}[\status{C}]\label{ob:cryst-DOM}
...
\begin{obligation}[\status{C}]\label{ob:cryst-EWALD}
...
\begin{obligation}[\status{C}]\label{ob:cryst-END}
...
\begin{obligation}[\status{C}]\label{ob:cryst-dom-noloss}
...
\begin{obligation}[\status{C}]\label{ob:cryst-EWALD-reduced}
...
\begin{obligation}[\status{C}]\label{ob:cryst-ewald-eps}
...
\begin{obligation}[\status{O}]\label{ob:cryst-PHASE}
...
\begin{obligation}[\status{C}]\label{ob:cryst-anomalous-nomination}
...
\begin{obligation}[\status{C}]\label{ob:cryst-diffraction-standard}
...
\begin{obligation}[\status{O}]\label{ob:cryst-PHASE-progress}
