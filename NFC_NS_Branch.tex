...
conditional on \textsc{idc} and $\alpha$ --- see
Thm.~\ref{thm:NSL1}), whenever
...
form and is therefore conditional on \textsc{idc}
\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
\eqref{hyp:NS-alpha}; MIG-050. The ledger-side contraction
...
through the same bridge.) Given the hypothesis, the
...
reconditioning:} the NS.6.1 conditional closes at Phase-A, $K_0=7$,
LS-2 scope \emph{with the hypothesis set enlarged by the named open
hypotheses \textsc{idc}
\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
\eqref{hyp:NS-alpha}} (the satisfaction of the contraction
condition, and the ledger-to-imbalance transfer, both pass through
them). The active frontier advances to NS.7.1
(Thm.~\ref{thm:NS71}), which requires only the Book~VI $H^1_x$-norm
interface legitimacy and is conditionally proved on the same
enlarged hypothesis set. The NS branch therefore reaches
domain-bounded CERT-CLOSE conditionally
(Cor.~\ref{cor:NS-both-give-endpoint}), \emph{conditional
additionally on \textsc{idc} and $\alpha$}.
\end{remark}
...
(Cor.~\ref{cor:NS61-discharged}, whose hypothesis set includes
\textsc{idc} and $\alpha$ per MIG-050), Book~III source-descended
...
transport-weighted imbalance is part of $\alpha$,
Eq.~\eqref{hyp:NS-alpha}).
...
named open hypotheses \textsc{idc}
\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
\eqref{hyp:NS-alpha} per MIG-050.]}
...
\textup{[Conditional on \textsc{idc}
\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
\eqref{hyp:NS-alpha}, via Thm.~\ref{thm:NSL1} (MIG-050).]}
...
\medskip\noindent\textbf{Named open hypotheses (MIG-050; unproved
--- not claimed).} The NS-0A--NS-0D constants audit (scratch record;
see migrations/MIGRATIONS.md, MIG-050) established that the successor
budget previously asserted here is not proved by the cited
subordination lemmas, and that the honest repair route requires two
statements not currently available in canon. They are recorded as
explicit conditional hypotheses; every downstream use names them.
Neither is proved; neither may be cited as proved.
...
\begin{equation}\label{hyp:NS-alpha}
 I_n \;=\; \mathcal{B}_n
\end{equation}
on the NS active-window regime: the Book~III interface burden
(Definition interface-burden) coincides with the boundary component
of the Book~II ledger. This definitional alignment across the two
formalisms is required by the obstruction/defect bridge below and is
pending.
...
\textup{[Conditional on \textsc{idc}
\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
\eqref{hyp:NS-alpha}, in addition to the standing NS hypotheses.]}
Assume \textsc{idc} and $\alpha$. Set
$\widetilde\eta := \eta + \eta_{\mathrm{int}} < 1$ and
$\widetilde\kappa := 1 - (1-\widetilde\eta)(c_1+c_2) < 1$. Then
the ledger contracts on the late safe tail,
$\mathfrak{D}_{k+1} \leq \widetilde\kappa\,\mathfrak{D}_k$, and
there exist a symbolic constant
$a = T_{\mathrm{NS}}/\bigl(M(1+C_{\mathcal{E}})\bigr) > 0$ (with
$M$ the SCT.4 bounded-obstruction ceiling) and an integer
$m > \log(1/a)/\log(1/\widetilde\kappa)$ such that, whenever
$\mathcal{O}_{\mathrm{NS}}(W_k) \geq T_{\mathrm{NS}}$,
\[
 \mathcal{O}_{\mathrm{NS}}(W_{k+m})
 ...
 \mathcal{O}_{\mathrm{NS}}(W_k),
 \qquad
 \lambda_{\mathrm{NS}} := \widetilde\kappa^{\,m}/a \;<\; 1.
\]
No per-step $\mu$-budget of the previous form is asserted.
\end{theorem}
\begin{proof}
\emph{Ledger balance.} The obstruction ledger is additive
($\mathcal{D}=\mathcal{E}+\mathcal{B}$, Book~II \S SCT), its
per-step inflow is exhaustively $E_k+B_k$ (Unified
Coercive-Transport Inequality; \emph{rmk:three-sources}; Book~V
excludes hidden channels, as invoked in the proof of SCT.1), and the
H1/H2 loss channels are genuine removals disjoint from inflow; hence
$\mathfrak{D}_{k+1} \leq \mathfrak{D}_k - \Phi_k + \Psi_k$ with
$\Psi_k = E_k + B_k$.
\emph{Net contraction.} By \textsc{idc}~\eqref{hyp:NS-IDC-i}
together with Theorem~\ref{thm:NS-H3-PASS-incoherent},
$\Psi_k \leq \widetilde\eta\,\Phi_k$; with
...
\mathfrak{D}_k$.
\emph{Bridge.} By $\alpha$~\eqref{hyp:NS-alpha} and
$C_n = B^{\mathrm{bulk}}_n - I_n$ (Book~III), the obstruction is
$\mathcal{O}_{\mathrm{NS}}(W_k) =
\mathrm{Avg}^{\mathrm{tr}}_{W_k}(\mathcal{B}_n - (E_n+B_n))$;
since $E_n, B_n \geq 0$,
$\mathcal{O}_{\mathrm{NS}}(W_k) \leq \mathfrak{D}_k$. Above
threshold, $\mathrm{Avg}(\mathcal{B}) \geq T_{\mathrm{NS}}$;
by \textsc{idc}~\eqref{hyp:NS-IDC-ii},
$\mathfrak{D}_k \leq (1+C_{\mathcal{E}})\,
\mathrm{Avg}(\mathcal{B})$; by the SCT.4 ceiling,
$\mathrm{Avg}(\mathcal{B}) \leq M$; hence
$\mathcal{O}_{\mathrm{NS}}(W_k) \geq a\,\mathfrak{D}_k$ with
$a = T_{\mathrm{NS}}/(M(1+C_{\mathcal{E}}))$.
\emph{Combination.} Over $m$ steps,
$\mathcal{O}(W_{k+m}) \leq \mathfrak{D}_{k+m} \leq
\widetilde\kappa^{\,m}\mathfrak{D}_k \leq
(\widetilde\kappa^{\,m}/a)\,\mathcal{O}(W_k)$, and
$\lambda_{\mathrm{NS}} < 1$ for the stated $m$. \qed
\end{proof}
...
the named open hypotheses \textsc{idc} and $\alpha$; no per-step
$\mu$-budget claim survives the constants audit. With the
reconditioned NS.L1$'$ available, NS.P1c holds in the corresponding
$m$-step, \textsc{idc}/$\alpha$-conditional form.
\end{remark}
...
$\alpha$/\textsc{idc} bridge
(\eqref{hyp:NS-alpha}, \eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii})
and is pending. Moreover the SCT.6b application below is conditional
...
$\lambda_{\mathrm{NS}} = \widetilde\kappa^{\,m}/a < 1$ under
\textsc{idc} and $\alpha$. The former derivation via
...
\varepsilon_{\mathrm{bdry}}$). MIG-050 note: the translation of the
SB2 defect/coercive separation into a window-budget margin is itself
the $\alpha$/\textsc{idc} bridge (see the reconditioned
Thm.~\ref{thm:NSSB2}); that route is therefore not independent of
the hypotheses named above, and NS.G1+ stands only in the
reconditioned, \textsc{idc}/$\alpha$-conditional form.
\end{remark}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 \eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} (named open
 hypothesis; MIG-050);
 \item $\alpha$ --- obstruction/ledger alignment
 \eqref{hyp:NS-alpha} (named open hypothesis; MIG-050).
\end{enumerate}
\end{corollary}
...
Prop.~\ref{prop:NS-status} --- with the hypothesis set enlarged by
MIG-050 to include the named open hypotheses \textsc{idc} and
$\alpha$ (see Thm.~\ref{thm:NSL1}). The residuals are the external
unconditional Stage-3 question and the internal
\textsc{idc}/$\alpha$ burden.
\end{remark}
...
The NS branch is a lawful descendant of the canonical spine and
has reached domain-bounded conditional CERT-CLOSE, the conditional
hypothesis set including --- in addition to Phase-A, $K_0=7$, LS-2,
and PASS --- the named open hypotheses \textsc{idc}
\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
\eqref{hyp:NS-alpha} introduced by the MIG-050 constants-audit
reconditioning. No claim in this proposition may be cited without
that enlarged hypothesis set.
\end{proposition}
...
+ Phase-A + PASS + \textsc{idc} + $\alpha$; the ledger ratio
$\kappa\le 1-8/343$ stands, its transfer to the window obstruction
being part of the \textsc{idc}/$\alpha$ bridge --- MIG-050; the
former per-step margin claim is withdrawn).
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 enlarged hypothesis set including \textsc{idc} and $\alpha$
 (MIG-050),
 \item domain-bounded conditional CERT-CLOSE at the declared
 scope (Prop.~\ref{prop:NS-status}), conditional on the same
 enlarged hypothesis set including \textsc{idc} and $\alpha$
 (MIG-050), and
 ...
 quantitative frontier reopened by the MIG-050 constants audit:
 the named open hypotheses \textsc{idc}
 \eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} and $\alpha$
 \eqref{hyp:NS-alpha}.
\end{enumerate}
