...
none is presently available, and none is assumed anywhere in this
book:
\begin{enumerate}[label=\textup{(\alph*)}]
 ...
 and controlled remainder $\rho_k$, against the \emph{same
 contracting} quantity --- or, equivalently, a direct closed
 contracting recurrence for $\mathcal{O}_{\mathrm{NS}}$ itself.
 \item \emph{Complete channel accounting.} Explicit treatment of
 every channel of the three-source/loss--renewal map: the level
 stock $I_n$; the transported survivor $\Theta_n C_n$; the
 transported loss $(1-\Theta_n)C_n$ (presently \emph{unassigned}
 --- neither loss $\Phi_n$ nor renewal $\Psi_n$); the interior
 and boundary defect increments $E_n, B_n$; the renewal inflow
 $\Psi_n$; redistribution; and remainder.
...
threshold regime, with constants $0 < a \leq b$ and remainder
$\rho_k$ absorbed on the late tail (or, equivalently, its direct
closed contracting recurrence for $\mathcal{O}_{\mathrm{NS}}$).
Nothing in this theorem asserts that discharge, and the presently
open obligation furnishes no constants. Set
...
(remainder absorbed on the late tail) --- or a direct closed
contracting recurrence for $\mathcal{O}_{\mathrm{NS}}$ --- is
supplied here \emph{only} by the discharge of
...
$\lambda_{\mathrm{NS}} = (b/a)\,\widetilde\kappa^{\,m} < 1$
under \textsc{idc} and the hypothesized \emph{discharge} of
