...
route-conditional symbolic form
$\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2) < 1$ (the
...
\begin{itemize}
 ...
 frontier.} The former framing (``remaining open obligation:
...
\noindent\textbf{Section 10 is the operational center of the branch.} It records the exact conditional package from which the continuation theorem follows once the proof-program frontier (\textbf{UH.3}), the reserve-side structural input (\textbf{SB2}), \emph{and the controlling common-state burden} (Obl.~\ref{ob:NS-common-state-bridge}: total-renewal control, same-state obstruction comparison, and the ledger-state map) are supplied. No conditional package in this branch closes without that common-state discharge (MIG-052).
...
The NS branch should presently be read in one of two conditional modes; \emph{in both modes the common-state burden (Obl.~\ref{ob:NS-common-state-bridge}) is an additional, controlling input --- neither mode closes without it (MIG-052)}. In \emph{theoremic near-closure mode}, the spike-free side remains inside the proof program through \textbf{UH.3}, while \textbf{SB2} remains the reserve-side input. In \emph{dual-input mode}, the unresolved spike-free dissipation mechanism is entered explicitly as \textbf{NS.SF}, parallel to \textbf{SB2}. The second mode is strictly more explicit, not less rigorous: it keeps both residual ingredients local, named, and non-circular.
\end{remark}
...
Item~(2) is a remaining structural burden of NS.6.1, alongside the
controlling renewal-control route itself.
\end{corollary}
...
(Thm.~\ref{thm:NSP1c}), are conditional on hypotheses (5)--(6) ---
in particular on the open common-state bridge
...
the branch remains at CERT-PROJ force with that frontier open. The
...
Obl.~\ref{ob:NS-common-state-bridge}; the ledger contraction
stands in its symbolic route-conditional form
$\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2) < 1$ --- the
...
ledger-state map with positive contraction domain)}, the branch
