...
the domain-bounded conditional CERT-CLOSE recorded in
Prop.~\ref{prop:NS-status}. The residual to \emph{unconditional} force
is the external Stage-3 question (unconditional global regularity),
not any link of this chain.
...
\[
 ...
 \Rightarrow \mathrm{NS.7.1}\,[C],
\]
yielding the domain-bounded conditional CERT-CLOSE of
Prop.~\ref{prop:NS-status}. The residual is the external
unconditional Stage-3 question.
\end{remark}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 (NS.6.1, NS.7.1) are conditionally discharged
 (Cor.~\ref{cor:NS61-discharged}, Thm.~\ref{thm:NS71},
 Cor.~\ref{cor:NS-both-give-endpoint}),
 \item domain-bounded conditional CERT-CLOSE at the declared
 scope (Prop.~\ref{prop:NS-status}), and
 \item an explicit named failure frontier: originally Ren.5 and
 SB2, both now conditionally discharged; the residual frontier is
 the \emph{external unconditional} Stage-3 question
 (unconditional global regularity).
\end{enumerate}
The NS branch may not claim unconditional endpoint closure or
unconditional global regularity.
\end{corollary}
...
conditionally discharged, and the branch has reached domain-bounded
conditional CERT-CLOSE (Prop.~\ref{prop:NS-status}). What remains
is the single \emph{external unconditional} frontier: unconditional
global regularity (the Stage-3 Clay question), which lies outside
the declared conditional scope and is claimed nowhere in this book.
