...
such a discharge. Nothing in this theorem asserts the discharge
itself, and the presently open obligation furnishes no constants. Set
...
$\Psi_k = E_k + B_k$ --- that is, the exact requirement of
burden (e) that, after the stated assignment or exclusion of every
per-step channel (level stock, transported survivor
$\Theta_k C_k$, transported loss $(1-\Theta_k)C_k$, interior and
boundary defect increments $E_k, B_k$, renewal inflow,
redistribution, remainder), including in particular the assignment
or exclusion of the transported loss $(1-\Theta_k)C_k$, the renewal
term entering the ledger law is exactly $E_k + B_k$ --- is
\emph{not} available in current canon: no decomposition theorem defines
