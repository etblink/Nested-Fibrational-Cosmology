...
\begin{enumerate}[label=\textup{(\alph*)}]
 ...
 must be exactly $\Psi_n = E_n + B_n$, or must satisfy the
 inequality $\Psi_n \leq E_n + B_n$, which suffices for the
 ledger-contraction argument of NS.L1$'$. Explicit channel
 treatment without this identity (or inequality) does \emph{not}
...
such a discharge. \emph{Of the two accounting alternatives of
burden (e), this theorem consumes only the inequality branch}
$\Psi_k \leq E_k + B_k$ (the equality
$\Psi_k = E_k + B_k$ is a stronger optional special case and is
\emph{not} inferred here from a generic discharge); an
equality-only argument, where needed, would be a separate template.
Nothing in this theorem asserts the discharge itself, and the
...
\emph{The inflow accounting bound}
$\Psi_k \leq E_k + B_k$ --- the inequality branch of burden (e):
after the stated assignment or exclusion of every per-step channel
(level stock, transported survivor $\Theta_k C_k$, transported loss
$(1-\Theta_k)C_k$, interior and boundary defect increments
$E_k, B_k$, renewal inflow, redistribution, remainder), including in
particular the assignment or exclusion of the transported loss
$(1-\Theta_k)C_k$, the renewal term entering the ledger law is
bounded by $E_k + B_k$ --- is \emph{not} available in current
canon: no decomposition theorem defines
...
Under that hypothesised discharge, $\Psi_k \leq E_k + B_k$ (with
equality $\Psi_k = E_k + B_k$ the stronger optional special case,
not inferred here); the contraction argument below uses only the
inequality.
\emph{Net contraction.} From $\Psi_k \leq E_k + B_k$ and, by
\textsc{idc}~\eqref{hyp:NS-IDC-i} together with
Theorem~\ref{thm:NS-H3-PASS-incoherent},
$E_k + B_k \leq \widetilde\eta\,\Phi_k$, we obtain
