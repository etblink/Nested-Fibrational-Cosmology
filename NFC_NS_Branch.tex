...
follows: defect satisfies $\mathcal{D}_{n+1} \leq \mathcal{D}_n -
(1-\widetilde\eta)\Phi_n$ on the late tail, at CERT-PROJ force
while that route is open. With tail positivity of the
...
$\widetilde\kappa < 1$ (the route-conditional contraction ratio
...
$\Psi_n \leq \widetilde\eta\,\Phi_n$ gives
$D_{n+1} \leq D_n - (1-\widetilde\eta)\Phi_n \leq D_n -
(1-\widetilde\eta)(c_1+c_2)\mathfrak{D}_n$.
Setting $\widetilde\kappa = 1 - (1-\widetilde\eta)(c_1+c_2) < 1$
(since $(1-\widetilde\eta)(c_1+c_2) > 0$ from
$\widetilde\eta < 1$, $c_1,c_2 > 0$; $\widetilde\eta$ remains
symbolic while the route is open), we get
$\mathfrak{D}_{n+1} \leq \widetilde\kappa\,\mathfrak{D}_n$.
...
\[
 ...
 \;<\; 1,
\]
symbolic until that route is discharged. Current canon certifies
...
$\widetilde\kappa < 1$. No numerical value is asserted: the
former substitution of $\eta \leq 6/7$ into a total ratio used
the boundary coefficient beyond its proved scope. \qed
\end{proof}
...
Under Theorem~\ref{thm:NS-window-uniformity}: the
boundary/geometry coefficients $c_1 \geq 1/7$, $c_2 \geq 1/49$,
and $\eta \leq 6/7$ hold uniformly across all sufficiently late
windows (not just the minimal window). \emph{This certifies those
...
$\widetilde\kappa = 1 - (1-\widetilde\eta)(c_1+c_2) < 1$ at
every sufficiently late window
($\widetilde\eta = \eta + \eta_{\mathrm{int}} + \eta_R < 1$;
no numerical value is certified while the route is open). By
induction, $\mathfrak{D}_n \leq
