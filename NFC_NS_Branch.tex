...
\emph{Current posture:} CERT-PROJ / open common-state frontier,
with \textsc{uwb} retained as a separate unproved conditional scope
hypothesis (MIG-052). The NS.6.1/NS.7.1 conditional chain is
complete on the ledger side; its transfer to the window obstruction
--- hence the endpoint --- requires hypothetical discharge of the
open common-state bridge obligation
(Obl.~\ref{ob:NS-common-state-bridge}), while its finite-state,
periodicity, and global-uniformity route separately requires the
\textsc{uwb} uniform window-boundary hypothesis. The former
domain-bounded conditional CERT-CLOSE posture (MIG-050 wording) is
superseded and may not be cited; unconditional global regularity
(the Clay Stage-3 question) remains the external frontier.

\emph{Authoritative source:} \ref{prop:NS-status}~(final status proposition (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this
branch --- 1 (Obl.~\ref{ob:NS-common-state-bridge}); curated
named-obligation roster --- ob:NS-common-state-bridge, with
\textsc{idc}~\eqref{hyp:NS-IDC-i}--\eqref{hyp:NS-IDC-ii} recorded
under it as subsidiary and insufficient and
$\alpha$~(\texttt{hyp:NS-alpha}) withdrawn; reduced irreducible
frontier --- the internal common-state bridge and the external
unconditional-regularity question; separate non-obligation scope
hypothesis --- \textsc{uwb}, unproved and not included in either
frontier count; branch posture --- CERT-PROJ / open common-state
frontier, with \textsc{uwb} retained as a separate unproved
conditional scope hypothesis.}
...
and, wherever full late-window uniformity is consumed, on the
separate unproved \textsc{uwb} uniform window-boundary hypothesis
--- see Thm.~\ref{thm:NSL1} and
Cor.~\ref{cor:NS61-discharged}, MIG-052), whenever
...
\textup{[In the current NS branch, satisfaction of the active
contraction hypothesis and its promotion across the declared global
window regime require both hypothetical discharge of the common-state
bridge (Obl.~\ref{ob:NS-common-state-bridge}) and the separate
unproved \textsc{uwb} uniform window-boundary scope hypothesis.]}
...
$\mathfrak{D}_{n+1}\leq\widetilde\kappa\,\mathfrak{D}_n$ of
Thm.~\ref{thm:NS61-global} --- with
$\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2)$ symbolic and
route-conditional (Thm.~\ref{thm:NS-kappa-explicit},
reconditioned): current canon certifies only the boundary
coefficient $\eta \leq 6/7$, not $\eta_{\mathrm{int}}$ or
$\eta_R$, so no explicit numerical total ratio is presently
certified --- transfers to the imbalance only through that open
bridge, so the discharge holds at CERT-PROJ force.) Given the
hypothesis, uniformity across all sufficiently late windows of the
active architecture is supplied by the complete chain actually
consumed --- no single element extends the contraction uniformly by
itself (Corrective~I): the finite-state/isometry classification
(Thm.~\ref{thm:NS-window-uniformity}, under its \textsc{uwb}
uniform window-boundary hypothesis), the deterministic transition
(Thm.~\ref{thm:NS-window-determinism}), the eventual-periodicity
corollary (Cor.~\ref{cor:NS-eventual-periodicity}, conditional on
\textsc{uwb}), and the global contraction theorem
...
Corollary~\ref{cor:NS61-discharged}: the window-uniformity
hypothesis (Thm.~\ref{thm:NS-window-uniformity} in its
finite-state/isometry form, conditional on the \textsc{uwb}
uniform window-boundary hypothesis --- Corrective~I --- with the
Q2a determinism resolution of
Thms.~\ref{thm:NS-window-determinism}--\ref{cor:NS-eventual-periodicity})
supplies the missing uniformity under \textsc{uwb}, and
Thm.~\ref{thm:NS61-global} supplies the ledger contraction in its
route-conditional symbolic form: in the geometric case
$q=\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2)\in(0,1)$,
with the finite-extinction alternative treated separately (the
former explicit bound $\kappa \le 1-8/343$ treated the
boundary-only $\eta$ as a total-renewal coefficient and is
withdrawn from active use --- MIG-052). \emph{MIG-050
...
frontier advances to NS.7.1 (Thm.~\ref{thm:NS71}), which additionally
requires the Book~VI $H^1_x$-norm interface legitimacy and is
conditionally proved on the same enlarged hypothesis set: the
common-state discharge is controlling and \textsc{uwb} is retained
separately as an unproved scope hypothesis. The NS branch therefore
holds its domain-bounded
...
On the complete current hypothesis set of
Theorem~\ref{thm:NS61} --- explicitly including hypothetical
discharge of Obligation~\ref{ob:NS-common-state-bridge} and the
separate unproved \textsc{uwb} scope hypothesis --- the branch lawfully
...
open common-state bridge obligation
Obl.~\ref{ob:NS-common-state-bridge} --- in particular the
hypothetical discharge of its renewal-control burden, part~(e), and
its ledger-state map with positive contraction-factor domain,
part~(f) --- with \textsc{idc} subsidiary and the \textsc{uwb}
uniform window-boundary hypothesis, per MIG-052/Corrective~I),
Book~III source-descended coercive structure, and Book~VI interface
legitimacy for the $H^1_x$-norm.]}
...
\begin{proof}
Under NS.6.1 (Cor.~\ref{cor:NS61-discharged}), the ledger-side
quantity satisfies, in the geometric case, the iterated
multiplicative contraction
\[
 \mathfrak{D}_{n} \;\leq\; q^{\,n-n_0}\,\mathfrak{D}_{n_0}
 \;\longrightarrow\; 0,
 \qquad
 q = \widetilde\kappa \in (0,1),
\]
with the declared factor $q = \widetilde\kappa =
1-(1-\widetilde\eta)(c_1+c_2)$ route-conditional and symbolic:
$q < 1$ from the certified bounds and $q > 0$ furnished by the
positive contraction-factor domain of the ledger-state burden
(Obl.~\ref{ob:NS-common-state-bridge}, part~(f)), under the
hypothetical discharge of parts~(e) and~(f) (Corrective~I; the
former display here used a superseded plain $\kappa$ and the
incorrect index $\mathfrak{D}_{n+1}$ on the left). In the separate
finite-extinction case $(1-\widetilde\eta)(c_1+c_2) \geq 1$,
nonnegativity forces $\mathfrak{D}_n = 0$ for all large $n$. In
either case lawful continuability holds throughout the promoted
target scope, at CERT-PROJ force: the identification of the
contracting ledger quantity with the transport-weighted imbalance
is exactly the open common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}); the former identity
...
In the geometric case, the decay
$\mathfrak{D}_n \leq q^{\,n-n_0}\mathfrak{D}_{n_0}$ with
$q \in (0,1)$ gives the summable obstruction sequence
$\sum_{n \geq n_0} \mathfrak{D}_n \leq
\mathfrak{D}_{n_0}/(1-q) < \infty$; in the finite-extinction
case the sequence has finitely many nonzero terms and is summable
trivially.
...
separately include the unproved \textsc{uwb} uniform
window-boundary scope hypothesis, per MIG-052/Corrective~J.]}
...
The \emph{current NS named failure frontier} is the formal internal
open obligation \emph{obstruction/ledger common-state bridge}
(Obl.~\ref{ob:NS-common-state-bridge}). Its hypothetical discharge
must supply the common temporal state, renewal control, same-state
comparison, and ledger-state map consumed by the NS.6.1/NS.7.1
conditional chain. The \textsc{uwb} uniform window-boundary
hypothesis is a separate, explicit, unproved conditional scope
restriction required by the present variable-window
global-uniformity template; it is not a new~[O], is not part of the
common-state obligation, and is not included in the reduced-frontier
count. The former Ren.5, UH.3, and SB2 frontier items are
conditionally discharged predecessor items and are not the current
formal burden. The subsidiary-insufficient \textsc{idc} hypotheses
remain recorded under the common-state analysis, while
$\alpha$~(\texttt{hyp:NS-alpha}) is withdrawn historical intake.
\end{definition}

\begin{theorem}[\status{U}]\label{thm:NS-frontier}
\textup{(NS Frontier Theorem.)} The NS branch cannot lawfully
promote its ledger-side conditional contraction to the window
obstruction or endpoint unless the common-state bridge obligation
(Obl.~\ref{ob:NS-common-state-bridge}) is hypothetically discharged.
Within the present variable-window global-uniformity template, the
conditional endpoint chain additionally requires the separate
unproved \textsc{uwb} scope hypothesis.
\end{theorem}

\begin{proof}
By the NS branch intake law (Proposition~\ref{prop:NS-lawful}), the
branch is lawful only through declared source descent and explicit
bridge structure. The current contraction is established only on the
ledger side. Obligation~\ref{ob:NS-common-state-bridge} is precisely
the missing typed passage from that ledger state to the window
obstruction; without its hypothetical discharge, the Continuum
Bridge Schema of Book~VI forbids promotion to endpoint force.
Separately, Corrective~I established that the scale-adaptive window
definition supplies no globally finite state space without
\textsc{uwb}; hence the present periodicity/global-uniformity route
also cannot be consumed without that explicit scope hypothesis.
Ren.5, UH.3, and SB2 do not replace either missing condition.
\qed
\end{proof}

\begin{corollary}[\status{U}]\label{cor:NS-rephrasing-useless}
Rephrasing the NS branch without hypothetically discharging
Obligation~\ref{ob:NS-common-state-bridge}, or consuming the present
variable-window global chain without explicitly retaining
\textsc{uwb}, does not advance the endpoint claim.
\end{corollary}
...
transport factor satisfies $\vartheta_k \in [0,1]$ generically:
it is the gross Book~III transported-survivor factor, and no
identification of $\vartheta_k$ with any net SCT loss/renewal
contraction ratio is made here. (The former identification
$\vartheta_k = \kappa \leq 1-8/343$ is \emph{withdrawn} ---
MIG-052; it conflated the gross transport factor with the net
ratio and used the boundary-only $\eta$ as a total coefficient.)
...
\emph{MIG-050 note, sharpened MIG-052:} identifying $\vartheta_k$
with the \emph{net} ratio $\kappa$ conflates the gross Book~III
transport factor $\Theta_n$ with the net SCT.6b ratio; that former
identification \emph{is withdrawn} (not merely unrelied-upon): the
theorem asserts only the generic $\vartheta_k \in [0,1]$, and the
reconditioned contraction chain (Thm.~\ref{thm:NSL1}) does not use
...
here $\Psi_k^{\mathrm{int}}$ and $\Psi_k^{\mathrm{bdry}}$ denote
proposed interior and boundary renewal components of the total
per-step ledger inflow $\Psi_k$ --- \emph{present-but-not-
independently-defined targets}: this block does \emph{not} assert
that $\Psi_k = \Psi_k^{\mathrm{int}}+\Psi_k^{\mathrm{bdry}}$ is an
exhaustive canonical decomposition (no such decomposition theorem
exists in canon; the first exhaustive, disjoint decomposition
$\Psi_k = \Psi_k^{\mathrm{int}}+\Psi_k^{\mathrm{bdry}}+R_k$ is
supplied only under a hypothetical Route~A discharge of
Obl.~\ref{ob:NS-common-state-bridge}, part~(e)). With
$\mathcal{E}_k,\mathcal{B}_k$ the interior and boundary components
of the Book~II obstruction ledger,
...
\begin{enumerate}[label=\textup{(\alph*)}]
 ...
 burden.
 \item \emph{Ledger-state map (branch-ledger implementation ---
 option~B of the Corrective-H authorization's correction~1;
 unrelated to the Route~A/B/C renewal taxonomy of burden~(e), whose
 Route~A is the route NS.L1$'$ consumes).} The active chain distinguishes
 three objects that current canon never identifies: the Book~II
 visible obstruction ledger
 $\mathcal{D}_n = \mathcal{E}_n + \mathcal{B}_n$; the NS
 successor-defect quantity $\mathfrak{D}_n$; and the Stage-2a
 defect-burden operator $D_n$ (an operator, never ledger
 shorthand). Discharge must supply an explicit, typed map from
 the Book~II ledger $\mathcal{D}_n$ to the branch state
 $\mathfrak{D}_n$ specifying: the temporal index; the
 window/averaging convention; the normalization; the transfer of
 the H1/H2 lower bounds
 ($\Phi^{\mathrm{quot}}_n \geq c_1\,\mathfrak{D}_n$,
 $\Phi^{\mathrm{mix}}_n \geq c_2\,\mathfrak{D}_n$) between the
 two states; the transfer of the loss/renewal recurrence
 ($\mathcal{D}$-side SCT.6b recurrence
 $\to$ $\mathfrak{D}$-side
 $\mathfrak{D}_{k+1} \leq \mathfrak{D}_k - \Phi_k + \Psi_k$);
 the same-state obstruction comparison of part~(d); preservation
 of nonnegativity; \emph{and the positive contraction-factor
 domain}: the resulting factor
 $\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2)$ must be
 furnished in $(0,1)$ (equivalently
 $0 < (1-\widetilde\eta)(c_1+c_2) < 1$), or the discharge must
 treat the extinction case
 $(1-\widetilde\eta)(c_1+c_2) \geq 1$ (which forces
 $\mathfrak{D} = 0$ in finitely many steps by nonnegativity)
 separately. Until this map is discharged, \emph{no recurrence,
 lower bound, or comparison may pass between $\mathcal{D}_n$ and
 $\mathfrak{D}_n$}, and no logarithmic threshold or geometric
 rate may be stated for $\widetilde\kappa$.
...
$\widetilde\kappa := 1 - (1-\widetilde\eta)(c_1+c_2)$, with
$\widetilde\kappa < 1$ from the certified bounds and
$\widetilde\kappa > 0$ \emph{hypothesised as part of the
burden-(f) contraction domain} (so
$\widetilde\kappa \in (0,1)$; in the excluded extinction case
$(1-\widetilde\eta)(c_1+c_2) \geq 1$, nonnegativity forces
$\mathfrak{D} = 0$ in finitely many steps and every conclusion
below holds trivially with any $m \geq 1$ --- the logarithmic
threshold applies only in the geometric case). Then
...
\end{theorem}
\begin{proof}
\emph{Ledger balance.} Book~II supplies additivity and the
loss/renewal recurrence for \emph{its own} visible ledger
$\mathcal{D}=\mathcal{E}+\mathcal{B}$ (\S SCT), with the H1/H2
loss channels genuine removals disjoint from inflow. \emph{The
same-state recurrence for the branch quantity},
$\mathfrak{D}_{k+1} \leq \mathfrak{D}_k - \Phi_k + \Psi_k$, is
\emph{not} a current-canon theorem --- no map identifies or
transfers between $\mathcal{D}_k$ and $\mathfrak{D}_k$ --- and is
taken here \emph{only} from the ledger-state map of burden~(f)
under the hypothesised discharge of
Obl.~\ref{ob:NS-common-state-bridge}.
...
remains pending. Moreover, hypothesis (H3) as proved
(Thm.~\ref{thm:NS-H3-PASS-incoherent}) covers the \emph{boundary}
inflow channel only, and \textsc{idc}-i alone does \emph{not}
supply an exhaustive total-renewal decomposition; the SCT.6b
application below is therefore conditional additionally on a
hypothetical renewal-control discharge (Route~A/B/C of
Obl.~\ref{ob:NS-common-state-bridge}, part~(e)) supplying the total
bound $\Psi_n \leq \widetilde\eta\,\Phi_n$ with
$\widetilde\eta < 1$.

\textup{Conditional on} quotient-loss on defect~(H1), finite-collar
mixing loss~(H2), \emph{boundary-only} PASS-incoherent renewal~(H3,
Thm.~\ref{thm:NS-H3-PASS-incoherent}), \emph{and} a hypothetical
renewal-control discharge (Obl.~\ref{ob:NS-common-state-bridge},
part~(e)) lifting the boundary bound to the total-renewal bound
$\Psi_n \leq \widetilde\eta\,\Phi_n$ that Book~II
Theorem~\ref{thm:SCT6b}~(SCT.6b) consumes.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:NSRen5} (Ren.5, now discharged), the
proof-program frontier has been cleared: the tail is spike-free
in the boundary term. H3 as proved
(Thm.~\ref{thm:NS-H3-PASS-incoherent}) bounds the \emph{boundary}
renewal channel only; SCT.6b consumes a bound on the \emph{total}
renewal $\Psi_n$. That total bound,
$\Psi_n \leq \widetilde\eta\,\Phi_n$ with
$\widetilde\eta < 1$, is supplied here \emph{only} by the
hypothetical renewal-control discharge of
Obl.~\ref{ob:NS-common-state-bridge}, part~(e), named in the
statement (boundary-only $\eta$ is \emph{not} used as a
total-renewal coefficient; no discharge proving
$\widetilde\eta = \eta$ is asserted). Under that hypothetical
discharge, by the Spine SCT.6b
(Book~II~\S\,\ref{sec:sct-doctrine}) strict dissipation bias
follows \emph{for the Book~II visible ledger}: 
$\mathcal{D}_{n+1} \leq \mathcal{D}_n -
(1-\widetilde\eta)\Phi_n$ on the late tail, at CERT-PROJ force
while that route is open. This theorem concludes \emph{only} this
$\mathcal{D}$-recurrence and the $\mathcal{D}$-separation below;
no statement about the branch quantity $\mathfrak{D}_n$ is made
here --- any transfer to $\mathfrak{D}_n$ passes through the
ledger-state map of burden~(f), consumed downstream
(\S\ref{sec:ns61-partial}). With tail positivity of the
...
withdrawn as unproved --- MIG-050; the former $\alpha$/\textsc{idc}
route for it is \emph{withdrawn} ($\alpha$ supplies no active
support; \textsc{idc} is subsidiary and insufficient), and the
translation now depends on the hypothetical discharge of
Obl.~\ref{ob:NS-common-state-bridge}.) The key asymmetry:
...
structural input} to a \emph{conditional theorem}.
\emph{(Historical transition note, MIG-014-era, superseded in
part:)} the burden was then described as verification of
(H1)--(H3) alone, the branch ``conditionally blocked by one
primitive-derived bias packet.'' \emph{Current posture (MIG-052):}
the controlling open burden is the common-state renewal-control
route (Obl.~\ref{ob:NS-common-state-bridge}, part~(e)) --- H3 as
proved covers the boundary channel only, and lifting it to the
total-renewal bound SCT.6b consumes passes through that route ---
in addition to the explicitly retained quantitative verifications
of H1, H2, and boundary-only H3. Filtration evidence that
spike-free tails can approach saturation without positive reserve
is fully explained: without SCT.6b, bounded control does not imply
strict separation. With SCT.6b (available here only under the
hypothetical renewal-control discharge), the equilibrium failure
mode is excluded because the quotient erases mismatch faster than
PASS-incoherent renewal can regenerate it.
\end{remark}
...
The controlling open NS burden is the common-state
renewal-control route (Obl.~\ref{ob:NS-common-state-bridge},
part~(e)); the verification of the SCT.6b hypotheses in the NS
safe-tail regime --- H1, H2, and boundary-only H3 below --- is
\emph{subsidiary} to it. Each subsidiary item is stated precisely:
\begin{itemize}
...
applies \emph{to the Book~II visible ledger}: the
primitive-derived dissipation bias forces
$\mathcal{D}_{n+1} \leq \mathcal{D}_n -
(1-\widetilde\eta)\Phi_n$, with
$\Phi_n = \Phi_n^{\mathrm{quot}} + \Phi_n^{\mathrm{mix}} > 0$ on
the active tail. This is the positive-reserve conclusion of SB2
(Theorem~\ref{thm:NSSB2}) --- a statement about
$\mathcal{D}_n, \mathcal{C}_n$, and $\Phi_n$ \emph{only} --- at
CERT-PROJ force while that renewal-control route is open. No
conclusion about the branch quantity $\mathfrak{D}_n$ is drawn
here (the branch-state lower bounds
$\Phi^{\mathrm{quot}}_n \geq c_1\mathfrak{D}_n$,
$\Phi^{\mathrm{mix}}_n \geq c_2\mathfrak{D}_n$ of H1/H2 enter
the $\mathcal{D}$-recurrence only through the ledger-state map of
burden~(f), consumed downstream). Hence SB2 is conditionally
discharged on the same hypothetical-discharge footing. \qed
\end{proof}
...
Obl.~\ref{ob:NS-common-state-bridge}, part~(e)), open ---
\emph{that route is the controlling open burden}. The
\emph{subsidiary} quantitative requirements (the original named
list; their boundary/geometry scope has since been certified) are:
\begin{enumerate}[label=\textup{(\arabic*)}]
...
Obl.~\ref{ob:NS-common-state-bridge}, whose burden-(f) contraction
domain places $\widetilde\kappa \in (0,1)$ (extinction case
separate; the presently open obligation furnishes no constants). The former derivation via
...
predecessor derivation path below. In the current MIG-052 posture,
Ren.5, UH.3, and SB2 are conditionally discharged predecessor items,
not the formal frontier; every operative global composition of this
path additionally consumes hypothetical discharge of
Obligation~\ref{ob:NS-common-state-bridge} and the separate unproved
\textsc{uwb} scope hypothesis:
\begin{align*}
 &\text{Ren.5}
...
\textbf{Current NS frontier (MIG-052).}
\begin{itemize}
 ...
 chain Ren.5$\Rightarrow$SB1 is active.
 \item \textbf{Controlling open burden: the common-state
 renewal-control/ledger-state obligation}
 (Obl.~\ref{ob:NS-common-state-bridge}) --- total-renewal
 control (part~(e)), the same-state two-sided obstruction
 comparison (part~(d)), and the ledger-state map with its
 positive contraction-factor domain (part~(f)). \emph{Current
 canon does not establish a ledger-to-obstruction transfer};
 every ledger contraction in this branch is a hypothetical
 template pending that discharge.
 \item \textbf{Subsidiary items:} H1, H2, and \emph{boundary-only}
 H3 (Thm.~\ref{thm:NS-H3-PASS-incoherent}), with the certified
 boundary/geometry quantities $c_1, c_2, \eta$. SB2 is a
 conditional theorem whose SCT.6b application consumes the
 hypothetical renewal-control discharge; its conclusion is a
 Book~II $\mathcal{D}$-statement.
 \item \textbf{Separate conditional scope hypothesis: \textsc{uwb}.}
 The uniform window-boundary bound is explicitly assumed, not
 proved; it is required for the finite-state, periodicity, and
 global-uniformity route, but is not a new~[O] and is not part of
 the common-state obligation or reduced-frontier count.
 \item \textbf{Branch posture: CERT-PROJ / open common-state
 frontier, with \textsc{uwb} retained as a separate unproved
 conditional scope hypothesis.} The former framing (``remaining open obligation:
 verify H1--H3; blocked by one primitive-derived bias packet'')
 is historical, MIG-014-era, and superseded.
\end{itemize}
\end{minipage}}
\end{center}
...
\noindent\textbf{Section 10 records predecessor conditional-package
modes; it does not define the current frontier.} UH.3 and SB2 are
conditionally discharged predecessor inputs. Every active reading of
either package that concludes NS.6.1 or the endpoint additionally
requires hypothetical discharge of the controlling common-state
burden (Obl.~\ref{ob:NS-common-state-bridge}: total-renewal control,
same-state obstruction comparison, and the ledger-state map) and the
separate unproved \textsc{uwb} uniform window-boundary scope
hypothesis. No conditional package in this branch closes without
both conditions (MIG-052/Corrective~J).
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 \item discharge of the spike-free proof-program chain through \textbf{UH.3},
 \item the hypothetical discharge of the controlling common-state burden (Obl.~\ref{ob:NS-common-state-bridge}), and
 \item the separate unproved \textsc{uwb} uniform window-boundary
 scope hypothesis, and
...
Assume the hypotheses of Theorem~\ref{thm:NSTN2} --- explicitly
including hypothetical discharge of
Obligation~\ref{ob:NS-common-state-bridge} and the separate unproved
\textsc{uwb} scope hypothesis --- and also \textbf{NS.7.1}. Then the
NS endpoint target follows.
\end{corollary}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 \item \textbf{SB2} (Positive Reserve on the Safe Tail),
 \item the hypothetical discharge of the controlling common-state
 burden (Obl.~\ref{ob:NS-common-state-bridge}), and
 \item the separate unproved \textsc{uwb} uniform window-boundary
 scope hypothesis.
\end{enumerate}
...
\begin{proof}
By Definition~\ref{def:NSSF}, recurring admissible boundary novelty
becomes eventually non-dangerous in transport-normalized units on late
active tails. Hence the spike-free chain culminates in
$\mathrm{SB1}$. With NS.L1a, retained transport ceiling, and SB2, the
late safe tail has positive reserve. The resulting ledger-side gap
passes to $\mathrm{NS.NS2}$ and $\mathrm{NS.G1+}$ only under the
hypothetical common-state discharge; \textsc{uwb} supplies the finite
global state space used for tail-uniform promotion. Thus the strict
gap gives above-threshold contraction and NS.6.1 only on the complete
stated hypothesis set. \qedhere
\end{proof}

\begin{corollary}[\status{C}]\label{cor:NSDI3}
\textup{(Dual-Input Conditional Endpoint Corollary.)}
Assume the hypotheses of Theorem~\ref{thm:NSDI2} --- explicitly
including hypothetical discharge of
Obligation~\ref{ob:NS-common-state-bridge} and the separate unproved
\textsc{uwb} scope hypothesis --- and also \textbf{NS.7.1}. Then the
NS endpoint target follows.
...
The NS branch may be read through either predecessor conditional mode;
\emph{in both modes the common-state burden
(Obl.~\ref{ob:NS-common-state-bridge}) is an additional controlling
input and \textsc{uwb} is a separate unproved scope hypothesis ---
neither mode closes without both (MIG-052/Corrective~J)}. UH.3, NS.SF,
and SB2 describe the predecessor proof architecture and are not the
current formal frontier.
\end{remark}

\begin{remark}[\status{R}]\label{rmk:SB2Freeze}
\textup{(Reserve-side freeze note --- historical predecessor record.)}
At the stage when this note was written, the reserve side had been
reduced to SB2 and safe-but-saturating witness behavior showed why
spike-freedom alone did not supply reserve. SB2 was therefore carried
as an explicit structural input. It has since been conditionally
discharged and is not the current formal frontier; its conclusion
remains only a boundary-side predecessor statement and supplies no
common-state transfer, \textsc{uwb}, continuation, or endpoint
closure.
\end{remark}

\begin{remark}[\status{R}]\label{rmk:NSXB2}
\textup{(Cross-branch closure-style refinement.)}
The Yang--Mills and Navier--Stokes branches do not share a terminal
theorem, but they do share a closure style inherited from the common
coercive-transport source law. The predecessor NS program did not
lack that style; it lacked branch-local subordination and reserve
statements. Those items are now conditionally discharged
predecessors. The current formal internal frontier is the common-state
bridge (Obl.~\ref{ob:NS-common-state-bridge}); \textsc{uwb} is
separately retained as an unproved scope hypothesis for the
global-window route.
\end{remark}

\begin{remark}[\status{R}]\label{rmk:NSDualInput}
\textup{(Dual-input conditional closure note --- predecessor mode.)}
This note records the earlier dual-input organization of UH.3/NS.SF
and SB2; those are now conditionally discharged predecessor items,
not the current formal frontier. Any active use of the associated
package theorem additionally requires hypothetical discharge of the
controlling common-state burden
(Obl.~\ref{ob:NS-common-state-bridge}: total-renewal control,
same-state comparison, ledger-state map with positive contraction
domain) and the separate unproved \textsc{uwb} scope hypothesis.
Absent either condition, the stack remains at CERT-PROJ force and no
global endpoint follows.
\end{remark}
...
NS.AC.)} This is a predecessor-architecture anti-circularity record,
not a statement of the current frontier; SB2 is now conditionally
discharged and the current formal internal burden is
Obligation~\ref{ob:NS-common-state-bridge}. A skeptical reader may
object that the boundary-side
...
Window-Contraction --- via the Hypothetical Ledger-State Map,
burden~(f).)}
\textup{[Conditional on SB2 discharge,
Theorems~\ref{thm:NS-H1-quotient-loss}--\ref{thm:NS-SB2-discharged},
the hypothetical renewal-control discharge
(Obl.~\ref{ob:NS-common-state-bridge}, part~(e)) those theorems
now consume, and the NS safe-tail architecture.]}
...
safe tail: there exists a symbolic
$\widetilde\kappa \in (0,1)$ --- positivity supplied by the
burden-(f) contraction domain, with the extinction case
$(1-\widetilde\eta)(c_1+c_2) \geq 1$ treated separately in the
proof --- (the route-conditional contraction ratio
$\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2)$; $c_1$,
$c_2$, and the boundary coefficient $\eta$ are certified, while
$\eta_{\mathrm{int}}$ and $\eta_R$ are supplied only by the
hypothetical renewal-control discharge) such that
\[
 \mathfrak{D}_{n+1} \;\leq\; \widetilde\kappa\,\mathfrak{D}_n
...
\begin{proof}
By Theorem~\ref{thm:NS-SB2-discharged} under its hypothetical
renewal-control discharge (Obl.~\ref{ob:NS-common-state-bridge},
part~(e)), the total-renewal bound
$\Psi_n \leq \widetilde\eta\,\Phi_n$ gives the \emph{Book~II
ledger} recurrence
$\mathcal{D}_{n+1} \leq \mathcal{D}_n -
(1-\widetilde\eta)\Phi_n$. \emph{This is a
$\mathcal{D}$-statement; no $\mathfrak{D}$-recurrence follows
from it in current canon.} The passage to the branch state is
supplied \emph{only} by the ledger-state map of burden~(f) under
the same hypothesised discharge: that map transfers the
loss/renewal recurrence to
$\mathfrak{D}_{n+1} \leq \mathfrak{D}_n - \Phi_n + \Psi_n$ and
the H1/H2 lower bounds to the same state
($\Phi_n \geq (c_1+c_2)\mathfrak{D}_n$), whence
$\mathfrak{D}_{n+1} \leq \mathfrak{D}_n -
(1-\widetilde\eta)(c_1+c_2)\mathfrak{D}_n
= \widetilde\kappa\,\mathfrak{D}_n$ with
$\widetilde\kappa = 1 - (1-\widetilde\eta)(c_1+c_2)$. The
certified bounds give $\widetilde\kappa < 1$; the domain
$\widetilde\kappa > 0$ is part of the burden-(f) contraction
domain (if instead $(1-\widetilde\eta)(c_1+c_2) \geq 1$, then
nonnegativity forces $\mathfrak{D} = 0$ in finitely many steps ---
the extinction case, in which the contraction condition of
Theorem~\ref{thm:NS61} holds trivially). In either case, we get
$\mathfrak{D}_{n+1} \leq \widetilde\kappa\,\mathfrak{D}_n$
with $\widetilde\kappa \in (0,1)$, or finite extinction. This is
the multiplicative window-to-window contraction condition of
Theorem~\ref{thm:NS61}. \qed
\end{proof}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 \item \emph{Explicit $\widetilde\kappa$ computation}: certify
 $\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2)$ explicitly.
 The boundary/geometry quantities $c_1$, $c_2$, $\eta$ are
 certified (Remark~\ref{rem:NS-SB2-status} and the quantitative
 propositions), but
 $\widetilde\eta = \eta + \eta_{\mathrm{int}} + \eta_R$ also
 requires the route coefficients
 $\eta_{\mathrm{int}}, \eta_R$, supplied only by discharging
 the renewal-control route
 (Obl.~\ref{ob:NS-common-state-bridge}, part~(e)); until then
 $\widetilde\kappa$ is symbolic.
...
\end{enumerate}
Item~(1) becomes a finite quantitative computation \emph{only
after} the renewal-control route is discharged; its
$c_1, \eta$ part is complete, and its $c_2$ part is complete
per-window ($c_2 \geq K_0^{-k}$), uniformly only as
$c_{2,*} = K_0^{-K_*}$ under \textsc{uwb} (Corrective~I).
Item~(2) is supplied conditionally by
Theorem~\ref{thm:NS61-global} under \textsc{uwb}; without \textsc{uwb}
no global promotion is asserted. Neither item is a separate current
frontier: the formal internal burden is
Obligation~\ref{ob:NS-common-state-bridge}, and \textsc{uwb} is the
separate unproved scope hypothesis.
\end{corollary}
...
CERT-CLOSE wording is superseded). The formal residuals are the
external Stage-3 question (unconditional global regularity) and the
internal common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}), through which every link's
transfer to the window obstruction passes. Separately, the
global-window composition retains the unproved \textsc{uwb} scope
hypothesis; it is not part of either frontier count.
...
sites: $c_2 \geq K_0^{-2} = 1/49$. \emph{The bound $K_0^{-k}$
decreases as $k$ increases} (Corrective~I; the former closing
sentence ``more refined windows give larger $c_2$'' asserted the
opposite size dependence and is withdrawn): larger windows give a
\emph{weaker} lower bound. A uniform positive lower bound across
the variable-size window family therefore requires a uniform bound
$k \leq K_*$ on the active boundary size (the \textsc{uwb}
hypothesis, Thm.~\ref{thm:NS-window-uniformity}), under which
$c_2 \geq K_0^{-K_*} > 0$ uniformly. The numerical value $1/49$
is the $k = 2$ (minimal-window) instance only. \qed
\end{proof}
...
\textup{(NS.6.1 Contraction Ratio --- Symbolic, Route-Conditional;
Reconditioned MIG-052.)}
Under the quantitative bounds above \emph{and a hypothetical
discharge of the renewal-control route
(Obl.~\ref{ob:NS-common-state-bridge}, part~(e)) supplying the
coefficients $\eta_{\mathrm{int}}, \eta_R$ with
$\widetilde\eta = \eta + \eta_{\mathrm{int}} + \eta_R < 1$},
the NS.6.1 multiplicative contraction ratio is
\[
 \widetilde\kappa \;=\; 1 - (1-\widetilde\eta)(c_1 + c_2)
 \;\in\; (0,1),
\]
symbolic until that route is discharged --- $\widetilde\kappa < 1$
from the certified bounds, $\widetilde\kappa > 0$ from the
burden-(f) contraction domain (the extinction case
$(1-\widetilde\eta)(c_1+c_2) \geq 1$, which forces
$\mathfrak{D} = 0$ in finitely many steps, is treated separately
wherever consumed). Current canon certifies
only $c_1 \geq 1/7$, $c_2 \geq 1/49$ \emph{on the minimal
($k=2$) window} (the per-window bound $K_0^{-k}$ decreases with
$k$; a uniform positive $c_2$ requires the \textsc{uwb}
hypothesis, giving $c_{2,*} = K_0^{-K_*}$ --- Corrective~I), and
the \emph{boundary} coefficient $\eta \leq 6/7$; it certifies
neither
$\eta_{\mathrm{int}}$ nor $\eta_R$, so \emph{no explicit
numerical total-ledger contraction bound is presently certified}.
\emph{Superseded (MIG-052; retained as historical intake):} the
former content of this theorem displayed
$\kappa = 1-(1-\eta)(c_1+c_2) \leq 1-8/343 \approx 0.9767$ and
claimed completion of the quantitative certification for NS.6.1;
that display treated the boundary-only $\eta$ as a total-renewal
coefficient and is withdrawn from active use.
\end{theorem}

\begin{proof}
With $\widetilde\eta < 1$ supplied by the hypothesised route
discharge (Obl.~\ref{ob:NS-common-state-bridge}, part~(e)) and
$c_1 + c_2 > 0$ from the certified quantitative propositions,
$(1-\widetilde\eta)(c_1+c_2) > 0$, hence
$\widetilde\kappa < 1$; and $\widetilde\kappa > 0$ holds by the
burden-(f) contraction domain hypothesised with the same discharge
($0 < (1-\widetilde\eta)(c_1+c_2) < 1$), so
$\widetilde\kappa \in (0,1)$. The certified lower bounds on
$c_1, c_2$ alone prove only $\widetilde\kappa < 1$, not
positivity. No numerical value is asserted: the former
substitution of $\eta \leq 6/7$ into a total ratio used the
boundary coefficient beyond its proved scope. \qed
\end{proof}

\begin{remark}[\status{R}]
\textbf{NS quantitative summary (reconditioned MIG-052).}
The three boundary/geometry quantities are certified with explicit
bounds:
\begin{align*}
 c_1 &\geq 1/7, \\
 c_2 &\geq K_0^{-k} \text{ (per-window, $k$ boundary sites;
 minimal window $k=2$: $1/49$; the bound \emph{decreases} with
 $k$ --- a uniform positive bound requires \textsc{uwb}, giving
 $c_{2,*} = K_0^{-K_*}$; Corrective~I)}, \\
 \eta &\leq 6/7 \text{ (boundary renewal channel only)}.
\end{align*}
These are \emph{subsidiary} quantitative requirements. \emph{No
explicit total contraction ratio is presently certified}: the
total-renewal coefficient
$\widetilde\eta = \eta + \eta_{\mathrm{int}} + \eta_R$
requires the route coefficients $\eta_{\mathrm{int}}, \eta_R$,
supplied only by the open renewal-control route
(Obl.~\ref{ob:NS-common-state-bridge}, part~(e)) --- the
controlling open burden. The former display
$\kappa \leq 1-8/343 \approx 0.977$ used the boundary-only
$\eta$ as a total coefficient and is superseded (MIG-052). The
remaining burdens for NS.6.1 (full scope) are that controlling
route together with the global promotion from the safe-tail window
to the full NS regime: the window-by-window contraction must extend
uniformly across all windows in the NS architecture, including
uniformity of the route data, not just the minimal safe-tail case.
\end{remark}
...
This section verifies the \emph{geometric} window-uniformity
hypothesis required by Theorem~\ref{thm:NS61-global} \emph{under
the uniform window-boundary hypothesis} (\textsc{uwb},
Corrective~I: $|\partial W_k| \leq K_* < \infty$ on the late
tail, explicitly assumed --- the scale-adaptive window family of
Definition~\ref{def:NS-window} bounds no $|\partial W_k|$ by
itself): under \textsc{uwb}, the NS safe-tail window geometry is
eventually periodic and the coefficient bounds ($c_1, \eta$;
$c_2 \geq K_0^{-K_*}$) are uniform across all sufficiently late
windows.

\begin{theorem}[\status{C}]\label{thm:NS-window-uniformity}
\textup{(NS Window Finite-State and Isometry Classification ---
reconditioned MIG-052, Corrective~I.)}
\textup{[Conditional on $K_0=7$, Phase-A, LS-2, the NS active
architecture declaration, \emph{and the uniform window-boundary
hypothesis} (\textsc{uwb}, Corrective~I): there exists
$K_* < \infty$ with $|\partial W_k| \leq K_*$ for all
sufficiently late windows $W_k$. \textsc{uwb} is an explicit
additional hypothesis, \emph{explicitly assumed, not proved}: the
active window family of Definition~\ref{def:NS-window} is
scale-adaptive --- window size adapts to the local contraction
geometry --- and current canon bounds no $|\partial W_k|$;
finiteness of each individual window does not make
$\bigcup_k \Sigma_\partial^{\,|\partial W_k|}$ finite.]}
(i)~Under \textsc{uwb}, the late-tail NS safe-tail window
configurations take values in a finite state space: each window
with $k$ boundary sites admits at most $K_0^{k}$ distinct canonical
collar-labelled configurations, and under
$|\partial W_k| \leq K_*$ the total configuration-state space is
bounded by $\sum_{j \leq K_*} K_0^{j} < \infty$. \emph{Without
\textsc{uwb} no globally finite state space is asserted.}
(ii)~Any two safe-tail windows with the same canonical
collar-labelled configuration are isometric under the canonical
collar-type labelling. \emph{This theorem does not assert eventual
periodicity}: finiteness alone yields recurrence, not periodicity;
eventual periodicity is derived in the eventual-periodicity
corollary (\texttt{cor:NS-eventual-periodicity}) from this
finite-state result together with the deterministic-transition
theorem (\texttt{thm:NS-window-determinism}) --- textual labels,
not dependencies: those results consume this one, not conversely
(Corrective~I).
\end{theorem}

\begin{proof}
\textbf{Step 1 (Finite collar-type state space, under
\textsc{uwb}).}
By Book~II LS-2: the stabilized collar type alphabet
$\Sigma_\partial$ is finite with $|\Sigma_\partial| = K_0 = 7$.
Each window $W_k$ with $|\partial W_k| = j$ boundary sites is
described by a configuration of collar types on those sites, so it
admits at most $K_0^{j}$ distinct configurations. \emph{This
per-window finiteness alone does not bound the global state space}:
the window family is scale-adaptive
(Definition~\ref{def:NS-window}), and if $|\partial W_k|$ is
unbounded then $\bigcup_k \Sigma_\partial^{\,|\partial W_k|}$ is
infinite (Corrective~I). Under the \textsc{uwb} hypothesis
$|\partial W_k| \leq K_*$ of the bracket, the total late-tail
configuration-state space is bounded by
$\sum_{j \leq K_*} K_0^{j} < \infty$. This proves (i). (\emph{Note, MIG-052:} the former Step~2 of this
proof invoked the Pigeonhole Principle to conclude eventual
periodicity; pigeonhole on a finite set yields only recurrence of
some value, not $w_{n+T} = w_n$ for all late $n$. That step is
withdrawn; eventual periodicity is \emph{not} concluded here.)

\textbf{Step 2 (Isometry classification).}
Under Phase-A transitivity ($K_0 = 7$ collar walk visits all
symbols), the collar-type equivalence relation on the boundary is
canonical, so two windows with the same collar-type configuration
are isometric under the canonical collar-type labelling. This
proves (ii). \qed
\end{proof}

\begin{corollary}[\status{C}]\label{cor:window-uniform-coeffs}
\textup{(Uniform Coefficient Bounds Across Windows.)}
Under the finite-state/isometry classification of
Theorem~\ref{thm:NS-window-uniformity} \emph{and the eventual
periodicity of Cor.~\ref{cor:NS-eventual-periodicity}} (which
supplies the periodicity consumed here; finiteness alone would
not, and on the \textsc{uwb} uniform window-boundary hypothesis
those results carry --- Corrective~I): the coefficients
$c_1 \geq 1/7$ and $\eta \leq 6/7$ (both independent of the
window boundary size) hold uniformly across all sufficiently late
windows, and the mixing coefficient satisfies the uniform positive
lower bound $c_2 \geq c_{2,*} = K_0^{-K_*} > 0$ under
\textsc{uwb}. \emph{No global $c_2 \geq 1/49$ is asserted}: the
per-window bound $K_0^{-k}$ decreases with $k$, and $1/49$ is its
$k = 2$ (minimal-window) instance, valid globally only were
$k \leq 2$ uniform --- which is not proved (Corrective~I). \emph{This certifies those
three quantities only}: no total-renewal ratio
$\widetilde\kappa$ is inferred here, because uniformity of the
remaining route data ($\eta_{\mathrm{int}}, \eta_R$ of the
hypothetical renewal-control discharge) is not supplied by window
geometry. The \emph{geometric} part of the window-uniformity
hypothesis of Theorem~\ref{thm:NS61-global} is verified; its
route part remains hypothetical (MIG-052).
\end{corollary}

\begin{proof}
By Cor.~\ref{cor:NS-eventual-periodicity} (finite state space of
Thm.~\ref{thm:NS-window-uniformity}(i) plus the deterministic
transition of Thm.~\ref{thm:NS-window-determinism}): the window
sequence is eventually periodic, so all sufficiently late windows
belong to the finitely many isometry classes along the cycle
(Thm.~\ref{thm:NS-window-uniformity}(ii)). The bounds
$c_1 \geq 1/7$ and $\eta \leq 6/7$ depend only on $K_0$
(Propositions~\ref{prop:NS-quantitative-c1}
and~\ref{prop:NS-quantitative-eta}) and are therefore uniform.
The mixing bound is per-window:
$c_2 \geq K_0^{-k}$ for a window with $k$ boundary sites
(Prop.~\ref{prop:NS-quantitative-c2}), and $K_0^{-k}$
\emph{decreases} with $k$; under \textsc{uwb} every late window
has $k \leq K_*$, so along the finitely many cycle classes
$c_2 \geq K_0^{-K_*} = c_{2,*} > 0$ uniformly (the worst class
governs). Hence the stated bounds are uniform. \qed
\end{proof}

\begin{remark}[\status{R}]
\textbf{NS.6.1 geometric uniformity condition closed; route
condition open (MIG-052).}
The \emph{geometric} window-uniformity hypothesis of
Theorem~\ref{thm:NS61-global} is verified by
Theorem~\ref{thm:NS-window-uniformity} and
Corollary~\ref{cor:window-uniform-coeffs} \emph{under the
\textsc{uwb} uniform window-boundary hypothesis those results now
carry} (Corrective~I; \textsc{uwb} is explicitly assumed, not
proved). The NS.6.1 global contraction theorem is conditional on
Phase-A, $K_0=7$, LS-2, the NS architecture declaration,
\textsc{uwb}, \emph{and} the hypothetical renewal-control and
ledger-state discharge with uniform route data
(Obl.~\ref{ob:NS-common-state-bridge}, parts~(e) and~(f)); beyond
\textsc{uwb} no additional external \emph{geometric} uniformity
...
Phase-A, $K_0=7$, LS-2, PASS (derived), the NS architecture
declaration, hypothetical discharge of the open common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}), and the separate unproved
\textsc{uwb} scope hypothesis. Per MIG-052, the terminal step holds
at CERT-PROJ force only while the common-state obligation is open;
without \textsc{uwb} the present global-window route is unavailable.
\end{remark}

\begin{remark}[\status{R}]\label{rem:NS-window-Q2a-hardening}
\textbf{Micro-hardening note: Deterministic Finite Transition Lemma.}
The former pigeonhole argument of
Thm.~\ref{thm:NS-window-uniformity} (withdrawn --- MIG-052) showed
only that the configuration sequence, \emph{granted the finite
state space now conditional on \textsc{uwb} (Corrective~I)}, must
repeat some state --- recurrence, not periodicity. For the conclusion of
...
Thm.~\ref{thm:NS-window-uniformity} --- in particular on the
\textsc{uwb} uniform window-boundary hypothesis carried by the
latter (Corrective~I); without \textsc{uwb} no finite global state
space, and hence no eventual periodicity, is available.]}
The safe-tail window configuration sequence is eventually periodic:
finite state space (under \textsc{uwb}) plus deterministic
transition implies eventual cycle.
\end{corollary}

\begin{proof}
By Thm.~\ref{thm:NS-window-uniformity}(i), the sequence takes
values in a finite set (finiteness alone would yield only
recurrence). By Thm.~\ref{thm:NS-window-determinism}, the
transition function $\sigma$ is well-defined and deterministic. A
deterministic map on a finite set must eventually enter a cycle:
once any configuration repeats, determinism forces the entire
subsequent segment to repeat. Hence the sequence is eventually
periodic, and the isometry classification of
Thm.~\ref{thm:NS-window-uniformity}(ii) applies along the cycle.
\qed
\end{proof}
...
in its reconditioned finite-state/isometry form (finite state
space \emph{under the \textsc{uwb} uniform window-boundary
hypothesis} --- Corrective~I), deterministic transition
(Thm.~\ref{thm:NS-window-determinism}), eventual cycle
(Cor.~\ref{cor:NS-eventual-periodicity}), uniform
boundary/geometry coefficient bounds
(Cor.~\ref{cor:window-uniform-coeffs}). The global NS.6.1
contraction remains \emph{route-conditional}: it consumes the
hypothetical common-state discharge
(Obl.~\ref{ob:NS-common-state-bridge}) and stands at CERT-PROJ
force while that burden is open (MIG-052).
\end{remark}
...
Section~\ref{sec:ns61-partial} established the
\emph{route-conditional, symbolic} contraction
$\widetilde\kappa \in (0,1)$ on the minimal safe-tail window ---
conditional on the hypothetical common-state discharge
(Obl.~\ref{ob:NS-common-state-bridge}); the certified quantities
are the boundary/geometry bounds $c_1, c_2, \eta$ only, and no
numerical total ratio is certified (MIG-052). This section
promotes that conditional contraction to the full NS regime on the
same hypothetical footing, under the additional \textsc{uwb}
uniform window-boundary hypothesis (Corrective~I) that makes the
window state space finite.

\begin{theorem}[\status{C}]\label{thm:NS61-global}
\textup{(NS.6.1 Full Scope: Uniform Window-to-Window Contraction.)}
\textup{[Conditional on the symbolic route-conditional ratio of
Theorem~\ref{thm:NS-kappa-explicit} --- hence on the hypothetical
discharge of the renewal-control and ledger-state burdens
(Obl.~\ref{ob:NS-common-state-bridge}, parts~(e) and~(f)) with the
route data $\eta_{\mathrm{int}}, \eta_R$ \emph{uniform across
the declared windows} and the contraction factor furnished in the
burden-(f) positive domain --- the \textsc{uwb} uniform
window-boundary hypothesis ($|\partial W_k| \leq K_*$,
Corrective~I), and the NS window-uniformity hypothesis: the window
geometry and the safe-tail active architecture are uniform across
all sufficiently late windows. Uniformity of $c_1, c_2, \eta$
...
By Cor.~\ref{cor:NS-eventual-periodicity} (the finite
configuration-state space of
Thm.~\ref{thm:NS-window-uniformity}(i) --- available under the
\textsc{uwb} hypothesis of the bracket, Corrective~I --- together
with the deterministic transition of
Thm.~\ref{thm:NS-window-determinism}), the active window structure
on the NS safe tail is eventually periodic: for all sufficiently
large $n$, the $n$-th window is isometric to the $(n+T)$-th for
some fixed period $T \geq 1$
(Thm.~\ref{thm:NS-window-uniformity}(ii)). This gives uniform
geometry across windows.
...
\begin{itemize}
 ...
 \item $c_2 \geq c_{2,*} = K_0^{-K_*} > 0$ holds uniformly
 (Proposition~\ref{prop:NS-quantitative-c2} with the
 \textsc{uwb} bound $k \leq K_*$, Corrective~I): the per-window
 bound $K_0^{-k}$ \emph{decreases} with $k$, so the uniform
 bound is governed by the largest boundary size along the cycle;
 no global $1/49$ is asserted.
...
The uniform boundary/geometry bounds, together with the
hypothesised uniform route data $\eta_{\mathrm{int}}, \eta_R$
supplied by the renewal-control discharge
(Obl.~\ref{ob:NS-common-state-bridge}, part~(e)),
give the same symbolic contraction ratio
$\widetilde\kappa = 1 - (1-\widetilde\eta)(c_1+c_2)
\in (0,1)$ at every sufficiently late window
($\widetilde\eta = \eta + \eta_{\mathrm{int}} + \eta_R < 1$;
positivity from the burden-(f) contraction domain; no numerical
value is certified while the route is open; in the extinction case
$(1-\widetilde\eta)(c_1+c_2) \geq 1$ nonnegativity forces
$\mathfrak{D} = 0$ in finitely many steps and the conclusion holds
trivially). In the geometric case, by induction,
$\mathfrak{D}_n \leq
\widetilde\kappa^{\,n-n_0}\mathfrak{D}_{n_0} \to 0$ as
$n \to \infty$. Hence the contraction holds globally, at
CERT-PROJ force while the route is open. \qed
\end{proof}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 window geometry, under the \textsc{uwb} uniform
 window-boundary hypothesis $|\partial W_k| \leq K_*$ ---
 explicitly assumed, Corrective~I);
...
(Thm.~\ref{thm:NSP1c}), are conditional on hypotheses (1), (5), and
(6) --- in particular on the separate \textsc{uwb} scope hypothesis
and the open common-state bridge
Obl.~\ref{ob:NS-common-state-bridge} --- and the ledger ratio is
symbolic, route-conditional (Thm.~\ref{thm:NS-kappa-explicit},
reconditioned). The continuation criterion of
Theorem~\ref{thm:NS61} follows on the stated hypothesis set. \qed
\end{proof}
...
(Obl.~\ref{ob:NS-common-state-bridge}) with \textsc{idc} subsidiary,
and separately retains the unproved \textsc{uwb} uniform
window-boundary scope hypothesis; the former hypothesis $\alpha$ is
withdrawn (see Thm.~\ref{thm:NSL1}). The formal residuals are the
external unconditional Stage-3 question and the internal common-state
bridge; \textsc{uwb} remains outside the frontier counts.
\end{remark}
...
and~\ref{thm:NS-H-CODIM2} for the Strong-BA conclusion; any
composition with the global NS.6.1/NS.7.1 chain is additionally
conditional on hypothetical discharge of the common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}) and on the separate unproved
\textsc{uwb} uniform window-boundary scope hypothesis.]}
...
common-state bridge, Obl.~\ref{ob:NS-common-state-bridge}, and using
the global-window route only under \textsc{uwb}), the NS branch
achieves $u \in C^\infty(\mathbb{R}^3 \times (0,\infty))$
conditionally at status $\mathsf{Cond}(K_0=7)$, on that complete
hypothesis set.
\end{corollary}
...
(Thm.~\ref{thm:NS-H-CODIM2}) into the canonical NS Branch
completes the H-ladder inputs of the conditional Stage-3 chain.
\emph{This does not close NS Stage-3, even conditionally, on its
own (MIG-052):} the chain's transfer to the window obstruction ---
hence any Stage-3 reading --- passes through the controlling open
common-state burden (Obl.~\ref{ob:NS-common-state-bridge}), and
the separate unproved \textsc{uwb} scope hypothesis is required by
the global-window route. The branch remains at CERT-PROJ force with
the common-state frontier open. The
conditional chain is now:
\[
...
mass gap --- and, for the terminal composition with
NS.6.1/NS.7.1, on hypothetical discharge of the open common-state
bridge (Obl.~\ref{ob:NS-common-state-bridge}) together with the
separate unproved \textsc{uwb} scope hypothesis; the terminal token is
...
The NS branch posture is \emph{CERT-PROJ / open common-state
frontier, with \textsc{uwb} retained as a separate unproved
conditional scope hypothesis}. The branch is a lawful descendant of
the canonical spine and
...
--- hypothetical discharge of the bridge obligation; the separate,
explicit, unproved \textsc{uwb} uniform window-boundary scope
hypothesis; and the subsidiary,
...
force, nor without both the common-state discharge and \textsc{uwb}.
\end{proposition}
...
Obl.~\ref{ob:NS-common-state-bridge} + the separate unproved
\textsc{uwb} scope hypothesis; in the geometric case the ledger
contraction stands in its symbolic route-conditional form
$q=\widetilde\kappa = 1-(1-\widetilde\eta)(c_1+c_2)\in(0,1)$,
with the finite-extinction alternative treated separately --- the
former explicit $\kappa\le 1-8/343$ used the boundary-only
$\eta$ as a total coefficient and is superseded (MIG-052) --- its
transfer to the window obstruction being exactly the open
common-state bridge; $\alpha$ withdrawn; the former per-step
margin claim remains withdrawn).
Window determinism: Thm.~\ref{thm:NS-window-determinism}
(Q2a resolved); the finite state space it acts on --- hence
eventual periodicity --- is conditional on the \textsc{uwb}
uniform window-boundary hypothesis (Corrective~I).
...
the \emph{formal conditional} frontier, not an unconditional
barrier: under the stated Phase-A/$K_0=7$/PASS/LS-2 hypotheses
\emph{together with the hypothetical discharge of the controlling
common-state burden (Obl.~\ref{ob:NS-common-state-bridge}:
total-renewal control, same-state obstruction comparison,
ledger-state map with positive contraction domain) and the separate
unproved \textsc{uwb} scope hypothesis}, the branch
would reach its declared conditional endpoint; absent that
discharge it stands at CERT-PROJ force with the common-state
frontier open (MIG-052). Unconditional global regularity
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 obligation Obl.~\ref{ob:NS-common-state-bridge} with \textsc{idc}
 subsidiary, and separately including the unproved \textsc{uwb}
 uniform window-boundary scope hypothesis
 (MIG-052/Corrective~J; $\alpha$ withdrawn),
 \item a domain-bounded conditional endpoint claim at CERT-PROJ
 force at the declared scope (Prop.~\ref{prop:NS-status}),
 conditional on the same complete hypothesis set, including both the
 hypothetical common-state discharge and separate \textsc{uwb},
 ...
 intake.
 \textsc{uwb} is retained separately as an unproved conditional
 scope hypothesis and is not added to either frontier count.
\end{enumerate}
The NS branch may not claim unconditional endpoint closure,
unconditional global regularity, or CERT-CLOSE at any scope while
Obl.~\ref{ob:NS-common-state-bridge} is open; no global-window
endpoint may be claimed without \textsc{uwb}.
\end{corollary}
...
Item~(e) records the pre-canonical state in which
Theorem~\ref{thm:NS61} was pending; the path has no canonical force
and does not define the current frontier.
\end{remark}
...
\end{itemize}
The current canonical branch does not identify
Theorem~\ref{thm:NS61} as an~[O]: it is conditionally discharged at
CERT-PROJ force. The formal internal~[O] is the common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}); \textsc{uwb} is a separate
unproved scope hypothesis, and unconditional Stage-3 regularity is
...
\begin{enumerate}
 ...
 control to the NS endpoint target is completely specified as a
 conditional template at CERT-PROJ force.
 \item Ren.5, UH.3, and SB2 are conditionally discharged predecessor
 items. The current formal internal frontier is the common-state
 bridge (Obl.~\ref{ob:NS-common-state-bridge}); the variable-window
 global route separately retains the unproved \textsc{uwb} scope
 hypothesis.
...
\begin{enumerate}
 \item \emph{Discharge of the common-state bridge.} No current
 theorem supplies the typed ledger-to-obstruction state transfer,
 renewal control, same-state comparison, or ledger-state map required
 by Obligation~\ref{ob:NS-common-state-bridge}.
 \item \emph{Proof of \textsc{uwb}.} The uniform window-boundary
 bound is retained as a separate conditional scope hypothesis; it is
 neither proved nor registered as another~[O].
 \item \emph{Resolution of the NS Millennium Prize question.}
 Unconditional Stage-3 regularity remains external to the declared
 conditional scope.
...
\begin{remark}[\status{R}]
For the predecessor conditional theorem packages, see
Section~\ref{sec:NS-near-closure-modes}. For the current formal
frontier and separate \textsc{uwb} scope restriction, see
Definition~\ref{def:NS-frontier}, the current frontier note in
Section~\ref{sec:NS-conditional-package}, and the final status
proposition.
\end{remark}
...
conditional endpoint claim at CERT-PROJ force
(Prop.~\ref{prop:NS-status}; MIG-052), conditional on both the
hypothetical common-state discharge and the separate unproved
\textsc{uwb} scope hypothesis. The formal named residuals are the
\emph{internal} obstruction/ledger
...
nowhere in this book. Separately, \textsc{uwb} remains an unproved
non-obligation scope hypothesis required by the present global-window
template and is not included in either frontier count.
...
The pre-canonical program recorded derivation sketches for the
boundary-order and Stage-3 predecessor program. Those sketches have
no canonical force and do not define the present frontier. Their
import is reflected in the canonical body only at conditional force:
NS.6.1 is conditionally discharged
...
transfer to the window obstruction requires hypothetical discharge
of the open common-state bridge,
Obl.~\ref{ob:NS-common-state-bridge}, and the global-window route
separately requires the unproved \textsc{uwb} scope hypothesis).
Unconditional global regularity (NS Stage-3, the Clay Prize
question) is the external frontier; the common-state bridge is the
formal internal frontier; \textsc{uwb} remains outside the frontier
counts.
\end{remark}
