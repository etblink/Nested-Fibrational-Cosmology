...
target scope is Corollary~\ref{cor:NS61-discharged}. \qed
\end{proof}
...
requires the following micro-lemma, which is the named hardening item
(Q2a in the synthesis record):

\begin{quote}
\textbf{Lemma (NS Deterministic Window-Transition).}
In the NS active architecture, the next safe-tail window configuration
is determined by the current canonical collar-labelled window
configuration plus the declared finite transport/defect update rule.
\end{quote}

If this determinism holds, the proof is clean: finite state space
plus deterministic transition implies eventual cycle, hence eventual
periodicity, hence uniform coefficient bounds, hence global NS.6.1
contraction.  Without it, the pigeonhole argument gives recurrence
but not periodicity.  The NS active architecture declaration is the
expected source for determinism, but the explicit statement is not
yet promoted to canonical theorem form.  This is the one
proof-quality issue to harden before treating
Thm.~\ref{thm:NS-window-uniformity} as fully clean [C] without
qualification.
\end{remark}
