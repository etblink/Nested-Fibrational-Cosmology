...
bridge, so the discharge holds at CERT-PROJ force.) Given the hypothesis, the
...
\begin{enumerate}[label=\textup{(\alph*)}]
 ...
 burden.
 \item \emph{Ledger-state map (Route~B implementation --- the
 route the NS chain consumes).} The active chain distinguishes
...
only $c_1 \geq 1/7$, $c_2 \geq 1/49$, and the \emph{boundary}
coefficient $\eta \leq 6/7$; it certifies neither
...
(i)~The NS safe-tail window configurations take values in a finite
state space: the number of distinct canonical collar-labelled
window configurations is at most $K_0^{|\partial W|} < \infty$.
...
eventual periodicity is derived in
Cor.~\ref{cor:NS-eventual-periodicity} from this finite-state
result together with the deterministic transition of
Thm.~\ref{thm:NS-window-determinism}.
\end{theorem}
...
$|\partial W|$ is the number of boundary sites per window. This
proves (i). (\emph{Note, MIG-052:} the former Step~2 of this
...
not): the boundary/geometry coefficients $c_1 \geq 1/7$,
$c_2 \geq 1/49$, and $\eta \leq 6/7$ hold uniformly across all
sufficiently late windows (not just the minimal window). \emph{This certifies those
...
(Thm.~\ref{thm:NS-window-uniformity}(ii)). The
...
contraction theorem is conditional on Phase-A, $K_0=7$, LS-2, the
NS architecture declaration, \emph{and} the hypothetical
renewal-control discharge with uniform route data
(Obl.~\ref{ob:NS-common-state-bridge}, part~(e)); no additional
external \emph{geometric} uniformity hypothesis is required.
...
space, deterministic transition
...
renewal-control discharge (Obl.~\ref{ob:NS-common-state-bridge},
part~(e)) with its route data $\eta_{\mathrm{int}}, \eta_R$
\emph{uniform across the declared windows} --- and the NS
window-uniformity hypothesis: the window geometry and the safe-tail
active architecture are uniform across all sufficiently late
windows. Uniformity of $c_1, c_2, \eta$ alone is insufficient.]}
...
Thm.~\ref{thm:NS-window-uniformity}(i) together with the
deterministic transition of
