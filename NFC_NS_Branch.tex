...
the former hypothesis $\alpha$~\eqref{hyp:NS-alpha} is withdrawn.
...
\eqref{hyp:NS-alpha}. \emph{MIG-052 reconditioning (current):}
...
$\alpha$~\eqref{hyp:NS-alpha} is withdrawn --- MIG-052).
...
hypothesis $\alpha$~\eqref{hyp:NS-alpha} is withdrawn as ill-typed
...
subsequent common-state analysis established that $\alpha$ is false
as written (a stock/increment type mismatch --- withdrawn below),
...
and insufficient}: \eqref{hyp:NS-IDC-i} lacks a canonical object to
bound --- no $\Psi^{\mathrm{int}}$ exists in canon, NS-H3 routing
all renewal through the boundary/halo channel --- and
...
temporal state spaces, so the identity is false as written --- a
type mismatch, not a normalization gap. The equation and its label
are retained solely as the historical record of the withdrawn
MIG-050 intake hypothesis; $\alpha$ may not be cited, at any force,
anywhere in the corpus. The bridging role $\alpha$ was intended to
...
Exhibit a common contracting state carrying both the level-stock
window obstruction $\mathcal{O}_{\mathrm{NS}}$ and the contracting
increment ledger $\mathfrak{D}_k$, in one of the two well-posed
frames identified by the NS-0G common-state analysis:
\begin{enumerate}[label=\textup{(\roman*)}]
 \item \emph{State-vector frame:} a state
 $X_n = (I_n, \mathcal{E}_n, \mathcal{B}_n, \dots)$ with an
 exhibited transition operator $T_n$ (whose $I$-row is a proved
 evolution law for the interface burden, including a genuine
 boundary-loss channel), a weight vector $w \geq 0$ with
 $V(X) = \langle w, X\rangle$ such that $T_n$ has $V$-spectral
 radius $< 1$ on the late tail, and a two-sided comparison
 $\mathcal{O}_{\mathrm{NS}} \asymp V(X_n)$; or
 \item \emph{Discounted-memory frame:} a declared discount
 $\rho \in (0,1)$ tied to the loss structure (new data --- not
 sourced from canon), the discounted accumulation
 $\widehat{\mathfrak{B}}^{(\rho)}_n
 = \sum_{j<n} \rho^{\,n-1-j}\,\mathcal{B}_j$ with its closed
 recurrence
 $\widehat{\mathfrak{B}}^{(\rho)}_n
 = \rho\,\widehat{\mathfrak{B}}^{(\rho)}_{n-1}
 + \mathcal{B}_{n-1}$, and a proved comparison
...
at CERT-PROJ force only, and no NS closure claim may be cited at
CERT-CLOSE force.
\end{obligation}
...
$\alpha$~\eqref{hyp:NS-alpha} is withdrawn as ill-typed
(MIG-052).]}
Assume \textsc{idc} and the common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}). Set
...
there exist symbolic constants $0 < a \leq b$ (the two-sided
bridge-comparison constants furnished by
Obl.~\ref{ob:NS-common-state-bridge}; the former expression
...
that route --- MIG-052) and an integer
$m > \log(b/a)/\log(1/\widetilde\kappa)$ such that, whenever
$\mathcal{O}_{\mathrm{NS}}(W_k) \geq T_{\mathrm{NS}}$,
\[
 \mathcal{O}_{\mathrm{NS}}(W_{k+m})
 ...
 \mathcal{O}_{\mathrm{NS}}(W_k),
 \qquad
 \lambda_{\mathrm{NS}} := (b/a)\,\widetilde\kappa^{\,m} \;<\; 1.
\]
...
\mathfrak{D}_k$.
\emph{Bridge.} The former bridge step invoked
$\alpha$~\eqref{hyp:NS-alpha} to rewrite the obstruction as
...
$\alpha$ equates a level stock with a step increment (ill-typed),
and SCT.4 (Book~II) supplies no absolute ceiling. The two-sided
comparison $a\,\mathfrak{D}_k \leq
\mathcal{O}_{\mathrm{NS}}(W_k) \leq b\,\mathfrak{D}_k$ against
the \emph{same contracting} ledger --- or a closed contracting
recurrence for $\mathcal{O}_{\mathrm{NS}}$ itself --- is precisely
the content of the common-state bridge,
Obl.~\ref{ob:NS-common-state-bridge}, and is assumed here, not
derived.
\emph{Combination.} Over $m$ steps, using both sides of the
bridge comparison,
$\mathcal{O}(W_{k+m}) \leq b\,\mathfrak{D}_{k+m} \leq
b\,\widetilde\kappa^{\,m}\mathfrak{D}_k \leq
(b/a)\,\widetilde\kappa^{\,m}\,\mathcal{O}(W_k)$, and
...
$\lambda_{\mathrm{NS}} = (b/a)\,\widetilde\kappa^{\,m} < 1$
under \textsc{idc} and the common-state bridge
(Obl.~\ref{ob:NS-common-state-bridge}). The former derivation via
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 obstruction/ledger alignment \eqref{hyp:NS-alpha} (MIG-050) ---
...
$\alpha$~\eqref{hyp:NS-alpha} (MIG-050) is withdrawn. The former
...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 $\alpha$~\eqref{hyp:NS-alpha} retained only as historical
 intake.
\end{enumerate}
The NS branch may not claim unconditional endpoint closure,
