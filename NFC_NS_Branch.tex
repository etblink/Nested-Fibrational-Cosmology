...
\emph{Current posture:} Domain-bounded conditional CERT-CLOSE reached (NS.6.1 and NS.7.1 conditionally discharged); unconditional global regularity (the Clay Stage-3 question) remains the external frontier.

\emph{Authoritative source:} \ref{prop:NS-status}~(final status proposition (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 0; reduced irreducible frontier --- none (unconditional global regularity is external, tracked as the NS frontier theorem); branch posture --- domain-bounded conditional CERT-CLOSE.}
...
branch has reached domain-bounded conditional CERT-CLOSE
(Prop.~\ref{prop:NS-status}). Unconditional global regularity (NS
Stage-3, the Clay Prize question) remains the external frontier.
\end{remark}
