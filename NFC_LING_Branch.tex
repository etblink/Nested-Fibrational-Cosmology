...
\begin{openobligation}[\status{O}]\label{ob:ling-rec-bridge-eps}
\textup{(O-LING.REC-Bridge-$\varepsilon$: Recursion Loss Convergence.)}
The minimal remaining recursion frontier is: prove that
$\varepsilon_n^{\mathrm{rec}} \to 0$ along the declared assembly
depth in the recursion-interface regime $\mathcal{R}_{\mathrm{LingRec}}$.

\textbf{NS finite-state insight.} The NS branch
(Thm.~\ref{thm:NS-window-determinism}) showed that a finite-state
deterministic transition function on a finite state space gives
eventual periodicity and uniform bounds. The LING analogue: if
the contrast-class and compositional-ledger state space is finite
at each certified depth level (which follows from the SCC-carrier
minimality of the recursion invariant), and the depth transition
is deterministic (by the no-hidden-channel structure), then the
recursion domain loss sequence satisfies a geometric decay bound,
giving $\varepsilon_n^{\mathrm{rec}} \to 0$ automatically.
The gap: whether the LING recursion-interface regime admits
a norm on the contrast/composition state space under which
this geometric decay holds. This is the one remaining
named condition.
\end{openobligation}
