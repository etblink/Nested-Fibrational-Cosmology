...
%% LING Branch Book: Linguistics Branch (Constituted; see Current Status Capsule)
%% Current status: Contrast+recursion+context-response closed; reference semantics conditional at two scopes. See rem:ling-two-status / cap:LING-status.
...
% ============================================================
\begin{remark}[\status{R}]\label{cap:LING-status}
\textbf{Current Status Capsule.}
\emph{Current posture:} Contrast, recursion, and context-response structure closed; reference semantics conditionally resolved at two scopes (base invisibility / nominated recovery extension).

\emph{Authoritative source:} \ref{rem:ling-two-status}~(two-status reading (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 0; reduced irreducible frontier --- none; recursion-to-infinity is carried at bridge force~[B] (\texttt{thm:ling-rec-infty-bridge}); branch posture --- contrast+recursion+context-response closed.}
...
This branch asks whether linguistic structure --- phonological
contrast, syntactic compositionality, semantic equivalence --- can be
realized as a lawful descendant of the NFC source architecture. The
aim is not to import the standard language of phonemes, grammars,
lexicons, or meaning-bearing expressions as primitives, but to
...
This posture is mandated by Book~I's no-smuggling stance, Book~V's
branch-legitimacy law, Book~VI's continuum-interface discipline, and
Book~VII's scoped-certificate law.

The central question is: starting from finite signal-object pairs, a
finite admissible contrast discipline, and the observational equivalence
imposed by that discipline, can one recover certified hierarchical
compositional structure? No alphabet, phoneme inventory, syntactic
category system, or semantic domain is assumed. Each such notion, if it appears at all,
...
\begin{obligation}[\status{C}]\label{ob:ling-CR}
...
\begin{obligation}[\status{C}]\label{ob:ling-CL}
...
\begin{obligation}[\status{C}]\label{ob:ling-CM}
...
\begin{obligation}[\status{C}]\label{ob:ling-NC}
...
\begin{obligation}[\status{C}]\label{ob:ling-GD}
...
\begin{obligation}[\status{C}]\label{ob:ling-REC}
...
\begin{obligation}[\status{C}]\label{ob:ling-SEM}
...
\begin{obligation}[\status{C}]\label{ob:ling-END}
...
\textup{(Contrast Realization --- O-LING.CR Conditionally Discharged.)}
...
\textup{(Compositional Ledger --- O-LING.CL Conditionally Discharged.)}
...
\textup{(Contrast Margin --- O-LING.CM Conditionally Discharged.)}
\textup{[Conditional on: O-LING.CR discharged
...
(Def.~\ref{def:ling-contrast-margin}). The non-directly-separating
...
contrast probe family is declared to be non-directly separating. The
...
\textup{(no-hidden Compositional Channel --- O-LING.NC Conditionally Discharged.)}
...
\textup{(Grammar Descent --- O-LING.GD Conditionally Discharged.)}
...
\textup{(Linguistics Endpoint --- O-LING.END Conditionally Discharged.)}
...
\begin{obligation}[\status{C}]\label{ob:ling-REC-open}
\textup{(O-LING.REC: Recursion --- SCC-carrier and finite generativity discharged; bridge [O].)}
...
\begin{obligation}[\status{C}]\label{ob:ling-SEM-open}
\textup{(O-LING.SEM: Semantic Grounding --- Context-response [C]; reference and truth [O].)}
...
\begin{obligation}[\status{C}]\label{ob:ling-rec-infty}
\textup{(O-LING.REC$_\infty$: Strong Infinite Generativity --- fam+noncollapse [C]; bridge [O].)}
...
\begin{obligation}[\status{C}]\label{ob:ling-rec-infty-bridge}
\textup{(O-LING.REC$_{\mathrm{bridge}}$: Asymptotic Coherence.)}
\textup{[Conditional on Thm.~\ref{thm:ling-rec-bridge-template} below.]}
The asymptotic coherence bridge reduces to a single named
convergence condition.
\end{obligation}

\begin{definition}[\status{D}]\label{def:ling-recursion-domain-loss}
\textup{(Recursion Domain Loss.)}
For a noncollapsing certified iterated assembly family
$(X_n)_{n\ge0}$ in regime $\mathcal{R}_{\mathrm{LingRec}}$,
the \emph{recursion domain loss} at depth $n$ is
\[
 \varepsilon_n^{\mathrm{rec}} :=
 \sup_{P \in \mathcal{P}_{\mathrm{rec}}}\;
 \bigl|
 \mathcal{R}_{\mathrm{finite},n}(P)
 - \mathcal{R}_{\mathrm{cts},n}(P)
 \bigr|,
\]
where $\mathcal{R}_{\mathrm{finite},n}$ is the finite-stage
contrast/composition Rayleigh quotient at depth $n$ and
$\mathcal{R}_{\mathrm{cts},n}$ is the continuous-depth surrogate
from the declared recursion-interface regime.
\end{definition}

\begin{theorem}[\status{C}]\label{thm:ling-rec-bridge-template}
\textup{(Recursion Bridge Template.)}
\textup{[Conditional on: Thm.~\ref{thm:ling-rec-noncollapse-criterion}
(noncollapsing family has unbounded finite generativity) and
$\varepsilon_n^{\mathrm{rec}} \to 0$ along the declared assembly
depth.]}
If the recursion domain loss vanishes, then the noncollapsing
certified iterated assembly family satisfies Book~VI asymptotic
coherence in $\mathcal{R}_{\mathrm{LingRec}}$, and the
infinite-generativity object is licensed as effective shorthand.
\end{theorem}

\begin{proof}
By Thm.~\ref{thm:ling-rec-noncollapse-criterion}, the finite-stage
family has unbounded generativity. The recursion domain loss
$\varepsilon_n^{\mathrm{rec}}$ measures the finite-to-continuous
error in the assembly Rayleigh quotient. The proof follows the
B2-DomainNoLoss and NS-bridge patterns: $\mathcal{R}_{\mathrm{cts},n}
\ge \mathcal{R}_{\mathrm{finite},n} - \varepsilon_n^{\mathrm{rec}}$;
taking depth limits with $\varepsilon_n^{\mathrm{rec}} \to 0$ gives
preservation of the generativity predicate in the infinite-depth
limit. Book~VI licenses the continuous-depth surrogate as effective
shorthand for the certified finite-depth family.
\qed
\end{proof}

\begin{obligation}[\status{C}]\label{ob:ling-rec-bridge-eps}
\textup{(O-LING.REC-Bridge-$\varepsilon$: Recursion Loss Convergence ---
Conditionally Discharged.)}
\textup{[Conditional on: the SCC-carrier minimality of the recursion
invariant (O-LING.REC$_{\mathrm{SCC}}$ [C]); the positive contrast
margin $\Delta_{\mathrm{Ling}} > 0$; and the contrast-class state
space being finite at each certified depth level (Book~II finite
collar-type alphabet).]}
The recursion domain loss satisfies
$\varepsilon_n^{\mathrm{rec}} \to 0$ at geometric rate
$O(e^{-\Delta_{\mathrm{Ling}} \cdot n})$.
\end{obligation}

\begin{theorem}[\status{C}]\label{thm:ling-rec-bridge-eps-discharged}
\textup{(O-LING.REC-Bridge-$\varepsilon$ Conditionally Discharged.)}
\textup{[Conditional on ob:ling-rec-bridge-eps hypotheses.]}
The recursion domain loss satisfies
$\varepsilon_n^{\mathrm{rec}} \to 0$ geometrically.
\end{theorem}

\begin{proof}
The proof follows the CRYST EWALD convergence pattern
(\texttt{thm:cryst-ewald-eps}) and NS finite-state argument
(\texttt{thm:NS-window-determinism}).
The contrast-class and compositional-ledger state space is finite
at each certified depth level (Book~II finite collar-type alphabet
applied to contrast types).  The depth transition is deterministic
by the SCC-carrier minimality of the recursion invariant
(O-LING.REC$_{\mathrm{SCC}}$ [C]): the minimal faithful SCC
carrier uniquely selects the next assembly step.  By eventual
periodicity of the state sequence (\texttt{cor:NS-eventual-periodicity}
pattern), the recursion domain loss is bounded by the depth-$n$
tail of the Book~III depth-sum:
\[
  \varepsilon_n^{\mathrm{rec}}
  \;\le\;
  \sum_{d > n} |\partial_d \mathcal{A}_n|\,
  e^{-\Delta_{\mathrm{Ling}} \cdot d}
  \;\le\;
  C \cdot e^{-\Delta_{\mathrm{Ling}} \cdot n}.
\]
Since $\Delta_{\mathrm{Ling}} > 0$ (the positive contrast margin
plays the role of the Bragg gap and BIO fidelity margin in the
parallel arguments), the tail decays geometrically.
\qed
\end{proof}

\begin{remark}[\status{R}]\label{rem:ling-REC-full-status}
\textbf{Full recursion status after sub-split.}
The LING recursion picture is now four-tiered:
\begin{enumerate}[label=\textup{(\alph*)}]
 \item \emph{SCC-carrier recursion} (O-LING.REC$_{\mathrm{SCC}}$):
 conditionally discharged --- unique minimal SCC carrier exists
 (O-SCC.MCS now discharged, Session~XLII); every finite stage
 factors through it.
 \item \emph{Noncollapsing finite generativity}
 (O-LING.REC$_{\mathrm{fam}}$ + O-LING.REC$_{\mathrm{noncollapse}}$):
 conditionally discharged --- certified finite iterated assembly
 is lawful; noncollapse implies unbounded finite novelty.
 \item \emph{Bridge template}
 (O-LING.REC$_{\mathrm{bridge}}$): conditionally reduced ---
 asymptotic coherence follows from $\varepsilon_n^{\mathrm{rec}}
 \to 0$ (Thm.~\ref{thm:ling-rec-bridge-template}).
 \item \emph{Loss convergence}
 (O-LING.REC-Bridge-$\varepsilon$): the single remaining
 named condition. The NS finite-state determinism pattern
 suggests it may follow from the SCC-carrier minimality of the
 recursion invariant plus a declared comparison norm on the
 assembly state space.
\end{enumerate}
\end{remark}
...
%% MSC MINIMAL CONTEXT FAMILY CANONICALIZATION
\begin{definition}[\status{D}]\label{def:ling-context-family-role}
\textup{(Context Family Role Equivalence.)}
Two context-frame families $\mathcal{F}_1$, $\mathcal{F}_2
\subseteq \mathcal{P}_{\mathrm{sem}}^{\mathrm{ctx}}$ are
\emph{role-equivalent for the semantic predicate} when:
(1) they induce the same semantic equivalence classes:
$[X]_{\sim_{\mathrm{sem}}}^{\mathcal{F}_1} =
[X]_{\sim_{\mathrm{sem}}}^{\mathcal{F}_2}$ for all $X$;
(2) they produce the same semantic congruence for declared
composition; and (3) they differ only in the number of declared
context-frame probe episodes invoked (their \emph{context complexity}).
\end{definition}

\begin{theorem}[\status{C}]\label{thm:ling-sem-MSC-context}
\textup{(MSC Minimal Context Family.)}
\textup{[Conditional on Thm.~\ref{thm:ling-sem-context-grounding}
and role-equivalence of context-frame families with the same
semantic power.]}
Among all role-equivalent context-frame families inducing the
same semantic equivalence classes, MSC/minimal-support discipline
selects the unique family with minimal context complexity: the
\emph{canonical minimal context family}
$\mathcal{F}_{\min} \subseteq \mathcal{P}_{\mathrm{sem}}^{\mathrm{ctx}}$.
\end{theorem}

\begin{proof}
Define the \emph{context carrier} of a family $\mathcal{F}$ as
the minimal support in the compositional ledger needed to represent
all probe episodes in $\mathcal{F}$. Two role-equivalent families
have the same semantic power by definition. Among them, the family
with minimal context carrier is selected by MSC: any larger family
contains redundant context-probe episodes that are non-load-bearing
for the semantic equivalence predicate. By the no-hidden-channel
standing rule (Sr.~\ref{sr:ling-no-hidden-probe}), removing
non-load-bearing episodes preserves the semantic predicate.
The minimal-support selection gives $\mathcal{F}_{\min}$ uniquely
up to declared context equivalence.
\qed
\end{proof}

\begin{remark}[\status{R}]\label{rem:ling-minimal-context-note}
\textbf{Canonical minimal context.}
Thm.~\ref{thm:ling-sem-MSC-context} upgrades the semantic grounding
from ``conditional on a declared nondegenerate context-response
regime'' to ``canonical minimal context family selected by MSC.''
This removes the arbitrariness in regime declaration: among all
context-frame families with the same semantic power, the canonical
one is uniquely selected by the minimal-support principle. This is
the LING analogue of the YM B3-HRSelect move: just as MSC selected
the highest-root $A_1\subset A_2$ embedding (minimal leakage support
among role-equivalent embeddings), MSC here selects the
minimal-complexity context family among semantically equivalent ones.
\end{remark}


\begin{obligation}[\status{C}]\label{ob:ling-SEM-ref-open}
\textup{(O-LING.SEM$_{\mathrm{ref}}$: Reference-Bearing Probes ---
Formally Structured.)}
\textup{[Conditional on Thm.~\ref{thm:ling-sem-ref-template} below.]}
The reference-bearing upgrade reduces to a two-route predicate
analogous to the CRYST phase problem.
\end{obligation}

\begin{definition}[\status{D}]\label{def:ling-ref-predicate}
\textup{(Reference-Bearing Predicate.)}
The \emph{reference-bearing predicate} holds for a LING probe
discipline $\mathcal{P}_{\mathrm{Ling}}$ and assembled LING object
$X$ when one of the following two sub-predicates holds:
\begin{enumerate}[label=\textup{(\Alph*)}]
  \item \emph{REF-INVIS}: The reference relation
    $\mathrm{ref}(X) \in \mathcal{C}_{\mathrm{obs}}$ is
    quotient-invisible under $\mathcal{P}_{\mathrm{Ling}}$:
    no admissible probe in $\mathcal{P}_{\mathrm{Ling}}$ can
    distinguish two LING objects that share the same
    context-response profile but refer to different certified
    observable objects.

  \item \emph{REF-RECOV}: There exists a lawful extended probe
    family $\mathcal{P}'_{\mathrm{Ling}} \supset
    \mathcal{P}_{\mathrm{Ling}}$ --- sourced through $\mathbf{U}$,
    introducing no hidden supplement --- under which the reference
    relation $\mathrm{ref}(X)$ is quotient-visible: two LING
    objects with the same $\mathcal{P}'_{\mathrm{Ling}}$ profile
    have the same reference.
\end{enumerate}
\end{definition}

\begin{theorem}[\status{C}]\label{thm:ling-sem-ref-template}
\textup{(Reference-Bearing Upgrade Template.)}
\textup{[Conditional on the context-response semantics being
certified (Thm.~\ref{thm:ling-sem-context-grounding}); and one of
REF-INVIS or REF-RECOV holding for the declared
$\mathcal{P}_{\mathrm{Ling}}$.]}
If REF-INVIS holds: context-response semantics is the maximum
quotient-visible semantic structure under $\mathcal{P}_{\mathrm{Ling}}$
--- reference-bearing semantics requires a strictly richer probe family.
If REF-RECOV holds: the extended probe family
$\mathcal{P}'_{\mathrm{Ling}}$ upgrades context-response semantics
to reference-bearing semantics, and the MSC selects the unique
minimal faithful reference-probe family among role-equivalent
extensions.
\end{theorem}

\begin{proof}
The proof follows the CRYST phase problem template
(\texttt{thm:cryst-phase-conditional}).
REF-INVIS: by the observational quotient discipline (Book~I),
if the reference relation is not quotient-visible under
$\mathcal{P}_{\mathrm{Ling}}$, then two LING objects with the
same context-response profile but different references are in the
same semantic equivalence class --- reference cannot be derived
from context-response data alone.  The Phase Information Bound
analogue (Book~II boundary capacity law applied to the probe's
information capacity) gives the necessary condition.

REF-RECOV: if $\mathcal{P}'_{\mathrm{Ling}}$ is lawful and makes
the reference quotient-visible, then the MSC applied to the
reference-probe family (the set of all lawful extensions
$\mathcal{P}' \supseteq \mathcal{P}_{\mathrm{Ling}}$ that make
reference quotient-visible) selects the unique minimal faithful
extension by the Zorn/finite-stabilization argument
(\texttt{thm:YM-MSC-existence} pattern).
\qed
\end{proof}

\begin{definition}[\status{D}]\label{def:ling-probe-factorization}
\textup{(Internal-Factorization Property.)}
The declared probe discipline $\mathcal{P}_{\mathrm{Ling}}$ has the
\emph{internal-factorization property} when every admissible probe
outcome functional factors through the LING-internal data of the
probed object --- its contrast classes, compositional ledger,
recursion structure, and context-frame responses --- and takes no
input channel from the certified observable class
$\mathcal{C}_{\mathrm{obs}}$.  Equivalently: probe outcomes are
constant on reference fibers, depending on $X$ only through data
that does not mention $\mathrm{ref}(X)$.  This is the formal
content of ``contrast-only'': the discipline interrogates the LING
object, never the objects it refers to.
\end{definition}

\begin{theorem}[\status{C}]\label{thm:ling-REF-INVIS}
\textup{(REF-INVIS for the Contrast-Only Discipline.)}
\textup{[Conditional on: context-response semantics certified
(Thm.~\ref{thm:ling-sem-context-grounding}); the declared
$\mathcal{P}_{\mathrm{Ling}}$ having the internal-factorization
property (Def.~\ref{def:ling-probe-factorization}); and
$\mathcal{C}_{\mathrm{obs}}$ admitting a nontrivial admissible
reassignment (at least two role-equivalent certified observable
objects).]}
REF-INVIS holds for the declared discipline: the reference
relation $\mathrm{ref}(X)$ is quotient-invisible under
$\mathcal{P}_{\mathrm{Ling}}$.
\end{theorem}

\begin{proof}
The proof mirrors the centrosymmetric phase-invisibility argument
(\texttt{thm:cryst-centro-PHASEINVIS}): exhibit a transformation
that preserves all probe-visible data while altering the hidden
quantity.  Let $\sigma$ be a nontrivial admissible reassignment of
$\mathcal{C}_{\mathrm{obs}}$ exchanging role-equivalent certified
objects, and for an assembled LING object $X$ define $X^\sigma$ to
be the object with identical LING-internal data and reference
relation $\mathrm{ref}(X^\sigma) = \sigma \circ \mathrm{ref}(X)$.
By the internal-factorization property, every admissible probe
outcome depends on its argument only through LING-internal data;
since $X$ and $X^\sigma$ share that data exactly, $P(X) =
P(X^\sigma)$ for every probe $P \in \mathcal{P}_{\mathrm{Ling}}$.
Hence $X$ and $X^\sigma$ lie in the same observational quotient
class.  By nontriviality of $\sigma$ on the relevant
role-equivalent pair, $\mathrm{ref}(X^\sigma) \neq
\mathrm{ref}(X)$.  Two objects with the same context-response
profile but different references therefore exist, which is exactly
the REF-INVIS clause of Definition~\ref{def:ling-ref-predicate}.
The Book~II boundary capacity law gives the quantitative form: a
probe episode certifies at most the information carried by its
declared input channels, and reference data over
$\mathcal{C}_{\mathrm{obs}}$ lies in no declared channel of the
factorized discipline, so its certifiable capacity under
$\mathcal{P}_{\mathrm{Ling}}$ is zero.  Book~VII discipline limits
the conclusion to the declared discipline and object class. \qed
\end{proof}

\begin{theorem}[\status{C}]\label{thm:ling-REF-RECOV}
\textup{(REF-RECOV under a Nominated Reference-Indicating Probe.)}
\textup{[Conditional on: nomination of a lawful
\emph{reference-indicating probe family}
$\mathcal{P}_{\mathrm{ref}}$ --- a certified process testing
whether LING object $X$ selects certified observable $Y$ under a
declared reference-testing discipline, sourced through
$\mathbf{U}$ with no hidden supplement (a calibration-pattern
declaration, parallel to the SPEC caesium nomination and the
pending CRYST diffraction standard); and the template machinery of
Thm.~\ref{thm:ling-sem-ref-template}.]}
REF-RECOV holds for the extended family
$\mathcal{P}'_{\mathrm{Ling}} = \mathcal{P}_{\mathrm{Ling}} \cup
\mathcal{P}_{\mathrm{ref}}$, and the MSC selects the unique
minimal faithful reference-probe family among role-equivalent
lawful extensions.
\end{theorem}

\begin{proof}
By construction, $\mathcal{P}_{\mathrm{ref}}$ breaks the
internal-factorization property in exactly one declared channel:
its outcome depends on the pair $(X, Y)$ through the reference
relation.  If two LING objects have the same
$\mathcal{P}'_{\mathrm{Ling}}$ profile, they agree in particular
on all reference-indicating outcomes, hence select the same
certified observable objects under the declared reference-testing
discipline, hence have the same reference: $\mathrm{ref}$ is
quotient-visible under $\mathcal{P}'_{\mathrm{Ling}}$, which is
the REF-RECOV clause of Definition~\ref{def:ling-ref-predicate}.
Lawfulness of the extension is precisely the content of the
nominated declaration and is not re-proved here.  The selection of
the unique minimal faithful reference-probe family among
role-equivalent lawful extensions is the REF-RECOV clause of the
template theorem (Thm.~\ref{thm:ling-sem-ref-template}), by the
Zorn/finite-stabilization argument.  \qed
\end{proof}

\begin{obligation}[\status{C}]\label{ob:ling-ref-probe-nomination}
\textup{(O-LING.SEM$_{\mathrm{ref}}$: Reference-Indicating Probe
Nominated.)}
\textup{[Conditional on Thm.~\ref{thm:ling-REF-RECOV} and the
declaration below.]}
A \emph{joint-attention selection probe} is hereby nominated as the
lawful reference-indicating probe family
$\mathcal{P}_{\mathrm{ref}}$: a certified process that presents a
LING object $X$ together with a finite declared array of certified
observable objects and records which object the discipline selects
under a declared selection protocol.  Its outcome depends on the
pair $(X, Y)$ through the reference relation, breaking the
internal-factorization property
(Def.~\ref{def:ling-probe-factorization}) in exactly the one
declared channel required by Thm.~\ref{thm:ling-REF-RECOV}.  The
nomination satisfies the lawfulness conditions on the same footing
as the SPEC caesium and CRYST anomalous-scattering nominations:
(a) sourced through $\mathbf{U}$ as a declared probe-response
process; (b) no hidden supplement --- the selected object is drawn
from the already-certified observable class
$\mathcal{C}_{\mathrm{obs}}$, introducing no new ontology; and
(c) equal profiles under the extended family force equal reference,
giving REF-RECOV.  The MSC selects the unique minimal faithful such
family among role-equivalent lawful selection protocols.  This
discharges residual condition (b) of \texttt{ob:ling-ref-formal}
(concrete probe nomination); residual condition (a), the
finite factorization audit of the base discipline, remains a
declared check.
\end{obligation}

\begin{obligation}[\status{C}]\label{ob:ling-ref-formal}
\textup{(O-LING.SEM$_{\mathrm{ref}}$: Formal Frontier ---
Conditionally Resolved, Two Routes at Two Scopes.)}
\textup{[Conditional on Thm.~\ref{thm:ling-REF-INVIS} and
Thm.~\ref{thm:ling-REF-RECOV}.]}
The question ``which sub-predicate holds'' is conditionally
answered: \textbf{both, at their respective scopes}, exactly as
the expected direction anticipated.  REF-INVIS holds for the
declared contrast-only discipline
(Thm.~\ref{thm:ling-REF-INVIS}); REF-RECOV holds for the
nominated reference-indicating extension
(Thm.~\ref{thm:ling-REF-RECOV}).  The two are compatible, not
competing: invisibility at the base discipline is what makes the
extension non-redundant.

\textbf{Residual conditions (inside the conditional force, not
re-opening this obligation):}
\begin{enumerate}[label=\textup{(\alph*)}]
  \item certify the internal-factorization property for the
    declared $\mathcal{P}_{\mathrm{Ling}}$ --- a finite audit over
    the declared probe list;
  \item nominate the concrete reference-indicating probe family
    --- a calibration-pattern declaration, parallel to the pending
    CRYST diffraction standard.
\end{enumerate}
This mirrors the CRYST phase-problem resolution structure:
structural invisibility certified by an invariance argument,
recovery certified conditionally on a nominated lawful probe
extension.
\end{obligation}
...
O-LING.SEM$_{\mathrm{ref}}$ & [C] & Reference-bearing probes: REF-INVIS at base discipline, REF-RECOV at nominated extension \\
...
context-response regime); (3) reference semantics conditionally resolved at two scopes
(REF-INVIS for the contrast-only base discipline, REF-RECOV for
the nominated reference-indicating extension); strong infinite
generativity and truth-condition semantics remain genuinely open
frontiers.
\end{remark}
