...
% ============================================================
\begin{remark}[\status{R}]\label{cap:book7-status}
\textbf{Current Status Capsule.} \emph{Current posture:} Canonical closure schema and limits of the spine: stable [U]/[D] governance layer. Branch postures and the four frontier accountings are catalogued here and in \texttt{NFC\_OBLIGATION\_ROSTER.md}. See Book~VII governance and \texttt{NFC\_STATE\_OF\_CANON.md} for the corpus-wide record.
\end{remark}
...
SCC Branch & Cond.\ CERT-CLOSE & UCTI/depth-sum conditions \\
GR Branch & Cond.\ CERT-CLOSE & Global curv.-subcrit. \\
CRYST Branch & Cond.\ CERT-CLOSE & Phase problem (toolkit-bdry) \\
SPEC Branch & CERT-CLOSE (gauge-resp.) & Matter-rich regime (SM-gated) \\
BIO Branch & Repl.-heredity endpoint & BND residual; EVO/MULTI \\
LING Branch & Contrast+rec.+ctx closed & Truth-cond.\ semantics \\
\hline
SM Branch & Cond.\ intrinsic-struct.\ closed & SM matter/external-spec.\ frontier \\
RH Branch & CERT-PROJ & RH4--6, S1 (arithmetic) \\
\hline
\end{tabular}
\end{center}

\emph{(Table reconciled --- migration \texttt{MIG-011}. SM: coupling-transfer
RG refinement is conditionally discharged and non-blocking (review ruling
F-6); the live SM deep frontier is matter content / external specification.
Four branches (CRYST, SPEC, BIO, LING) constituted after this table's first
authorship are now included (F-9).)}
...
\begin{itemize}
  ...
  \item \textbf{SM O-SM.Matter}: Quarks, leptons, Higgs field content
    (at full canonical force, gated on O\_ID\textsuperscript{cont}).
\end{itemize}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
  \item \textbf{Named open obligations} $[\mathrm{O}]$ --- formal
    ...
    canonical files (13 items at the time of this snapshot; see
    \texttt{NFC\_OBLIGATION\_ROSTER.md} for the current reconciled
    partition of all \texttt{ob:} labels and the live syntactic~[O]
    census) has a label, a declared
