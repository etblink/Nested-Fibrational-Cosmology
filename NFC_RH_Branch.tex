...
% ============================================================
\begin{remark}[\status{R}]\label{cap:RH-status}
\textbf{Current Status Capsule.}
\emph{Current posture:} CERT-PROJ: lawful branch with the RH kernel-positivity program conditionally advanced; the arithmetic S1 witness and the RH4--6 obstructions are the live frontier.

\emph{Authoritative source:} \ref{prop:RH-status}~(branch status proposition (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 1 (\texttt{ob:rh-s1-formal}); reduced irreducible frontier --- 2 (RH arithmetic orbit grammar; RH packet-local synthesis); branch posture --- CERT-PROJ.}
...
O-SCC.MCS, SCC Branch) applied to the Riemann $\Xi$-function.
...
\end{itemize}
\textbf{SCC-MCS cascade (Session~XLII update).}
O-SCC.MCS has been conditionally discharged in the SCC Branch
(\texttt{thm:scc-mcs}: existence by finite-stabilization/Zorn
argument from SCC-FL + SCC-LB; uniqueness by SCC-WeakGlue).
This advances the SCC dependency chain to O-SCC.UC (branchwise
completion). For RH: the SCC support-rigidity used in ob:RH5
(Thm.~\texttt{rh-ac-a3}, SCC Branch) is downstream of the
carrier structure whose MCS gate is now conditionally closed.
The RH selection-exclusion argument is correspondingly strengthened
at the SCC-admissibility level. The remaining RH gap (ob:RH4:
full operator realization at canonical force) is unchanged.
\end{remark}
...
\begin{obligation}[\status{C}]\label{ob:RH4}
...
\begin{obligation}[\status{C}]\label{ob:RH5}
...
\begin{obligation}[\status{C}]\label{ob:RH6}
...
\begin{proposition}[\status{C}]\label{prop:RH-status}
\textup{(RH Branch Status --- Updated.)}
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 \item \emph{SCC-MCS cascade}: O-SCC.MCS is conditionally discharged
 (SCC Branch, Thm.~\ref{thm:scc-mcs}); the RH selection-exclusion
 argument (ob:RH5, Thm.~\ref{thm:rh-ac-a3}) is correspondingly
 strengthened at SCC-admissibility level.
 ...
 scaling-flow/transfer-operator candidate framework,
 RH-N2 log-derivative realization (conditional),
 transport-localization chain L1--L7.
 \item \emph{Open obligations}: RH4 (full operator realization at
 canonical force), RH5--RH6 (exclusion and localization, blocked
 on RH4), S1 (witness assembly --- irreducible arithmetic frontier).
 \item \emph{Honest status}: the RH program is conditionally
 complete through all algebraic, spectral, support-rigidity, and
 transport-localization layers. The irreducible remaining obligation
 is S1 (witness assembly): pure arithmetic (Mellin/Dirichlet descent
 from $\Xi$) outside the current NFC collar-algebra toolkit.
\end{enumerate}
\end{proposition}

%% ---------------------------------------------------------------
\section{S1 Formal Theorem Packet}
\label{sec:rh-s1-formal}
%% ---------------------------------------------------------------

This section formalizes the Witness Assembly obligation S1 from a
named barrier to a precise theorem statement with named dependencies.
The ClaySpec methodology (applied to YM in the parallel branch) is
applied here to turn ``S1 is the irreducible arithmetic frontier''
into a formal conditional theorem shell.

\begin{definition}[\status{D}]\label{def:rh-arithmetic-witness}
\textup{(Arithmetic Witness Package.)}
An \emph{arithmetic witness package} for the RH Branch is a tuple
\[
 \mathcal{W}_{\mathrm{arith}} \;=\;
 (A_{\mathrm{RH}},\; W_{\mathrm{RH}},\; T_{\mathrm{RH}},\; D_{\mathrm{RH}})
\]
such that:
\begin{enumerate}[label=\textup{(\arabic*)}]
 \item $A_{\mathrm{RH}}$ is the canonical RH arena (the
 branch-declared SCC-admissible collar observable algebra
 for $\Xi$);
 \item $W_{\mathrm{RH}}\subset\mathcal{H}_{\mathrm{RH}}^{\mathrm{tr}}$
 is a finite SCC-admissible witness subfamily realizing the
 RH-N2 prime-power explicit-formula constraint;
 \item $T_{\mathrm{RH}}$ is the completed kernel/trace package,
 descended from $\mathbf{U}$; and
 \item $D_{\mathrm{RH}}$ is the RH branch defect ledger, certifying
 no hidden arithmetic primitive is imported.
\end{enumerate}
\end{definition}

\begin{definition}[\status{D}]\label{def:rh-s1-predicate}
\textup{(S1 Witness-Assembly Predicate.)}
The \emph{S1 predicate} holds for an arithmetic witness package
$\mathcal{W}_{\mathrm{arith}}$ when:
\begin{enumerate}[label=\textup{(\arabic*)}]
 \item \emph{S1-ARC}: the SCC admissibility condition
 $Q[\psi]\ge0$ holds for all $\psi\in W_{\mathrm{RH}}$;
 \item \emph{S1-TRACE}: the trace formula pairing
 $\mathrm{Tr}_{G_{\mathrm{sf}}}(h) =
 \sum_{p,m}(\log p)\,p^{-m/2}\,h(m\log p)$
 is certified for all $h\in W_{\mathrm{RH}}$;
 \item \emph{S1-DESCENT}: the arithmetic content descends to
 the Mellin/Dirichlet arithmetic data for $\zeta(s)$ without
 hidden import;
 \item \emph{S1-COMP}: the arithmetic witness package is
 SCC-minimal faithful (no non-load-bearing arithmetic sector).
\end{enumerate}
\end{definition}

\begin{theorem}[\status{C}]\label{thm:rh-s1-conditional}
\textup{(S1 Conditional Implication.)}
\textup{[Conditional on: the S1 predicate
(Def.~\ref{def:rh-s1-predicate}) holding for a declared arithmetic
witness package; and the SCC-RH equivalence
(Thm.~\ref{thm:scc-rh-equivalence}).]}
If the S1 predicate holds, then:
\[
 Q[\psi] \;\ge\; 0
 \quad\text{for all admissible }\psi,
\]
and therefore the Riemann Hypothesis holds conditionally on the
declared SCC-admissible scope.
\end{theorem}

\begin{proof}
By S1-ARC: $Q[\psi]\ge0$ for all $\psi\in W_{\mathrm{RH}}$.
By S1-TRACE: the trace formula realizes the prime-power explicit
formula (RH-N2), certifying the positive measure structure.
By S1-DESCENT: the arithmetic data descends without hidden import.
By the SCC-RH equivalence (Thm.~\ref{thm:scc-rh-equivalence}):
RH $\Leftrightarrow$ $Q[\psi]\ge0$ for all admissible $\psi$.
Therefore the S1 predicate implies RH at the declared scope.
\qed
\end{proof}

\begin{obligation}[\status{O}]\label{ob:rh-s1-formal}
\textup{(O-RH.S1: Arithmetic Witness Assembly --- Formal Statement.)}
The precise remaining obligation is: construct a finite arithmetic
witness package $\mathcal{W}_{\mathrm{arith}}$ satisfying the S1
predicate (Def.~\ref{def:rh-s1-predicate}).

The four S1 sub-predicates identify the irreducible arithmetic work:
\begin{enumerate}[label=\textup{(\arabic*)}]
 \item \emph{S1-ARC}: prove $Q[\psi]\ge0$ directly from the
 arithmetic structure of $\Xi$ without relying on the RH hypothesis.
 This is the mathematical content of RH itself, not yet proved.
 \item \emph{S1-TRACE}: the scaling-flow candidate already
 conditionally realizes RH-N2 (ob:rh-sf-trace-pairing-law [C]);
 this sub-predicate is conditionally discharged.
 \item \emph{S1-DESCENT}: the defect ledger $D_{\mathrm{RH}}$ and
 the transport-localization chain (L1--L7) establish descent
 without hidden import (conditionally).
 \item \emph{S1-COMP}: SCC-MCS is now discharged (SCC Branch,
 Thm.~\ref{thm:scc-mcs}); the minimal faithful SCC carrier for
 the RH witness package exists.
\end{enumerate}
Therefore the irreducible arithmetic content reduces to
\emph{S1-ARC alone}: proving $Q[\psi]\ge0$ from the arithmetic
structure of $\Xi$. All other S1 sub-predicates are conditionally
resolved. This is the sharpest possible statement of the
remaining RH frontier within the NFC canonical framework.
\end{obligation}

\begin{remark}[\status{R}]\label{rem:rh-s1-reduction}
\textbf{S1 reduction to S1-ARC.}
The S1 formal packet reduces the Witness Assembly obligation from a
four-predicate bundle to a single arithmetic predicate S1-ARC:
$Q[\psi]\ge0$ for all admissible $\psi$. Everything else in the
bundle is conditionally discharged: S1-TRACE via the scaling-flow
candidate (conditional), S1-DESCENT via L1--L7 (conditional), and
S1-COMP via SCC-MCS (now conditionally discharged). The residual
is precisely the mathematical content of RH --- the positivity of
the quadratic form $Q$ on the declared SCC-admissible probe space.
This is consistent with and strengthens the pre-existing
characterization: ``S1 is pure arithmetic, outside the NFC
collar-algebra toolkit.''

\textbf{CRYST parallel.} The CRYST branch phase problem
(O-CRYST.PHASE) is structurally parallel: both S1-ARC and
the PHASE-INVIS predicate are questions about what the declared
admissible probe discipline can certify from observational data
alone. In RH: can $Q[\psi]\ge0$ be proved from the arithmetic
structure of $\Xi$ without assuming RH? In CRYST: is the
crystallographic phase quotient-invisible under intensity-only
measurement? Both are irreducible observational information
questions --- the deepest frontiers in their respective branches,
with no NFC collar-algebra tool available to resolve them directly.
This parallel is the sharpest expression of the common structure
underlying the two principal open frontiers of the NFC corpus.
\end{remark}
...
\begin{obligation}[\status{C}]\label{ob:rh-sf-logderiv-legality}
...
\begin{obligation}[\status{C}]\label{ob:rh-sf-trace-pairing-law}
...
\begin{obligation}[\status{C}]\label{ob:rh-sf-d2-audit}
...
\begin{obligation}[\status{C}]\label{ob:rh-tloc-L3}
...
\begin{obligation}[\status{C}]\label{ob:rh-tloc-L4}
...
\begin{obligation}[\status{C}]\label{ob:rh-tloc-L5}
\textup{(L5: Gram Test.)}
\textup{[Conditionally discharged by
...
\begin{obligation}[\status{C}]\label{ob:rh-tloc-L6}
...
$\mathcal{W}_\rho^{\mathrm{core}}$. \qed
\end{proof}
...
\begin{obligation}[\status{C}]\label{ob:rh-tloc-L7}
