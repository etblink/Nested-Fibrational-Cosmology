...
% ============================================================
\begin{remark}[\status{R}]\label{cap:book3-status}
\textbf{Current Status Capsule.} \emph{Current posture:} Persistence, transport, and observational transfer (incl.\ the Variable Recovery Program arc): stable [U]/[C] layer; VRP items conditionally discharged or characterized. See Book~VII governance and \texttt{NFC\_STATE\_OF\_CANON.md} for the corpus-wide record.
\end{remark}
...
\begin{theorem}[\status{C}]\label{thm:vrp-charge-continuum-survival}
\textup{(Charge Discreteness Survives the Continuum Bridge.)}
\textup{[Conditional on: Thm.~\ref{thm:vrp-charge-quantization}
(finite-regime charge discreteness); a Book~VI continuum bridge
(\texttt{def:continuum-bridge}) faithful to the persistent finite
type-transition carrier in the sense of \texttt{thm:continuum-bridge-schema}.]}
The finite set of admissible charge values, carried by the persistent
finite type-transition structure, remains isolated under a faithful
Book~VI continuum bridge: no accumulation point appears in the continuum
encoding.
\end{theorem}

\begin{proof}
The admissible charge classes are finite in number
(Thm.~\ref{thm:vrp-charge-quantization}: charges are discrete because
types are, the type-transition operator being finite). A faithful bridge
(\texttt{def:continuum-bridge}) encodes the certified discrete structure
without collapsing quotient-visible distinctions and introduces no
continuum structure beyond that encoding
(\texttt{thm:continuum-bridge-schema}). A finite set has no accumulation
point in any Hausdorff continuum encoding; an accumulation could arise
only from infinitely many classes (contradicting finiteness) or from
bridge-introduced continuum charge values not descended from the finite
carrier (contradicting faithfulness/encoding-only discipline). Hence the
charge spectrum stays isolated. The conditional status rests on the
Book~VI faithfulness hypothesis, a bridge condition, not an import. \qed
\end{proof}

\begin{corollary}[\status{C}]\label{cor:vrp-frequency-lattice-survival}
\textup{(Abstract Frequency-Lattice Survival.)}
\textup{[Conditional on: Thm.~\ref{thm:vrp-phase-frequency-duality};
a faithful Book~VI bridge preserving the phase torus.]}
As the character lattice of the phase torus, the frequency module
$\Omega_B(I) \cong \mathbb{Z}^r$ survives the continuum bridge discretely
\emph{qua abstract lattice}.
\end{corollary}

\begin{proof}
By Thm.~\ref{thm:vrp-phase-frequency-duality},
$\Omega_B(I) \cong \mathbb{Z}^r = \widehat{T^r}$ is the Pontryagin dual of
the compact phase torus. The dual of a compact group is discrete; a
faithful bridge preserving $T^r$ preserves its character lattice. The
abstract-lattice discreteness is therefore carried across the bridge.
\emph{This is discreteness of $\Omega_B$ as an abstract group, not as a
subset of $\mathbb{R}$ --- see Rmk.~\ref{ob:vrp-charge-quant}.} \qed
\end{proof}

\begin{remark}[\status{O}]\label{ob:vrp-charge-quant}
\textbf{Open obligation O\_Charge.Quant-cont.b2 (Numeric Isolation of the
Frequency Module in $\mathbb{R}$).} The original O\_Charge.Quant-cont
bundled charge and frequency continuum survival as one deferral; that was
too coarse, and is now sharpened. The charge half
(Thm.~\ref{thm:vrp-charge-continuum-survival}, O\_Charge.Quant-cont.a) and
the abstract frequency-lattice half
(Cor.~\ref{cor:vrp-frequency-lattice-survival},
O\_Charge.Quant-cont.b1) are discharged conditionally on Book~VI bridge
faithfulness. What remains open is whether the frequency module
$\Omega_B(I) = \langle \lambda_i - \lambda_j \rangle$, viewed as a
\emph{subset of} $\mathbb{R}$, is numerically isolated. A finitely
generated subgroup of $\mathbb{R}$ of rank $\geq 2$ with rationally
independent generators is \emph{dense} in $\mathbb{R}$; so numeric
isolation requires a rank-one, commensurability, or uniform-separation
hypothesis on the spectral gaps. No such hypothesis is licensed by the
source data, and the anti-smuggling discipline
(Rmk.~\ref{rmk:vrp-frequency-scope}) bars importing $\hbar$, $E=\hbar\omega$,
or an external frequency scale to force separation. Hence numeric
isolation in $\mathbb{R}$ is retained as the sharpened open obligation.
Charge and frequency discreteness, analogous at finite spectral origin,
differ at the continuum-topology interface: the former is a finite set
(robustly isolated), the latter a finitely generated subgroup of
$\mathbb{R}$ (generically dense). \end{remark}
...
discrete frequency module splits at the bridge: as an abstract character
lattice it survives (Cor.~\ref{cor:vrp-frequency-lattice-survival}), but
its numeric isolation as a subset of $\mathbb{R}$ is the sharpened open
residual (Rmk.~\ref{ob:vrp-charge-quant}, O\_Charge.Quant-cont.b2), since
a finitely generated subgroup of $\mathbb{R}$ need not be isolated. This
is distinct from charge discreteness
(Thm.~\ref{thm:vrp-charge-continuum-survival}), a finite set that is
robustly isolated.  \emph{(iii) Out of toolkit:} the
...
\ref{sch:icdc-abstract}\ ICDC Schema (abstract)
  & [C] & \ref{thm:unified-coercive}; five conditions
