...
CLAIMS_DIR = ROOT / "claims" / "branches" / "LING"
...
EXPECTED_FRONTIERS: List[str] = ["ob:LING.REF-INVIS-RECOV"]
...
def load_ling_claims() -> Dict[str, Dict[str, Any]]:
...
def render_tex(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        r"\caption{LING successor-pilot claim catalog}\label{tab:ling-successor-claims}\\",
    ...
    body_parts = [
        ...
        r"\section*{LING Branch Successor Pilot}",
        r"\addcontentsline{toc}{section}{LING Branch Successor Pilot}",
        "",
        ...
        rf"Computed LING posture: \textbf{{{tex_escape(computed.get('status:LING', {}).get('effective_status', 'unknown'))}}}. The REF-INVIS/RECOV frontier remains active and is intentionally preserved in this pilot.",
        "",
        table,
        "",
    ...
    packet = "\n\n".join(body_parts)
    (TEX / "ling_branch_successor.tex").write_text(packet + "\n", encoding="utf-8")
...
\title{NFC LING Branch Successor Pilot}
\author{Generated governance shell}
\date{}
\begin{document}
\maketitle
\tableofcontents
\input{ling_branch_successor.tex}
\end{document}
"""
    (TEX / "ling_branch_successor_standalone.tex").write_text(standalone, encoding="utf-8")


def write_equivalence_report(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> Dict[str, Any]:
    generated_ids = [cid for cid in LING_ORDER if cid in claims]
    missing_expected = [cid for cid in LING_ORDER if cid not in claims]
    unexpected = sorted(set(claims) - set(LING_ORDER))
    active_frontiers = [
        cid for cid in generated_ids
        if computed.get(cid, {}).get("effective_status", claims[cid].get("declared_status")) == "O"
    ]
    posture = computed.get("status:LING", {}).get("effective_status")
    checks = [
        {
            "id": "ling.successor.expected-claim-list",
            "status": "pass" if not missing_expected else "fail",
            "observed": {"missing": missing_expected},
            "expected": "All LING_ORDER claims are present.",
        },
        {
            "id": "ling.successor.posture",
            "status": "pass" if posture == EXPECTED_POSTURE else "fail",
            "observed": posture,
            "expected": EXPECTED_POSTURE,
        },
        {
            "id": "ling.successor.frontier-profile",
        ...
        {
            "id": "ling.successor.no-unexpected-claims",
            "status": "pass" if not unexpected else "warn",
            "observed": unexpected,
            "expected": "No branch-local LING records outside the pilot order.",
        },
        {
            "id": "ling.successor.no-reference-frontier-erasure",
            "status": "pass" if posture == EXPECTED_POSTURE and "ob:LING.REF-INVIS-RECOV" in active_frontiers else "fail",
            "observed": {"posture": posture, "active_frontiers": active_frontiers},
            "expected": "Conditional linguistic closure is preserved while the reference invisibility/recovery frontier remains active.",
        },
    ]
    ...
    (REPORTS / "ling_successor_equivalence_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    lines = [
        ...
        f"**Computed LING posture:** `{posture}`",
    ...
    (TABLES / "ling_successor_equivalence_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report


def main() -> int:
    claims = load_ling_claims()
