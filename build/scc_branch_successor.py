...
CLAIMS_DIR = ROOT / "claims" / "branches" / "SCC"
...
EXPECTED_FRONTIERS: List[str] = []
EXPECTED_POSTURE = "conditional-cert-close"
REQUIRED_BASIS = ["thm:SCC.MCS", "thm:SCC.UC", "thm:SCC.TSI"]
...
def load_scc_claims() -> Dict[str, Dict[str, Any]]:
...
def render_tex(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        r"\caption{SCC successor-pilot claim catalog}\label{tab:scc-successor-claims}\\",
    ...
    body_parts = [
        ...
        r"\section*{SCC Branch Successor Pilot}",
        r"\addcontentsline{toc}{section}{SCC Branch Successor Pilot}",
        "",
        ...
        rf"Computed SCC posture: \textbf{{{tex_escape(computed.get('status:SCC', {}).get('effective_status', 'unknown'))}}}. The pilot preserves structural counterfactual capacity only; no phenomenological endpoint is asserted.",
        "",
        table,
        "",
    ...
    packet = "\n\n".join(body_parts)
    (TEX / "scc_branch_successor.tex").write_text(packet + "\n", encoding="utf-8")
...
\title{NFC SCC Branch Successor Pilot}
\author{Generated governance shell}
\date{}
\begin{document}
\maketitle
\tableofcontents
\input{scc_branch_successor.tex}
\end{document}
"""
    (TEX / "scc_branch_successor_standalone.tex").write_text(standalone, encoding="utf-8")


def write_equivalence_report(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> Dict[str, Any]:
    generated_ids = [cid for cid in SCC_ORDER if cid in claims]
    missing_expected = [cid for cid in SCC_ORDER if cid not in claims]
    unexpected = sorted(set(claims) - set(SCC_ORDER))
    active_frontiers = [
        cid for cid in generated_ids
        if computed.get(cid, {}).get("effective_status", claims[cid].get("declared_status")) == "O"
    ]
    posture = computed.get("status:SCC", {}).get("effective_status")
    basis_statuses = {cid: computed.get(cid, {}).get("effective_status") for cid in REQUIRED_BASIS}
    obligations = ["ob:SCC.MCS", "ob:SCC.UC", "ob:SCC.TSI"]
    obligation_statuses = {cid: computed.get(cid, {}).get("effective_status") for cid in obligations}
    checks = [
        {
            "id": "scc.successor.expected-claim-list",
            "status": "pass" if not missing_expected else "fail",
            "observed": {"missing": missing_expected},
            "expected": "All SCC_ORDER claims are present.",
        },
        {
            "id": "scc.successor.posture",
            "status": "pass" if posture == EXPECTED_POSTURE else "fail",
            "observed": posture,
            "expected": EXPECTED_POSTURE,
        },
        {
            "id": "scc.successor.zero-frontier-profile",
            "status": "pass" if not active_frontiers else "fail",
            "observed": active_frontiers,
            "expected": "No active SCC-local frontier in the MCS+UC+TSI successor pilot.",
        },
        {
            "id": "scc.successor.mcs-uc-tsi-basis",
            "status": "pass" if all(v in {"C", "U"} for v in basis_statuses.values()) else "fail",
            "observed": basis_statuses,
            "expected": "MCS, UC, and TSI basis theorems have conditional or unconditional force.",
        },
        {
            "id": "scc.successor.obligations-conditionally-closed",
            "status": "pass" if all(v in {"C", "U"} for v in obligation_statuses.values()) else "fail",
            "observed": obligation_statuses,
            "expected": "SCC MCS, UC, and TSI obligation records are conditionally closed.",
        },
        {
            "id": "scc.successor.no-unexpected-claims",
            "status": "pass" if not unexpected else "warn",
            "observed": unexpected,
            "expected": "No branch-local SCC records outside the pilot order.",
        },
        {
            "id": "scc.successor.no-phenomenological-upgrade",
            "status": "pass" if posture == EXPECTED_POSTURE and not active_frontiers else "fail",
            "observed": {"posture": posture, "active_frontiers": active_frontiers},
            "expected": "Closure remains structural counterfactual capacity only; no phenomenological/consciousness endpoint is inferred.",
        },
    ]
    ...
    (REPORTS / "scc_successor_equivalence_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    lines = [
        ...
        f"**Computed SCC posture:** `{posture}`",
    ...
    for cid, st in basis_statuses.items():
    ...
    (TABLES / "scc_successor_equivalence_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report


def main() -> int:
    claims = load_scc_claims()
