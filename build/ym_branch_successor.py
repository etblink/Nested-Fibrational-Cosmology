...
def load_ym_claims() -> Dict[str, Dict[str, Any]]:
...
def render_tex(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        r"\caption{YM successor-pilot claim catalog}\label{tab:ym-successor-claims}\\",
    ...
    body_parts = [
        ...
        r"\section*{YM Branch Successor Pilot}",
        r"\addcontentsline{toc}{section}{YM Branch Successor Pilot}",
        "",
        ...
        rf"Computed YM posture: \textbf{{{tex_escape(computed.get('status:YM', {}).get('effective_status', 'unknown'))}}}. The surviving residual frontier is \texttt{{ob:YM.B3.A2}}.",
        "",
        table,
        "",
    ...
    packet = "\n\n".join(body_parts)
    (TEX / "ym_branch_successor.tex").write_text(packet + "\n", encoding="utf-8")
...
\title{NFC YM Branch Successor Pilot}
\author{Generated governance shell}
\date{}
\begin{document}
\maketitle
\tableofcontents
\input{ym_branch_successor.tex}
\end{document}
"""
    (TEX / "ym_branch_successor_standalone.tex").write_text(standalone, encoding="utf-8")


def write_equivalence_report(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> Dict[str, Any]:
    generated_ids = [cid for cid in YM_ORDER if cid in claims]
    missing_expected = [cid for cid in YM_ORDER if cid not in claims]
    unexpected = sorted(set(claims) - set(YM_ORDER))
    active_frontiers = [
        cid for cid in generated_ids
        if computed.get(cid, {}).get("effective_status", claims[cid].get("declared_status")) == "O"
    ]
    posture = computed.get("status:YM", {}).get("effective_status")
    checks = [
        {
            "id": "ym.successor.expected-claim-list",
            "status": "pass" if not missing_expected else "fail",
            "observed": {"missing": missing_expected},
            "expected": "All YM_ORDER claims are present.",
        },
        {
            "id": "ym.successor.posture",
            "status": "pass" if posture == EXPECTED_POSTURE else "fail",
            "observed": posture,
            "expected": EXPECTED_POSTURE,
        },
        {
            "id": "ym.successor.frontier-preservation",
        ...
        {
            "id": "ym.successor.no-unexpected-claims",
            "status": "pass" if not unexpected else "warn",
            "observed": unexpected,
            "expected": "No branch-local YM records outside the pilot order.",
        },
        {
            "id": "ym.successor.no-clay-upgrade",
            "status": "pass" if "ob:YM.B3.A2" in active_frontiers else "fail",
            "observed": active_frontiers,
            "expected": "B3-A2 remains open; no unconditional Clay-grade upgrade is inferred.",
        },
    ]
    ...
    (REPORTS / "ym_successor_equivalence_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    lines = [
    ...
    (TABLES / "ym_successor_equivalence_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report


def main() -> int:
    claims = load_ym_claims()
