...
CLAIMS_DIR = ROOT / "claims" / "branches" / "SPEC"
...
def load_spec_claims() -> Dict[str, Dict[str, Any]]:
...
def render_tex(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> None:
    ...
    lines = [
        ...
        r"\caption{SPEC successor-pilot claim catalog}\label{tab:spec-successor-claims}\\",
    ...
    body_parts = [
        ...
        r"\section*{SPEC Branch Successor Pilot}",
        r"\addcontentsline{toc}{section}{SPEC Branch Successor Pilot}",
        "",
        ...
        rf"Computed SPEC posture: \textbf{{{tex_escape(computed.get('status:SPEC', {}).get('effective_status', 'unknown'))}}}. No active SPEC-local frontier is expected in the declared gauge-response regime.",
        "",
        table,
        "",
    ...
    packet = "\n\n".join(body_parts)
    (TEX / "spec_branch_successor.tex").write_text(packet + "\n", encoding="utf-8")
...
\end{document}
"""
    (TEX / "spec_branch_successor_standalone.tex").write_text(standalone, encoding="utf-8")


def write_equivalence_report(claims: Dict[str, Dict[str, Any]], computed: Dict[str, Any]) -> Dict[str, Any]:
    generated_ids = [cid for cid in SPEC_ORDER if cid in claims]
    missing_expected = [cid for cid in SPEC_ORDER if cid not in claims]
    unexpected = sorted(set(claims) - set(SPEC_ORDER))
    active_frontiers = [
        cid for cid in generated_ids
        if computed.get(cid, {}).get("effective_status", claims[cid].get("declared_status")) == "O"
    ]
    posture = computed.get("status:SPEC", {}).get("effective_status")
    checks = [
        {
            "id": "spec.successor.expected-claim-list",
            "status": "pass" if not missing_expected else "fail",
            "observed": {"missing": missing_expected},
            "expected": "All SPEC_ORDER claims are present.",
        },
        {
            "id": "spec.successor.posture",
            "status": "pass" if posture == EXPECTED_POSTURE else "fail",
            "observed": posture,
            "expected": EXPECTED_POSTURE,
        },
        {
            "id": "spec.successor.zero-frontier-profile",
            "status": "pass" if not active_frontiers else "fail",
            "observed": active_frontiers,
            "expected": "No active SPEC-local frontier in the gauge-response successor pilot.",
        },
        {
            "id": "spec.successor.no-unexpected-claims",
            "status": "pass" if not unexpected else "warn",
            "observed": unexpected,
            "expected": "No branch-local SPEC records outside the pilot order.",
        },
        {
            "id": "spec.successor.no-matter-rich-upgrade",
            "status": "pass" if posture == "cert-close" and not active_frontiers else "fail",
            "observed": {"posture": posture, "active_frontiers": active_frontiers},
            "expected": "Closure is scoped to gauge-response spectroscopy only; no matter-rich spectroscopy closure is inferred.",
        },
    ]
    ...
    (REPORTS / "spec_successor_equivalence_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    lines = [
        ...
        f"**Computed SPEC posture:** `{posture}`",
    ...
    (TABLES / "spec_successor_equivalence_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report


def main() -> int:
    claims = load_spec_claims()
