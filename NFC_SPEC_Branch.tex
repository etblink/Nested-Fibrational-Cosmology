...
\begin{openobligation}[\status{O}]\label{ob:SPEC-Planck-calibration}
\textup{(O-SPEC.Planck: Planck Calibration --- Conditional Frontier.)}
\textup{[Conditional on Thm.~\ref{thm:SPEC-Planck-calibration}
and a specific declared empirical reference transition.]}
Thm.~\ref{thm:SPEC-Planck-calibration} is conditional on declaring a
calibration-eligible spectroscopic transition $T_{\mathrm{cal}}$.
The obligation is to: (1) nominate a specific reference transition
(e.g.\ the caesium hyperfine transition at $\nu_{\mathrm{Cs}} =
9{,}192{,}631{,}770\,\mathrm{Hz}$, which is the SI definition of
the second); (2) certify it as calibration-eligible within the SPEC
branch; and (3) verify the canonical energy difference $\Delta E_{\mathrm{can}}^{\mathrm{cal}}$
against the SPEC transition ledger. This is not needed for the
Clay YM proof (mathematical units); it is needed if the program
claims measured SI physical constants.
\end{openobligation}
