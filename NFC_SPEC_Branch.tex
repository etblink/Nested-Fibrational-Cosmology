...
%% SPEC Branch Book: Spectroscopy Branch (Constituted; see Current Status Capsule)
%% Current status: CERT-CLOSE on the gauge-response regime; continuum interfaces at bridge [B]; matter-rich regime open (SM-gated). See prop:SPEC-status / cap:SPEC-status.
%% Constitutional intake (as constituted): Architecturally constituted; pre-closure — all theorem shells carry open obligations. [retained as intake; superseded — MIG-005]
%%
%% Status legend:
...
\author{Elements of Nested Fibrational Cosmology\\
 Constituted Derived Branch\\
 {\normalsize Current canonical status is governed by the Current Status Proposition in this volume.}}
\date{}
\maketitle
\tableofcontents
\newpage
...
% ============================================================
\begin{remark}[\status{R}]\label{cap:SPEC-status}
\textbf{Current Status Capsule.}
\emph{Current posture:} CERT-CLOSE on the gauge-response spectroscopy regime (obligations O-SPEC.PR--END conditionally discharged); continuum interfaces licensed at bridge force~[B]; the matter-rich regime remains open, gated on the SM branch.

\emph{Authoritative source:} \ref{prop:SPEC-status}~(branch status proposition (current posture)).

\emph{Frontier accountings (never summed): syntactic~[O] in this branch --- 0; reduced irreducible frontier --- none (matter coupling is downstream/SM-gated); branch posture --- CERT-CLOSE on the gauge-response regime. The earlier \emph{Initial Constitution Status} proposition (Prop.~\ref{prop:spec-current-status}) records the branch as constituted and is superseded by Prop.~\ref{prop:SPEC-status}.}

\emph{Constitutional intake:} The Purpose/Seed and front-block material below records how this branch was \emph{constituted} (founding question and original obligations); it is constitutional intake, not current status.
\end{remark}
...
\begin{obligation}[\status{C}]\label{ob:spec-frontier-min}
...
\subsection{Initial Constitution Status (superseded)}\label{sec:spec-current-status}

\begin{proposition}[\status{C}]\label{prop:spec-current-status}
\textup{(Spectroscopy Branch: Initial Constitution Status.)}
\emph{This proposition records the branch status \textbf{at constitution},
before the Section~\ref{sec:spec-discharges} discharge packet. It is
retained as constitutional intake and is \textbf{superseded} for current
status by Prop.~\ref{prop:SPEC-status} (gauge-response CERT-CLOSE).}
As constituted, the spectroscopy branch was a prospective lawful descendant
program with strong architectural precedent but without canonical closure.
More precisely, at that stage:
\begin{enumerate}[label=\textup{(\arabic*)}]
 \item it inherited lawful descent conditions from the spine and
 branch-governance books (Books~I--VII);
 \item it inherited an operator-spectral precedent from the YM branch;
 \item it inherited a response-structure precedent from the GR branch;
 \item it did not yet possess a certified transition theorem;
 \item it did not yet possess a certified no-hidden-loss theorem for
 spectroscopy proper;
 \item it did not yet possess a fully discharged continuum spectroscopy
 bridge;
 \item full matter-rich spectroscopy remained downstream of O-SPEC.MAT.
\end{enumerate}
Items~(4)--(6) were subsequently discharged at conditional force in
Section~\ref{sec:spec-discharges}; item~(7) (matter-rich spectroscopy)
remains open and SM-gated. Current status is governed by
Prop.~\ref{prop:SPEC-status}.
\end{proposition}

\begin{proof}
Items~(1)--(3) state the architectural inputs already in place at
constitution. Items~(4)--(7) were, at that stage, exactly the named open
obligations recorded in the closure ledger (Def.~\ref{def:spec-closure-ledger}). \qed
\end{proof}
...
\begin{obligation}[\status{C}]\label{ob:spec-MAT-open}
...
\begin{obligation}[\status{C}]\label{ob:SPEC-Planck-calibration}
