...
The \emph{source-side gauge algebra object} at refinement scale $R$ is
the sextuple
\[
  \mathbb{K}_{\mathrm{YM}}(R) \;=\;
  \bigl(\Sigma_R^\partial,\; \mathcal{G}_R^\partial,\;
        T_R,\; D_R,\; A_R,\; \Delta_{A,R}\bigr),
\]
where:
\begin{enumerate}[label=\textup{(\arabic*)}]
  \item $\Sigma_R^\partial$: stabilized collar-type data (Book~II);
  \item $\mathcal{G}_R^\partial$: admissible collar generators and
    toggles (Book~II, collar-alphabet discipline);
  \item $T_R$: transfer and boundary-law structure (Books~II--III);
  \item $D_R$: defect ledger and coercive bookkeeping (Books~I, III);
  \item $A_R$: collar-inherited connection object (YM branch); and
  \item $\Delta_{A,R}$: screened covariant Laplacian on the declared
    Phase-A sector (YM front block).
\end{enumerate}
$\mathbb{K}_{\mathrm{YM}}(R)$ is not yet a Lie algebra; it is the
most precise source-side object currently licensed by the canon.
The target of O-ID is to identify a Lie algebra structure on a
quotient of $\mathbb{K}_{\mathrm{YM}}(R)$.
\end{definition}
...
\textup{(Admissible Composition Is Associative on the Collar Domain.)}
On the stabilized collar domain, admissible composition of collar
transformations is associative.
\end{lemma}

\begin{proof}
By Axiom~A1 (Book~I, closure under typed composition), admissible
composition $X \circ Y$ is defined whenever the types match.
Composition of functions on a finite set is always associative:
$(X \circ Y) \circ Z = X \circ (Y \circ Z)$ because both sides
evaluate to the same function.  Since collar transformations are
functions on the finite collar-alphabet domain, associativity is
unconditional. \qed
\end{proof}

\begin{corollary}[\status{U}]\label{cor:jacobi-holds}
\textup{(Jacobi Identity for Collar Commutators.)}
For admissible collar transformations $X, Y, Z$ on the Phase-A sector,
\[
  [X,[Y,Z]] + [Y,[Z,X]] + [Z,[X,Y]] = 0.
\]
\end{corollary}
...
\begin{proof}
The commutator $[X,Y] = X \circ Y - Y \circ X$ is a commutator of
linear maps on a finite-dimensional space.  The Jacobi identity is an
algebraic consequence of associativity of composition (expand each
double commutator; all terms cancel in pairs).  Associativity holds by
Lemma~\ref{lem:collar-assoc}. \qed
\end{proof}
...
Let $B$ be a transport-compatible invariant bilinear form on the
...
implies $[h_i, e_j] = -e_j$), the off-diagonal coupling gives
...
incidence graph (Lemma~\ref{lem:E8-graph-reduction}) to all nodes. \qed
\end{proof}
...
\begin{lemma}[\status{C}]\label{lem:S1a-pendant-T-commute}
\textup{(TM14 Block-Constant Lift: Pendant Generators Commute with
$T_\partial$.)}
\textup{[Conditional on $K_0=7$, Phase-A.]}
