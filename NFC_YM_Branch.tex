...
\begin{openobligation}[\status{O}]\label{ob:YM-B3-A2}
...
Status note: the shared $A_1/A_2$ interface is not zero and not
quotient-invisible. The viable routes are gap-subcriticality
(faster) or pure $A_2$ re-branching (cleaner but possibly requiring
a new sub-branch).
\end{openobligation}
...
\begin{openobligation}[\status{O}]\label{ob:O-YM-WeakGlue-standalone}
\textup{(O-YM.WeakGlue: Standalone Open Obligation.)}
The WeakGlue property is conditionally discharged by
Thm.~\ref{thm:YM-WeakGlue} under the stated carrier hypotheses.
It remains a named obligation to be verified for each new class
of YM spectral carriers arising under regime extension or
domain change.
\end{openobligation}

\begin{openobligation}[\status{O}]\label{ob:O-YM-SVC}
\textup{(O-YM.SVC: Spectral Variation Control.)}
For lawful Phase-A YM carriers $K' \preceq_{YM} K$, prove the
Lipschitz spectral variation bound:
\[
 |\lambda_1(\Delta_A^K) - \lambda_1(\Delta_A^{K'})| \;\le\;
 |\Lambda_{K \to K'}^{YM}|_{\mathrm{spec}}.
\]
This is the stronger variant of O-YM.SLC. It must remain
carrier-local and Phase-A scoped to avoid circular dependence on
$O_{RIG}$.
\end{openobligation}
...
\begin{openobligation}[\status{O}]\label{ob:B3-A2-reserve}
\textup{(O-YM.B3-A2-Reserve: $A_2$ Reserve Inequality.)}
Certify $g_{A_2} - E_{A_2} - B_{A_2} > \tfrac{1}{3}B_{I_{12}}$
with declared (rather than unit-burden) boundary ledger weights.
This is the final inequality needed to promote B3-SU3-Bridge from
a unit-stress-test result to a fully certified gap theorem.
\end{openobligation}
...
\begin{openobligation}[\status{O}]\label{ob:Planck-SI-calibration}
