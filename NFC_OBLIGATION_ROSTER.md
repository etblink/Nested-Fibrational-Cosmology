...
**Corrective J note (semantic synchronization only).** Canonical NS posture: **CERT-PROJ / open common-state frontier, with UWB retained as a separate unproved conditional scope hypothesis.** Every active consumer of the global window chain names both the hypothetical discharge of `ob:NS-common-state-bridge` and UWB. UWB remains outside the `[O]` census and reduced-frontier counts; Ren.5, SB2, and UH.3 are predecessor inputs rather than the current formal burden; IDC remains subsidiary and insufficient; α remains withdrawn; unconditional Stage-3 regularity remains external. No theorem, hypothesis, obligation, label, status, proof force, endpoint posture, or census changed (Corrective J).
...
**Branch posture:** NS = **CERT-PROJ / open common-state frontier, with UWB retained as a separate unproved conditional scope hypothesis**;
