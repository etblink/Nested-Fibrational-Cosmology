...
% ============================================================
\begin{remark}[\status{R}]\label{cap:book6-status}
\textbf{Current Status Capsule.} \emph{Current posture:} Effective continuum and calculus legitimacy: stable [U]/[C] layer. See Book~VII governance and \texttt{NFC\_STATE\_OF\_CANON.md} for the corpus-wide record.
\end{remark}
...
\textup{(Normed Asymptotic Coherence.)}
Let $\mathcal{F}$ be a certified observable family and let
$\|\cdot\|$ be a declared transport-compatible comparison norm on
the increment space (as in the governing interface regime
$\mathfrak{R}$).  The family $\mathcal{F}$ is \emph{asymptotically
coherent with respect to $\|\cdot\|$} if the scale-normalized
increments $h_n(U)$ form a Cauchy sequence in $\|\cdot\|$ as
$n \to \infty$: for every declared tolerance $\varepsilon > 0$,
there exists $N$ such that
\[
  \|h_m(U) - h_n(U)\| < \varepsilon
  \quad\text{for all } m, n \geq N.
\]
When the norm is understood from context, we say $\mathcal{F}$
is simply \emph{asymptotically coherent}.  The norm $\|\cdot\|$
must always be declared in the governing branch or book document;
bare asymptotic coherence without a declared norm is not canonical
content.
\end{definition}
...
\textup{(Hardened Continuum Interface Regime.)}
A \emph{continuum interface regime} is a declared quadruple
$\mathfrak{R} = (\mathcal{R},\, \mathcal{F},\, \|\cdot\|,\, \mathcal{S})$
consisting of:
\begin{enumerate}[label=\textup{(\arabic*)}]
  \item \emph{Effective domain} $\mathcal{R}$: a smooth connected
    domain equipped with a certified geometric/topological structure
    derived from the observable carrier (not assumed as a primitive
    manifold);
  \item \emph{Certified observable family} $\mathcal{F}$: a
    directed system of certified observables whose scale-normalized
    increments generate the differential structure on $\mathcal{R}$;
  \item \emph{Declared comparison norm} $\|\cdot\|$: an
    explicit norm on the increment space, transport-compatible
    (in the sense of Book~II Definition~\ref{def:transport-compatible-norm})
    and governing all Cauchy-sequence convergence assertions; and
  \item \emph{Declared scope} $\mathcal{S}$: the explicit
    statement of what differential and integral language is
    licensed within $\mathfrak{R}$, and what is not --- no
    continuum claim has force outside this declared scope.
\end{enumerate}
The quadruple $\mathfrak{R}$ is \emph{lawful} if:
\begin{enumerate}[label=\textup{(\alph*)}]
  \item $\mathcal{F}$ is asymptotically coherent on $\mathcal{R}$
    with respect to $\|\cdot\|$;
  \item the scale-normalized increments converge in $\|\cdot\|$
    to the relevant differential expressions of the effective smooth
    representative on $\mathcal{R}$; and
  \item the discrete accumulators converge in $\|\cdot\|$ to the
    corresponding integral expressions within the declared scope
    $\mathcal{S}$.
\end{enumerate}
When context is clear the quadruple $\mathfrak{R}$ is abbreviated
$(\mathcal{R}, \mathcal{F})$ as before; the norm and scope are
then understood to be whatever was declared in the governing branch
or book document.
\end{definition}

\begin{remark}[\status{R}]\label{rem:quadruple-regime-significance}
The four-component declaration makes explicit three things that the
two-component version left implicit: (a) the norm governing
convergence is \emph{declared}, not assumed; (b) the scope of
licensed continuum language is \emph{explicit}, not inferred; and
(c) the domain $\mathcal{R}$ is itself \emph{derived from the
observable carrier}, not smuggled in as a primitive geometric space.
This is the canonical response to the hostile referee question
``What topology are you using?''\ and ``In what space?''  The
quadruple $\mathfrak{R}$ answers both questions explicitly, by
reference to declared branch data.
\end{remark}
...
%% ---------------------------------------------------------------
%% ---------------------------------------------------------------
\section{NS-4B Sobolev Surrogate and E-CONT Bridge}
\label{sec:sobolev-econt}
%% ---------------------------------------------------------------

\subsection{NS-4B: Discrete Sobolev Embedding Surrogate}

\begin{theorem}[\status{C}]\label{thm:NS-4B-sobolev}
\textup{(NS-4B: Sobolev Embedding Surrogate.)}
\textup{[Conditional on $K_0=7$, Phase-A, $\mathrm{BUO}(\delta)$
(Book~III, Def.~\ref{def:BUO}), and the ACA Coercivity Chain
(Book~III, Thm.~\ref{thm:ACA-chain}).]}
The following Sobolev-type embedding holds with all constants
table-computable from NF$_2$ data:
\[
  \|f\|_\infty \;\leq\; C_S(s)\,\|f\|_{H^s},
  \quad s > d/2,\quad d = 9\;\text{(NF$_2$ dimension)},
\]
where $C_S(s) = F_S(s, d, C_{\mathrm{HK}})$ is a finite function
of the Sobolev parameter $s$, the NF$_2$ dimension $d=9$, and
the ultracontractivity constant $C_{\mathrm{HK}}$ (from
Book~III, Thm.~\ref{thm:ACA-chain}).  No metric geometry or
continuum manifold is imported.
\end{theorem}

\begin{proof}
By the ACA Coercivity Chain (Book~III, Thm.~\ref{thm:ACA-chain}):
ultracontractivity ($C_{\mathrm{HK}}$) gives the heat-kernel
bound; Nash ($C_N$) gives the $L^2$-type control; the Sobolev
surrogate $\|f\|_\infty \leq C_S(s)\|f\|_{H^s}$ for $s > d/2$
then follows by spectral functional calculus on the finite-dimensional
type-walk Laplacian $\mathcal{L}_{\mathrm{walk}}$.  The exponent
threshold $s > d/2 = 9/2$ is the standard Sobolev condition for
the effective dimension $d=9$ of the NF$_2$ interaction class space.
All constants are finite functions of the BUO parameter $\delta$
and the table dimension. \qed
\end{proof}

\begin{remark}[\status{R}]
Theorem~\ref{thm:NS-4B-sobolev} is the NS-4B Sobolev surrogate
identified in the Holding (v7.35 §9333).  It licenses the use of
$\|\cdot\|_\infty \lesssim \|\cdot\|_{H^s}$ in both the NS and YM
branch books via the Book~VI declared scope $\mathcal{S}$ of the
relevant continuum interface regime.  The canonical chain is:
$\mathrm{BUO}(\delta)$ (Book~III) $\to$ Cheeger gap
(Book~III, Lem.~\ref{lem:cheeger-from-BUO}) $\to$ ACA Chain
(Book~III, Thm.~\ref{thm:ACA-chain}) $\to$ NS-4B
(Thm.~\ref{thm:NS-4B-sobolev}) $\to$ Book~VI scope legitimacy.
\end{remark}

\subsection{E-CONT Bridge and Book VI Gate}

\begin{remark}[\status{R}]\label{rem:econt-gate}
\textbf{E-CONT Bridge Obligations as Book~VI Gate.}
The v7.35 E-CONT bridge obligations record the two concrete
instances of the Book~VI Continuum Bridge Schema
(Thm.~\ref{thm:continuum-bridge-schema}):
\begin{enumerate}[label=\textup{(\roman*)}]
  \item \textbf{YM continuum (R3.6)}: encoding map
    $\Gamma: K(G_n) \to \mathcal{M}$ --- discharged by O\_ENC
    (Thm.~\ref{thm:O-ENC}, YM Branch) as a concrete instance
    of the abstract Continuum Bridge Schema with the Weyl encoding
    as the declared realization functor.
  \item \textbf{Gravity continuum (R1.1--R1.2)}: curvature
    compatibility E3-proxy --- discharged by NFC-5 curvature
    encoding (Thm.~\ref{thm:nfc5}, GR Branch) and the KPO-3
    import (Hyp.~\ref{hyp:KPO3}, GR Branch).
\end{enumerate}
Both R3.6 and R1.1--R1.2 are properly gated by
Thm.~\ref{thm:continuum-bridge-schema}: no branch may invoke
continuum notation outside the declared scope without passing
through this schema.  The E-CONT bridge confirms that the
canonical schema is correctly matched to the pre-canonical
program's bridge obligations.
\end{remark}

\begin{theorem}[\status{U}]\label{thm:T-partial-stability}
\textup{(Perturbation Stability of $T_\partial$.)}
\textup{[Unconditional: $K_0=7$ proved (Thm.~\ref{thm:K0-prime}, Book~I); Phase-A follows (Cor.~\ref{cor:phase-a-universal-K07}).]}
The type-transition operator $T_\partial : V \to V$ is stable
under small perturbations of the admissible collar substrate:
if $T_\partial'$ is the type-transition operator of a
substrate differing from the canonical one by a perturbation of
size $\epsilon$ in the collar transition weights, then
\[
  \|T_\partial' - T_\partial\|_{\mathrm{op}} \;\leq\;
  \delta_{\mathrm{stab}}\,\epsilon,
\]
where $\delta_{\mathrm{stab}}$ is the stability constant,
finite and computable from the NF$_2$ type table.  In
particular, the eigenvalues of $T_\partial$ are Lipschitz
in the perturbation with constant $\delta_{\mathrm{stab}}$.
\end{theorem}

\begin{proof}
The operator norm $\|T_\partial' - T_\partial\|_{\mathrm{op}}$
is bounded by $K_0 \cdot \epsilon_{\max}$, where
$\epsilon_{\max}$ is the maximum perturbation of any single
transition weight.  Since $T_\partial$ is a finite-dimensional
matrix of size $K_0 \times K_0 = 7 \times 7$, the operator
norm perturbation bound follows from the finite-dimensional
Weyl inequalities for eigenvalue perturbation:
$|\lambda_k(T_\partial') - \lambda_k(T_\partial)| \leq
\|T_\partial' - T_\partial\|_{\mathrm{op}} \leq K_0 \epsilon$.
The stability constant is $\delta_{\mathrm{stab}} = K_0 = 7$,
table-computable. \qed
\end{proof}

\begin{theorem}[\status{C}]\label{thm:lorentzian-signature}
\textup{(Emergent Lorentzian Signature.)}
\textup{[Conditional on Phase-A, $K_0=7$, and the spatial
isotropy hypothesis \textup{(SI)}: the admissibly invariant
nondegenerate quadratic form $Q$ satisfies cone-null
compatibility $\partial C \subseteq \{v : Q(v)=0\}$, and
$C$ is the cone of causally future-directed vectors derived
from the canonical temporal preorder of Book~III.]}
Under Phase-A coherence and hypothesis~\textup{(SI)}, the
quadratic form $Q$ has Lorentzian signature $(-,+,+,+)$ with
the negative direction aligned with the canonical time
direction of Book~III.
\end{theorem}

\begin{proof}
\textbf{Anti-smuggling audit.}
Hypothesis~(SI) (spatial isotropy + cone-null compatibility)
must be checked against Book~I anti-smuggling discipline.
The cone $C$ is derived from the Book~III temporal preorder
(the canonical defect-accumulation direction), which is
source-descended.  The cone-null compatibility condition
$\partial C \subseteq \{v : Q(v)=0\}$ is a constraint on
$Q$ relative to a declared source-descended structure; it does
not import metric geometry from outside.  Spatial isotropy
(the symmetry of $C$ under spatial relabeling) follows from
Phase-A transitivity ($K_0 = 7$ collar walk visits all
symbols) + the canonical $G = \mathrm{SU}(K_0)$ symmetry
established by PASS (Thm.~\ref{thm:PASS-derived}, GR Branch).
Hence hypothesis~(SI) is source-descended and passes the
anti-smuggling check.

\textbf{Signature derivation.}
The quadratic form $Q$ is nondegenerate (by hypothesis) and
$G$-invariant (by PASS).  Under Phase-A, the collar walk
distinguishes one canonical direction (the temporal preorder)
from the remaining $K_0 - 1 = 6$ directions.  In $d=4$
spacetime (the unique value forced by the PASS + $K_0 = 7$
constraint established in O\_ACT, Thm.~\ref{thm:O-ACT},
YM Branch), the signature of a $G$-invariant nondegenerate
form with one timelike direction is $(-,+,+,+)$ by Sylvester's
law of inertia: the negative direction is the canonical time
direction; the three positive directions are the spatial ones
related by $\mathrm{SU}(K_0)$ isotropy. \qed
\end{proof}

\begin{remark}[\status{R}]
Theorem~\ref{thm:lorentzian-signature} promotes the Holding
entry ``Emergent Lorentzian Signature'' from [S-RAW] to [C],
after the anti-smuggling audit confirms that spatial isotropy
is source-descended (from Phase-A + PASS, not imported).
The Lorentzian signature is not an assumption; it is derived
from the collar algebra structure.
\end{remark}

%% ---------------------------------------------------------------
\section{Derived Continuum Eligibility: Y-Series}
\label{sec:b6-y-series}
%% ---------------------------------------------------------------

This section establishes the CMP-grade foundational theorems for the
continuum bridge program.  The Y-series removes the largest external
assumptions from the continuum interface, replacing them with
theorems derived from NFC structural principles.

\begin{theorem}[\status{C}]\label{thm:derived-continuum-eligibility}
\textup{(Derived Continuum Eligibility, Y2.)}
\textup{[Conditional on Phase-A, LS-2, and the existence of a
point-separating eventually-coherent observable algebra $\mathcal{A}$
on the directed observable system $({\mathcal{O}_i, \widetilde{T}_{ij}})_{i \in I}$.]}
Let $M_{\mathrm{eff}}$ be the character space of $\mathcal{A}$ (all
unital evaluation functionals $\chi:\mathcal{A} \to \mathbb{R}$) with
the weak evaluation topology, and let $D \subseteq M_{\mathrm{eff}}$ be
the realized domain where distinguished observables stabilize.  Under
the four conditions: (D1) point separation, (D2) scale-normalized
increment convergence, (D3) persistence-simple closure, (D4) realized
domain non-degeneracy, $D$ is a derived effective geometric carrier and
every $f \in \mathcal{A}_D^\infty$ defines $\hat{f} \in C^\infty(D)$.
\end{theorem}

\begin{proof}
$M_{\mathrm{eff}}$ is the Gelfand character space of $\mathcal{A}$:
the carrier is \emph{derived} from the observable algebra, not
assumed as a manifold.  Point separation (D1) makes $M_{\mathrm{eff}}$
Hausdorff.  Scale-normalized increment convergence (D2) gives the
limit algebra $\mathcal{A}_D^\infty$ of smooth observables on $D$.
Persistence-simple closure (D3) ensures $\mathcal{A}_D^\infty$ is
closed under derivations and algebra operations.  Non-degeneracy (D4)
ensures $D$ is non-empty and genuinely geometric.  The Gelfand
representation then identifies $\hat{f}(\chi) = \chi(f)$ as a smooth
function on $D$ for every $f \in \mathcal{A}_D^\infty$. \qed
\end{proof}

\begin{remark}[\status{R}]
Theorem~\ref{thm:derived-continuum-eligibility} is the CMP-grade
version of the effective representation theorems
(Thms.~\ref{thm:effective-rep-existence}--\ref{thm:effective-rep-uniqueness}).
The key upgrade is that spacetime/carrier is now \emph{derived from
the observable algebra}, not assumed.  This removes the largest
external assumption from the Book~VI continuum bridge program.
\end{remark}

\begin{theorem}[\status{C}]\label{thm:refinement-stability}
\textup{(Refinement Stability, Y2b.)}
\textup{[Conditional on Phase-A, persistence-simple structure, LS-2.]}
Let $\Phi = (\Phi_j)$ be a persistence-simple observable family
satisfying: (R1) local support, (R2) transport compatibility,
(R3) null-space stability, (R4) persistence-simple minimality,
(R5) $\varepsilon_j \to 0$.  Then
$\|\Phi_k - \widetilde{T}_{jk}(\Phi_j)\|_{\mathcal{O}_k}
\leq C \cdot \varepsilon_j$ for all $k \geq j \geq j_0$.
\end{theorem}

\begin{proof}
\textbf{Step 1.}  Quotient compatibility (from transport invariance):
zeroth-order drift is removed, leaving a first-order residual.

\textbf{Step 2.}  Local support (R1): defect region scales with
$\varepsilon_j$; contributions outside the $\varepsilon_j$-collar
are negligible.

\textbf{Step 3.}  Persistence-simple minimality (R4): any order-$1$
defect would produce a second transport-invariant component,
contradicting minimality (Book~II,
Def.~\ref{def:persistence-simple}).  Hence the defect is first-order
in $\varepsilon_j$.

\textbf{Step 4.}  Combining Steps~1--3:
$\|\Phi_k - \widetilde{T}_{jk}(\Phi_j)\| \leq C\varepsilon_j$.

\textbf{Key structural insight.}  First-order refinement stability is
not an extra assumption: it is \emph{forced} by transport invariance +
locality + persistence-simple minimality. \qed
\end{proof}

\begin{theorem}[\status{C}]\label{thm:coherent-increment-regularity}
\textup{(Coherent Increment Regularity, Y2a.)}
\textup{[Conditional on Theorem~\ref{thm:refinement-stability} and
Phase-A.]}
Let $\Phi$ be a persistence-simple observable family satisfying
(I1)--(I4) (local support, transport invariance, persistence-simple
minimality, $\varepsilon_j \to 0$).  Then $\Phi$ admits coherent
increments of all finite orders on $D$, the algebra generated by
persistence-simple observables lies in $\mathcal{A}_D^\infty$, and
each such observable defines $\hat{\Phi} \in C^\infty(D)$.
\end{theorem}

\begin{proof}
Refinement Stability (Thm.~\ref{thm:refinement-stability}) gives
first-order convergence.  Higher-order convergence follows by
induction: at each order $k$, the $k$-th increment satisfies the
same local-support + persistence-simple argument, bounding the
$k$-th order defect by $C_k \varepsilon_j^k$.  Closure under algebra
operations follows from the persistence-simple closure property (R3),
since persistence-simple elements form a ring under admissible
composition (Book~II, Def.~\ref{def:persistence-simple}).  The
Gelfand representation (Thm.~\ref{thm:derived-continuum-eligibility})
then identifies each such element with $\hat{\Phi} \in C^\infty(D)$.

\textbf{Key structural insight.}  The same persistence-simple
structure that selects $E_8$ (by the absorbed-mode elimination,
YM Branch Lem.~\ref{lem:absorbed-mode-elimination}) also guarantees
smooth regularity under refinement --- a deep internal consistency
of the NFC framework. \qed
\end{proof}

\begin{corollary}[\status{C}]\label{cor:y-series-upgrade}
\textup{(Y-Series Upgrade to Effective Representation.)}
Under the conditions of
Theorems~\ref{thm:derived-continuum-eligibility}--\ref{thm:coherent-increment-regularity},
the effective representation theorems
(Thms.~\ref{thm:effective-rep-existence}--\ref{thm:effective-rep-uniqueness})
hold with the carrier $M_{\mathrm{eff}}$ derived from the observable
algebra, and the smooth structure on $D$ inherited from the coherent
increment algebra.  This removes all external manifold assumptions
from the Book~VI foundation.
\end{corollary}

\begin{proof}
Thm.~\ref{thm:derived-continuum-eligibility} provides the derived
carrier; Thm.~\ref{thm:coherent-increment-regularity} provides the
smooth structure.  Together they supply the inputs required by
Thms.~\ref{thm:effective-rep-existence}--\ref{thm:effective-rep-uniqueness}
without any externally assumed manifold. \qed
\end{proof}
