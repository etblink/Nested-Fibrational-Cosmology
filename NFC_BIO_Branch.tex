...
This branch studies whether biological structure --- self-replication,
...
architecture.  Its task is not to import the standard language of
cells, genes, fitness landscapes, proteins, or metabolic pathways as
primitives.  Its task is to determine when such language is licensed
as an effective encoding of certified discrete observable structure
within a declared branch regime.  That posture is required by
...
is assumed.  No gene is assumed.  No fitness landscape is assumed.  No
vitalistic force or biological essence is assumed.  Each such notion,
...
\begin{openobligation}[\status{O}]\label{ob:bio-REP}
...
\begin{openobligation}[\status{O}]\label{ob:bio-MET}
...
\begin{openobligation}[\status{O}]\label{ob:bio-FID}
...
\begin{openobligation}[\status{O}]\label{ob:bio-NC}
...
\begin{openobligation}[\status{O}]\label{ob:bio-BND}
...
\begin{openobligation}[\status{O}]\label{ob:bio-HER}
...
\begin{openobligation}[\status{O}]\label{ob:bio-EVO}
...
\begin{openobligation}[\status{O}]\label{ob:bio-MULTI}
...
\begin{openobligation}[\status{O}]\label{ob:bio-END}
\textup{(O-BIO.END: Biology Endpoint.)}
...
\textup{(Replication Certification --- O-BIO.REP Discharged.)}
...
\textup{(Metabolic Bookkeeping --- O-BIO.MET Discharged.)}
...
\textup{(Replication Fidelity Margin --- O-BIO.FID Discharged.)}
\textup{[Conditional on: O-BIO.REP discharged
...
\textup{(No Hidden Hereditary Channel --- O-BIO.NC Discharged.)}
...
\textup{(Hereditary Descent --- O-BIO.HER Discharged.)}
...
\begin{openobligation}[\status{O}]\label{ob:bio-EVO-pop-bridge}
\textup{(O-BIO.EVO-Bridge: Population Dynamics Book~VI Bridge --- Remains Open.)}
Continuous-time selection dynamics require a Book~VI bridge theorem:
prove that the discrete hereditary transition family $(P_n)_{n\ge0}$ is
asymptotically coherent in a declared population-interface regime, so
that continuous-time population laws are licensed as effective
shorthand.  No such bridge theorem is available in the current canon.
\end{openobligation}
...
\begin{openobligation}[\status{O}]\label{ob:bio-DEV-scc-dep}
\textup{(O-BIO.DEV-SCC: SCC Role-Carrier Dependency --- Conditional Frontier.)}
Thm.~\ref{thm:bio-role-MCS} is conditional on SCC-MCS available for
the BIO role observable.  The SCC branch (\texttt{thm:scc-mcs})
conditionally discharges MCS existence under SCC-SV, SCC-TL, SCC-FL,
SCC-LB, and the declared UCTI/depth-sum/threshold-stability structure.
Whether those conditions hold for BIO role observables specifically
requires a branch-local verification that the BIO role support
structure satisfies SCC support sufficiency and finite localizability.
This is the one remaining gating dependency for the developmental
endpoint.
\end{openobligation}
