...
\emph{transport-invariant} if for all $j \geq i$,
\[
  \widetilde{T}_{ij}([v]) = [T_{ij}(v)]
\]
remains in the same equivalence class under transport equivalence.
\end{definition}
...
coefficients $a_k \in \mathbb{R}$, such that under arbitrarily small
perturbations of the coefficients:
\[
  \sum_{k=1}^m (a_k + \delta_k) v_k \in \mathcal{N}_i.
\]
That is, $r$ depends on fine coefficient cancellation to remain
observable.
\end{definition}
...
large $j$, transport amplifies imbalance.''  This is structurally
sound but requires a precise norm estimate on the collar system for a
full P3-grade proof; see the Canon Ledger for the outstanding
collar-geometry lemma.
\end{remark}
...
\begin{proof}
That $v_{\mathrm{branch}}$ satisfies conditions (i)--(iii) of
Definition~\ref{def:persistence-simple} is a direct check against the
collar transfer system: $v_{\mathrm{branch}}$ is nontrivial, transports
stably, and admits no nontrivial invariant splitting.  That
$v_{\mathrm{internal}}$ is a balanced residual follows from the
coefficient identity $E_V + E_C - 2E_S \to 0$ under the admissible
coefficient perturbation $E_V,E_C \to E_V + \delta, E_C - \delta$; by
Lemma~\ref{lem:residual-instability}, it is not transport-invariant.
\end{proof}
